...
if ( ! class_exists( 'Lumen_Global_Settings' ) ) {

	/**
	 * Lumen Global Settings
	 */
    class Lumen_Global_Settings {
		...
		/**
		 * Corresponds to the lumen_global_force_typography option for forcing important
		 *
		 * @var boolean
		 */
		 ...
		 */
  		function __construct() {
			...
			if ( lumen_is_frontend() ) {
				...
				add_filter( 'lumen_inline_styles_nodep', array( $this, 'color_add_global_styles' ) );
				...
				$typography = get_option( 'lumen_global_typography' );
				if ( ! empty( $typography ) && is_array( $typography ) ) {
					$this->force_typography = get_option( 'lumen_global_force_typography' );
				...
				if ( in_array( $this->get_apply_typography_to(), array( 'blocks-lumen-native', 'blocks-all' ) ) ) {
					add_filter( 'render_block', array( $this, 'typography_detect_native_blocks' ), 10, 2 );
				}

				// Fixes columns issue with Native Posts block.
				add_filter( 'lumen_global_typography_selectors', array( $this, 'posts_block_columns_fix' ), 10, 2 );

				// Add our global typography styles in the frontend only.
				add_filter( 'lumen_inline_styles_nodep', array( $this, 'typography_add_global_styles' ) );
			}
		}
		...
		public static function get_four_range_properties() {
			...
			return Lumen_Global_Settings::create_global_schema( $four_range_type );
		}
		...
		public static function get_string_four_range_properties() {
			...
			return Lumen_Global_Settings::create_global_schema( $string_four_range_type );
		}
		...
		public static function get_string_properties() {
			...
			return Lumen_Global_Settings::create_global_schema( $string_type );
		}
		...
		public static function get_number_properties() {
			...
			return Lumen_Global_Settings::create_global_schema( $number_type );
		}
		...
		public function register_global_settings() {
			...
			$lumen_global_typography_schema = array(
				'type' => 'object',
				'properties' => array(
					'fontFamily' => array(
						'type' => 'string',
					...
					'show_in_rest' => array(
						'schema' => array(
							'items' => array(
								'type' => 'object',
								'properties' => array(
									'h1' => $lumen_global_typography_schema,
									'h2' => $lumen_global_typography_schema,
									'h2' => $lumen_global_typography_schema,
									'h3' => $lumen_global_typography_schema,
									'h4' => $lumen_global_typography_schema,
									'h5' => $lumen_global_typography_schema,
									'h6' => $lumen_global_typography_schema,
									'p' => $lumen_global_typography_schema,
									'.lmn-subtitle' => $lumen_global_typography_schema,
									'.lmn-button__inner-text' => $lumen_global_typography_schema,
								)
							)
						)
					),
		...
		/**
		 * Updates the old "use Lumen colors only" option to the new individual hide options.
		 *
		 * @return void
		 *
		 * @since 3.11.0
		...
		public function fix_deprecated_options() {
			if ( ! empty( get_option( 'lumen_global_colors_palette_only' ) ) ) {
				update_option( 'lumen_global_hide_theme_colors', '1', 'no' );
				update_option( 'lumen_global_hide_default_colors', '1', 'no' );
				update_option( 'lumen_global_hide_site_editor_colors', '1', 'no' );
				delete_option( 'lumen_global_colors_palette_only' );
			}
		}
		...
		public function color_add_global_styles( $current_css ) {
			...
			return apply_filters( 'lumen_global_colors_frontend_css', $current_css, $colors );
		}
		...
		public function typography_parse_global_styles() {
			...
			foreach ( $active_typography as $tag => $styles ) {
				...
				if ( isset( $styles['fontFamily'] ) ) {
					Lumen_Google_Fonts::register_font( $styles['fontFamily'] );
				}
			...
			if ( count( $css ) ) {
				...
				$inline_css .= implode( "\n", $css );
				$this->generated_typography_css = apply_filters( 'lumen_frontend_css', $inline_css );
			}
		}
		...
		*/
		...
		public function form_class_selector( $selector ) {
			...
			// Include Lumen blocks.
			$selectors[] = '.lmn-block ' . $selector;
			$selectors[] = '.lmn-block' . $selector;
			...
			return apply_filters( 'lumen_global_typography_selectors', $selectors, $selector );
		}
		...
		public function form_tag_selector( $tag ) {
			...
			// Include Lumen blocks.
			$selectors[] = '.lmn-block ' . $tag;

			$apply_to = $this->get_apply_typography_to();

			// Include native blocks.
			if ( $apply_to !== 'blocks-lumen' ) {
			...
			return apply_filters( 'lumen_global_typography_selectors', $selectors, $tag );
		}
		...
		public function form_paragraph_selector() {
			...
			$is_apply_body_to_html = get_option( 'lumen_is_apply_body_to_html' );
		 ...
		 * @see src/utils/typography/styles.js createTypographyStyles function
		 *
		...
		/**
		 * @see src/utils/font.js getFontFamily function
		 */
		 ...
		 *
		 * @see src/utils/styles/index.js clampInheritedStyle
		 *
		 * @param {*} value
		...
		public function get_apply_typography_to() {
			$apply_global_typography_to = get_option( 'lumen_global_typography_apply_to' );
			return ! $apply_global_typography_to ? 'blocks-lumen-native' : $apply_global_typography_to;
		}
		...
		public static function generate_global_block_layouts( $option_name, $settings_name ) {
			...
			$defaults = Lumen_Block_Design_System::get_block_design_system();
			...
			foreach ( $block_layouts as $property => $values ) {
				$states = array_filter( $values, array( 'Lumen_Global_Settings', 'get_block_layout_states' ), ARRAY_FILTER_USE_KEY );

				foreach ( $states as $state => $value ) {
					$unit = Lumen_Global_Settings::get_block_layout_unit( $block_layouts, $property, $state );
					...
					if ( is_string( $value ) && ! is_numeric( $value ) ) {
						if ( strpos( $value, 'rgb' ) ) {
							...
							$split_value = Lumen_Global_Settings::extract_rgba( $value );
							$color = Lumen_Global_Settings::rgba_to_hex_alpha( $split_value['color'] );
						...
						$default_value = Lumen_Global_Settings::get_block_layout_defaults( $defaults, $property, $device );
						...
						$style  = Lumen_Global_Settings::append_unit_if_needed( $top, $unit ) . ' '
								. Lumen_Global_Settings::append_unit_if_needed( $right, $unit ) . ' '
								. Lumen_Global_Settings::append_unit_if_needed( $bottom, $unit ) . ' '
								. Lumen_Global_Settings::append_unit_if_needed( $left, $unit );
					} else {
						$style = $value . $unit;
					}
					...
					if ( $property === 'button-icon-size' || $property === 'icon-size' ) {
						$css[ $device ][ '--lmn-custom-' . $property ] = $style;
					}
				}
			}
