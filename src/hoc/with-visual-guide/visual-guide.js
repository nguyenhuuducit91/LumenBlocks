...
const VisualGuideer = props => {
	...
	const highlightSelector = `.editor-styles-wrapper.editor-styles-wrapper .lmn-${ uniqueId } ${ selector.replace( /%s/g, uniqueId ) }`.trim()
...
const createHightlightStyles = ( highlightType, selector, value, defaultValue, computedStyles ) => {
	switch ( highlightType ) {
		...
		case 'columns:column-gap':
			...
			${ selector } > [data-type="lumen/column"]:not(:last-child)::before {
				content: '';
				position: absolute;
				z-index: 2;
				left: 100%;
		...
		case 'column-spacing':
			...
			${ selector }::after {
				...
				top: calc(max(var(--column-spacing-top, var(--lmn-columns-spacing, ${ defaultValue })), 1px) * -1);
				border-top-width: max(var(--column-spacing-top, var(--lmn-columns-spacing, ${ defaultValue })), 1px);
				right: calc(max(var(--column-spacing-right, var(--lmn-columns-spacing, ${ defaultValue })), 1px) * -1);
				border-right-width: max(var(--column-spacing-right, var(--lmn-columns-spacing, ${ defaultValue })), 1px);
				bottom: calc(max(var(--column-spacing-bottom, var(--lmn-columns-spacing, ${ defaultValue })), 1px) * -1);
				border-bottom-width: max(var(--column-spacing-bottom, var(--lmn-columns-spacing, ${ defaultValue })), 1px);
				left: calc(max(var(--column-spacing-left, var(--lmn-columns-spacing, ${ defaultValue })), 1px) * -1);
				border-left-width: max(var(--column-spacing-left, var(--lmn-columns-spacing, ${ defaultValue })), 1px);
				background-color: transparent !important;
			}`
