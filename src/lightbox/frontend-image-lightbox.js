...
const getRootColumnsBlock = el => {
	...
	while ( currentEl ) {
		...
		// the outermost block.
		currentEl = currentEl.parentElement && currentEl.parentElement.closest( '.entry-content .lmn-block, .entry-content .wp-block-columns, .entry-content .wp-block-group' )
		if ( currentEl ) {
...
const isButtonBlock = el => {
	return el && ( el.classList.contains( 'lmn-block-button' ) || el.classList.contains( 'lmn-block-icon-button' ) || el.classList.contains( 'lmn-block-icon' ) )
}

const isImageBlock = el => {
	return el && el.classList.contains( 'lmn-block-image' )
}
...
class LumenImageLightbox {
	...
	gatherImages = () => {
		document.querySelectorAll( '.lmn--has-lightbox' ).forEach( el => {
			const imageBlock = el.querySelector( 'img[srcset], img[src]' )

			// Look for the anchor link where we can get the link to open in the
			// lightbox.
			let link = el.querySelector( '.lmn-link:not(.lmn-button)' )
			if ( isButtonBlock( el ) ) {
				link = el.querySelector( '.lmn-link' )
			}
			if ( imageBlock && ! link ) {
				link = imageBlock.closest( '.lmn-link' ) || imageBlock.closest( 'a' )
			}
			...
			this.elements.push( {
				block: link.classList.contains( 'lmn-block' ) ? link : link.closest( '.lmn-block' ),
				element: link,
				href: linkToOpen,
		...
		elementsGrouped.forEach( elementGroup => {
			...
			if ( Array.isArray( elementGroup ) ) {
				...
				const lightbox = GLightbox( {
					skin: 'clean glightbox-lmn', // Add a class to style lightbox description.
					elements,
				} )
...
window.lumenImageLightbox = new LumenImageLightbox()
domReady( window.lumenImageLightbox.init )
