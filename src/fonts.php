<?php
/**
 * Loads the Google Fonts used in Lumen blocks in the frontend.
 */
...
if ( ! class_exists( 'Lumen_Theme_Fonts' ) ) {
	class Lumen_Theme_Fonts {
		...
		public function register_rest_fields() {
			...
			register_rest_route( 'lumen/v3', '/get_theme_fonts', array(
				'methods' => 'GET',
	...
	new Lumen_Theme_Fonts();
}


if ( ! class_exists( 'Lumen_Google_Fonts' ) ) {
  class Lumen_Google_Fonts {
		...
		public function gather_google_fonts( $block_content, $block ) {
			...
			if ( ! isset( $block['blockName'] ) || ( strpos( $block['blockName'], 'lumen/' ) === false && strpos( $block['blockName'], 'lmb/' ) === false ) ) {
				return $block_content;
			}
		...
		public function is_lumen_block( $block_name ) {
			if ( ! empty( $block_name ) ) {
				return strpos( $block_name, 'lmb/' ) === 0 || strpos( $block_name, 'lumen/' ) === 0;
			}
			return false;
		}
		...
		public static function register_font( $font_name ) {
			...
			if ( Lumen_Theme_Fonts::is_theme_font( $font_name ) ) {
				self::$done_registering[ $font_name ] = true;
				return;
			}
			...
			if ( ! isset( self::$google_fonts[ $font_name ] ) ) {
				...
				if ( apply_filters( 'lumen_enqueue_font', true, $font_name ) ) {
					self::$google_fonts[ $font_name ] = true;
					self::$done_registering[ $font_name ] = true;

					// Enqueue the fonts in the footer.
					add_filter( 'wp_footer', array( 'Lumen_Google_Fonts', 'enqueue_frontend_block_fonts' ) );
				}
			}
		}
		 ...
		 */
		public static function enqueue_google_fonts( $google_fonts, $handle = 'lumen-google-fonts' ) {
