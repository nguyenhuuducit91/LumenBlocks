 ...
 */
import SVGLumenIconTextured from './images/lumen-icon-textured.svg'
...
export function colorizeIcon( SvgIcon ) {
	return cloneElement( SvgIcon, {
		...
		className: `lmn-lumen-icon-gradient ${ SvgIcon.props.className || '' }`,
	} )
}

// Add our SVG gradient placeholder definition that we'll reuse.
domReady( () => {
	if ( document.querySelector( 'svg.lmb-lumen-gradient' ) ) {
		return
	}

	const lumenGradient = document.createElement( 'DIV' )
	lumenGradient.setAttribute( 'style', 'height: 0; width: 0; overflow: hidden;' )
	lumenGradient.setAttribute( 'aria-hidden', 'true' )
	document.querySelector( 'body' ).appendChild( lumenGradient )
	createRoot( lumenGradient ).render(
		<svg
			xmlns="http://www.w3.org/2000/svg"
			className="lmb-lumen-gradient"
			height="0"
			width="0"
			style={ { opacity: 0 } }
		>
...
export function SVGLumenCategoryIcon() {
	return colorizeIcon( <SVGLumenIconTextured width="20" height="20" className="components-panel__icon" /> )
}

export function SVGLumenIcon( { className } ) {
	return <SVGLumenIconTextured className={ className } width="20" height="20" />
}

export function LumenIcon() {
	return colorizeIcon( <SVGLumenIconTextured width="20" height="20" /> )
}
