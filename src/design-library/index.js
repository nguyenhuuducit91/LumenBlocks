...
export const fetchDesignLibrary = async ( forceReset = false, version = '', type = 'patterns' ) => {
	if ( forceReset ) {
		doAction( 'lumen.design-library.reset-cache' )
		designLibrary = {}
		designs = {}
		pages = {}
	}
	...
	if ( ( type === 'patterns' && ! Object.keys( designs ).length ) ||
		...
		const results = await apiFetch( {
			path: `/lumen/v2/design_library/${ type }${ forceReset ? '/reset' : '' }`,
			method: 'GET',
		} )
...
export const setDevModeDesignLibrary = async ( devMode = false ) => {
	const results = await apiFetch( {
		path: `/lumen/v2/design_library_dev_mode/`,
		method: 'POST',
		data: {
			devmode: devMode,
		},
...
export const getDesign = async ( designId, version = '' ) => {
	...
	let design = await applyFilters( 'lumen.design-library.get-design', null, designId, meta, version )
