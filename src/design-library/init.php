...
if ( ! class_exists( 'Lumen_Design_Library' ) ) {
	/**
	 * Class Lumen Design Library
	 */
	class Lumen_Design_Library {
		...
		public function __construct() {
			...
			add_action( 'lumen_delete_design_library_cache', array( $this, 'delete_cache_v3' ) );

			add_filter( 'lumen_localize_script', array( $this, 'add_wp_theme_global_styles' ) );
		}
		...
		public function register_route() {
			register_rest_route( 'lumen/v2', '/design_library/(?P<type>[\w]+)(?:/(?P<reset>reset))?', array(
				'methods' => 'GET',
			...
			register_rest_route( 'lumen/v3', '/design_library_image', array(
				'methods' => 'POST',
		...
		public function delete_cache_v3() {
			...
			delete_transient( 'lumen_get_design_library' );
			delete_transient( 'lumen_get_design_library_json_v4' );
		...
		public function delete_cache() {
			// Delete design library.
			delete_transient( 'lumen_get_design_library_v4' );
			delete_transient( 'lumen_get_design_library_pages_v4' );

			do_action( 'lumen_delete_design_library_cache' );
		}
		...
		public function get_design_library_from_cloud( $type = 'patterns' ) {
			$transient_name = 'lumen_get_design_library_v4';
			$filename = 'library.json';

			if ( $type === 'pages' ) {
				$transient_name = 'lumen_get_design_library_pages_v4';
				$filename = 'pages.json';
			}
			...
			if ( empty( $designs ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$content = apply_filters( 'lumen_design_library_retreive_body', $content_body );
		...
		public static function get_cdn_url() {
			$url = apply_filters( 'lumen_design_library_url', LUMEN_DESIGN_LIBRARY_URL );
			return empty( $url ) ? '' : trailingslashit( $url );
		}

		public function add_wp_theme_global_styles( $args ) {
			$wp_global_styles = apply_filters( 'lumen.design-library.global-theme-styles', '' );
