...
import {
	...
	clearActiveTour,
	getActiveTourId,
} from '~lumen/ui/guided-modal-tour/util'
...
const ModalTour = memo( props => {
	...
	useEffect( () => {
		...
		element.className = `lmb-tour-modal__glow lmb-tour-modal__glow--hidden`
	...
	const [ modalOffsetX, modalOffsetY ] = useMemo( () => {
		...
		const modalRect = modalRef.current.querySelector( '.lmb-tour-modal' ).getBoundingClientRect()
		...
		if ( glowTarget && isVisibleDelayed ) {
			...
			if ( target ) {
				...
				if ( glowElementRef.current ) {
					glowElementRef.current.className = `lmb-tour-modal__glow lmb-tour-modal__glow--${ glowTargetSize }`
			...
			glowElementRef.current.className = `lmb-tour-modal__glow lmb-tour-modal__glow--hidden`
		}
	...
	return (
		<Modal
			title={ title }
			overlayClassName={ classNames( 'lmb-tour-modal--overlay', {
				'lmb-tour-modal--overlay--hidden': ! isVisible,
			} ) }
			...
			className={ classNames(
				'lmb-tour-modal',
				`lmb-tour-modal--${ position.replace( /-.*$/, '' ) }`,
				`lmb-tour-modal--${ position }`,
				{
					'lmb-tour-modal--visible': isVisible,
					'lmb-tour-modal--visible-delayed': isVisibleDelayed,
				} ) }
			ref={ modalRef }
		>
			<style>
			...
			{ description }
			{ help && (
				<div className="lmb-tour-modal__help">
			...
			{ ctaLabel && (
				<Button
					...
					variant="primary"
					className="lmb-tour-modal__cta"
				>
					{ ctaLabel }
				</Button>
			) }
			<Flex className="lmb-tour-modal__footer">
				<Steps
...
const Steps = memo( props => {
	...
	return (
		<div className="lmb-tour-modal__steps">
			{ Array.from( { length: numSteps } ).map( ( _, index ) => {
				const classes = classNames( [
					'lmb-tour-modal__step',
					currentStep === index && 'lmb-tour-modal__step--active',
				] )
