...
export const addBackgroundScheme = (
	...
	const getTargetBlock = blocks => {
		blocks.forEach( _block => {
			...
			const isTarget = customAttributes?.find( attribute => attribute[ 0 ] === 'lmn-design-library__bg-target' && attribute[ 1 ] === 'true' )
			if ( isTarget ) {
...
export const adjustPatternSpacing = ( attributes, category, spacingSize, isDesignLibraryDevMode ) => {
	...
	if ( additionalCss && attributes.uniqueId ) {
		attributes.generatedCss += `.lmn-${ attributes.uniqueId } {${ additionalCss }}`
	}
}
...
export const parseDisabledBlocks = parsedBlocks => {
	const disabledBlocks = settings.lumen_block_states || []
...
export const addPlaceholderForPostsBlock = ( content, postsPlaceholder, defaultValues, img = null ) => {
	...
	// Regex to match all lumen/posts blocks
	const postBlockRegex = /<!--\s*wp:lumen\/posts\s+(\{[\s\S]*?\})\s*-->([\s\S]*?)<!--\s*\/wp:lumen\/posts\s*-->/g
	...
	return normalized.replace( postBlockRegex, ( match, jsonStr, innerHtml ) => {
		...
		const templateMatch = innerHtml.match( /<!--\s*\/lmn-start:posts\/template\s*-->([\s\S]*?)<!--\s*\/lmn-end:post\/template\s*-->/ )
		if ( ! templateMatch ) {
		...
		const renderedPosts = currentPosts.map( ( post, index ) => {
			const imgSrc = img ?? `${ IMAGE_STORAGE }lmn-design-library-image-${ index + 1 }.jpeg`
			return template
				...
				.replace( /img class="lmn-img"/g, `img class="lmn-img" src="${ imgSrc }" width="${ width }" style="width: ${ width } !important;"` )
		} ).join( '\n' )

		// Replace just the template portion, keep rest of the block
		const updatedInnerHtml = innerHtml.replace(
			/<!--\s*\/lmn-start:posts\/template\s*-->([\s\S]*?)<!--\s*\/lmn-end:post\/template\s*-->/,
			renderedPosts
		)

		return `<!-- wp:lumen/posts ${ jsonStr } -->${ updatedInnerHtml }<!-- /wp:lumen/posts -->`
	} )
}
...
export const getAdditionalStylesForPreview = () => {
	...
	styles += 'body { --lmn-block-wide-width: 1000px; }'
	...
	styles += `.lmn-block-count-up__text:not(.lmn--count-up-active) { opacity: 1; }`

	// Fill the vertical line in timeline blocks
	styles += `.lmn-block-timeline { --line-bg-color: var(--line-accent-bg-color, #000); }`

	// Display correctly the progress in progress bar and progress circle blocks
	styles += `.lmn-progress-bar .lmn-progress-bar__bar { width: var(--progress-percent, 0px); }`
	styles += `.lmn-progress-circle .lmn-progress-circle__bar { stroke-dashoffset: var(--progress-dash-offset); }`

	// Set styles for maps graphic
	styles += `.lmn-block-map { height: 100%; } .lmn-block-map > img { height: 100%; width: 100%; object-fit: cover; }`
	...
	if ( ! isPro ) {
		styles += `.lmn-block-posts.is-style-horizontal-2 {
	.lmn-block-posts__item > .lmn-container {
		padding: 0;
		display: flex;
		flex-direction: row;

		> .lmn-block-posts__image-link:empty ~ .lmn-container-padding,
			.lmn-container-padding:only-child {
				width: 100%;
		}
	}
	.lmn-img-wrapper {
