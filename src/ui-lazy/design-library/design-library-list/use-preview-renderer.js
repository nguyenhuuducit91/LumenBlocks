...
import { fetchDesign } from '~lumen/design-library'
...
import { cleanSerializedBlock } from '~lumen/utils'
...
export const usePreviewRenderer = (
	...
	const adjustScale = ( force = 2, callCount = 0 ) => {
		const parentDiv = ref?.current?.querySelector( '.lmn-block-design__design-container' )
	...
	const renderPreview = ( blockContent = content ) => {
		...
		// Add a class for the first tab to be the active tab in the preview
			preview = preview.replace( '"lmn-block-tabs__tab"', '"lmn-block-tabs__tab lmn-block-tabs__tab--active"' )
		}
		...
		if ( preview.includes( 'lmn-block-map' ) ) {
			// Regex to match <div ... class="...lmn-block-map...">...</div>
			preview = preview.replace(
				/(<div[^>]*class="[^"]*lmn-block-map[^"]*"[^>]*>)([\s\S]*?)(<\/div>)/g,
	...
	useEffect( () => {
		...
		const initialize = async () => {
			...
			if ( selectedTab === 'patterns' ) {
				...
				if ( _contentForPreview.includes( 'lmn-design-library__bg-target="true"' ) ) {
