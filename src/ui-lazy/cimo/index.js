...
const CimoDownloadNotice = props => {
	...
	const onDismiss = () => {
		const settings = new models.Settings( { lumen_hide_cimo_notice: true } ) // eslint-disable-line camelcase
		settings.save()
		...
		// Update the global lumen.cimo hideNotice variable
		if ( typeof window !== 'undefined' && window.lumen?.cimo ) {
			window.lumen.cimo.hideNotice = true
		}
	...
	const pollStatus = ( action, link, pollOnce = false ) => {
		...
		fetch( ajaxUrl, {
			...
			body: new URLSearchParams( {
			  action: 'lumen_check_cimo_status',
			...
			if ( ! res.success ) {
				...
				throw new Error( 'Lumen: ' + errorMessage )
			}
			...
			if ( pollOnce || pollCountRef.current >= 3 ||
				...
				// Update the global lumen.cimo status/action variables
				// so new image block selections reflect the latest Cimo installation state
				if ( typeof window !== 'undefined' && window.lumen?.cimo ) {
					window.lumen.cimo.status = _data.status
					window.lumen.cimo.action = _data.action
				}
				return
			}
...
const CimoDownloadNoticeWrapper = props => {
	return <div className="lmn-cimo-notice"> <CimoDownloadNotice { ...props } /> </div>
}
...
domReady( () => {
	...
	const CustomDetailsView = CurrentDetailsView.extend( {
		render() {
			...
			if ( details && ! this.el.querySelector( '.lmn-cimo-notice' ) ) {
				const noticeDiv = document.createElement( 'div' )
				noticeDiv.className = 'lmn-cimo-notice'
