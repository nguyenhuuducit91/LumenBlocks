...
import { AdvancedRangeControl, Tooltip } from '~lumen/ui'
...
export const ColumnsControl = props => {
	...
	const {
		...
		if ( multiBlocks.length && multiBlocks[ 0 ].name === 'lumen/tabs' ) {
			multiBlocks.forEach( block => {
				const tabContent = block.innerBlocks[ 0 ].name === 'lumen/tab-content' ? block.innerBlocks[ 0 ] : block.innerBlocks[ 1 ]
	...
	const [ isDuplicate, setIsDuplicate ] = useLocalStorage( 'lmn__columns_new_duplicate', false )
	...
	const setColumns = numColumns => {
		const _changeColumnsFunc = ( _clientId, _numColumns, _numInnerBlocks, _innerBlocks, quietUpdate = false ) => {
			...
			if ( _numColumns === '' ) {
				...
				for ( let i = 0; i < numToAdd; i++ ) {
					const block = getBlockFromExample( 'lumen/column', pick( newBlock, [ 'attributes', 'innerBlocks' ] ) )
					if ( quietUpdate ) {
						dispatch( 'core/block-editor' ).__unstableMarkNextChangeAsNotPersistent()
					}
					insertBlock( block, _numInnerBlocks + i + 1, _clientId, false )
...
export const CloneButton = props => {
	return (
		...
		>
			<Button
				className="lmn-control-columns__settings-button"
				isSmall
				isTertiary
