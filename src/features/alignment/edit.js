	...
	const containerSize = props.hasContainerSize && <>
		...
		{ props.hasContainerHeight &&
			<AdvancedRangeControl
				...
				allowReset={ true }
				placeholder="0"
				visualGuide={ { selector: '.lmn-%s-container', highlight: 'outline' } }
			...
			<InspectorLayoutControls>
				...
				{ props.hasColumnJustify &&
					<AdvancedToolbarControl
						...
						visualGuide={ {
							selector: '.lmn-%s-column > * > * > [data-type]',
							highlight: 'outline',
						} }
				...
				{ props.hasColumnAlignment &&
					<AdvancedToolbarControl
						...
						visualGuide={ {
							...
							// block belongs to.
							selector: ', .lmn-block-columns:has( > .lmn-inner-blocks > * > * > [data-type="lumen/column"] > * > .lmn-%s)',
							highlight: 'outline-second-offset',
						} }
						className="lmb-column-align-control"
					/>
				}
				{ props.hasRowAlignment &&
					<AdvancedToolbarControl
						...
						visualGuide={ { selector: '.lmn-%s-column > * > * > [data-type]', highlight: 'outline' } }
					/>
				}
				{ props.containerSizePriority === 5 && containerSize }
				...
				{ ( props.hasColumnAlignment || props.hasBlockAlignment ) &&
					<AdvancedToolbarControl
						...
						visualGuide={ {
							selector: '.lmn-%s-container, .lmn-%s-container > * > .block-editor-block-list__layout > [data-type]',
				...
				{ innerBlockOrientation && // This is "column gap" when the blocks are horizontal.
					<AdvancedRangeControl
						...
						visualGuide={
							innerBlockWrap !== 'wrap'
								? {
									selector: '.lmn-%s-container > * > .block-editor-block-list__layout',
...
if ( userAgent && userAgent.indexOf( 'Firefox' ) !== -1 ) {
	addFilter( 'lumen.block-components.block-div.classnames', 'alignment-editor-has-polyfill', ( classes, props ) => {
		...
		if ( innerBlocks.length > 0 ) {
			for ( let i = 0; i < innerBlocks.length; i++ ) {
				...
				if ( blockMargin && ( blockMargin.top === 'auto' || blockMargin.bottom === 'auto' ) ) {
					classes.push( 'lmn--alignment-polyfill' )
				}
			}
		}
		return classes
