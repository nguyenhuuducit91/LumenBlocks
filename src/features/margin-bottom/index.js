...
import { ResizableBottomMargin } from '~lumen/ui'
import { getUniqueBlockClass } from '~lumen/utils'
import { useBlockAttributesContext, useBlockContextContext } from '~lumen/hooks'
...
export const MarginBottom = memo( props => {
	...
	const parentInnerBlockOrientation = useBlockContextContext( context => {
		return context[ 'lumen/innerBlockOrientation' ]
	} )
	...
	const enable = applyFilters( 'lumen.edit.margin-bottom.enable-handlers', true, parentBlock )
	if ( ! enable ) {
		return null
	}
