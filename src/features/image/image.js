...
import { useDynamicContent } from '~lumen/ui/dynamic-content-control'
import { ResizerTooltip } from '~lumen/ui'
...
const getImageWrapperClasses = props => {
	return classnames( [
		...
		// Shape.
		'lmn-img--shape': props.shape,

		// Firefox doesn't do stretching via SVG attribute and needs to be done via CSS.
		'lmn-image--shape-stretch': props.shapeStretch,

		// Shadow is only available when there is no shape.
		[ `lmn--shadow-${ props.shadow }` ]: ! props.shape && props.shadow,

		'lmn-img--gradient-overlay': props.hasGradientOverlay,

		'lmn--has-lightbox': props.hasLightbox,
	} )
}
...
const Image = memo( props => {
	...
	const isImageBlock = useMemo( () => {
		return select( 'core/block-editor' ).getBlockName( clientId ) === 'lumen/image'
	}, [ clientId ] )
	...
	const imageWrapperClasses = classnames( [
		...
		'lmn-img-placeholder': ! src || hasImageError,
		'lmn--never-resized': ( ! src || hasImageError ) && neverResized,
		'lmn--is-resizing': isResizing,
		'lmn--no-click-to-edit': ! props.enableClickToEdit,
		// If the image is too small, hide the size tooltip.
		'lmn--too-small': imageWidthIsTooSmall,
	} )
	...
	return (
		<ResizableBox
			...
			onResize={ ( _event, _direction, elt, delta ) => {
				...
				if ( props.heightUnit === '%' ) {
					...
					setTempStyle( `.lmn--is-resizing { height: ${ currentHeight }% !important; }` )
				} else {
		...
		>
			{ src && props.onRemove && props.hasRemove && props.showTooltips && (
				<button
					className="lmn-img-upload-remove"
					onClick={ ev => {
						props.onRemove()
						ev.stopPropagation()
					} }
			...
			<div className="lmn-img-resizer-wrapper" ref={ wrapperRef }>
				<img
					ref={ imageRef }
			...
			{ ( isSelected && props.enableClickToEdit ) && (
				<MediaUpload
					...
					render={ obj => {
						...
						return (
							<Button
								className="lmn-img-media-manager-button"
								onClick={ () => obj.open() }
							/>
						)
					} }
...
const ImageContent = props => {
	...
	return (
		applyFilters( 'lumen.image.save.wrapper',
