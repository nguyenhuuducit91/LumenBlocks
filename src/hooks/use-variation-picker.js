...
import { VariationPicker } from '~lumen/ui'
	...
	return uniqueId ? null : (
		<VariationPicker
			...
			onSelect={ ( _nextVariation = defaultVariation ) => {
				...
				const nextVariation = applyFilters( 'lumen.variation-picker.variation-selected', _nextVariation, block.name )

				const _activeVariation = getActiveBlockVariation( block.name, block.attributes )
				const activeVariation = applyFilters( 'lumen.variation-picker.variation-selected', _activeVariation, block.name )
				...
				dispatch( 'core/block-editor' ).updateBlockAttributes( clientId, newAttributes ) // eslint-disable-line lumen/no-update-block-attributes
						...
						Object.keys( currentBlockTypes ).forEach( blockName => {
							currentBlockTypes[ blockName ].forEach( ( currentBlock, index ) => {
								if ( nextVariationBlockTypes[ blockName ] ) {
									...
									} else if ( blockName === 'lumen/button' || blockName === 'lumen/icon-button' ) {
