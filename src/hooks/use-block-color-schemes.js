...
import { getPropertyLabel } from '~lumen/utils'
...
export const useBlockColorSchemes = () => {
	const {
		...
		} = select( 'lumen/global-color-schemes' ).getSettings()

		const allColorSchemes = applyFilters( 'lumen.global-settings.global-color-schemes.custom-color-schemes', colorSchemes, true )
		...
		const colorSchemesCollection = allColorSchemes.reduce( ( obj, _scheme ) => {
			...
			// 	scheme.colorScheme.backgroundColor.desktop = 'var(--lmn-block-background-color)'
			// }
		...
		// (e.g., backgroundColor --> --lmn-scheme-default-1-background-color)
		const toKebab = ( property, slug ) => {
			const result = kebabCase( property )
			return '--lmn-' + slug + '-' + result
		}
