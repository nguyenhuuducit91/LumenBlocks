...
const getLinkedData = () => {
	return JSON.parse( window.localStorage.getItem( `lumen-linking` ) ) || []
}
...
export const useLinking = () => {
	...
	useEffect( () => {
		...
		window.addEventListener( 'lumen-linked-storage', handleLinkedChanged ) // eslint-disable-line @wordpress/no-global-event-listener
		return () => window.removeEventListener( 'lumen-linked-storage', handleLinkedChanged ) // eslint-disable-line @wordpress/no-global-event-listener
	}, [] )
	...
	const setIsLinked = isLinked => {
		...
		// Update localStorage
		window.localStorage.setItem( `lumen-linking`,
			JSON.stringify(
		...
		window.dispatchEvent( new window.CustomEvent( 'lumen-linked-storage' ) )
	}
...
export const useIsLinked = clientId => {
	const attributes = useBlockAttributes( clientId ) // eslint-disable-line lumen/no-use-block-attributes
