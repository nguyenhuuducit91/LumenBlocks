...
const STORE_ACTIONS = {
	updateSelectedBlock: ( clientId, editorDom ) => {
		...
		const parentHoverEl = blockEl?.closest( '.lmn-hover-parent' )?.closest( '[data-block]' )
		...
		const collapsedEl = blockEl?.closest( '.lmn-block-accordion' )?.closest( '[data-block]' ) || ( blockEl?.getAttribute( 'data-type' ) === 'lumen/accordion' ? blockEl : null )
...
register( createReduxStore( 'lumen/hover-state', {
...
export const useBlockHoverState = ( { forceUpdateHoverState = false } = {} ) => {
	...
	const {
		...
		const hoverState = select( 'lumen/hover-state' ).getHoverState()
		const hoverStateClientId = select( 'lumen/hover-state' ).getSelectedBlock()
		...
		if ( isBlockSelected ) {
			if ( hoverState === 'hover' || hoverState === 'parent-hover' ) {
				blockHoverClass = 'lmn--is-hovered'
			}
...
export const useBlockHoverClass = () => {
	...
	const {
		...
		return {
			hoverState: select( 'lumen/hover-state' ).getHoverState(),
			parentHoverChildrenClientIds: select( 'lumen/hover-state' ).getSelectedParentHoverBlockChildren(),
		}
	}, [] )
