...
import DEFAULT_PRESETS from '~lumen/extensions/global-settings/preset-controls/presets.json'
...
export const usePresetControls = property => {
	...
	// Get the theme/default presets if the user have one, else return the lumen presets
	...
	const getPresetMarks = ( { customOnly = false, addNonePreset = false } = {} ) => {
		...
		return presets
			...
			.map( preset => ( {
				...preset,
				value: `var(--lmn--preset--${ prefix }--${ preset.slug }, ${ preset.size })`,
			} ) )
	}
