...
if ( ! function_exists( 'lumen_load_videopopup_frontend_script' ) ) {
	function lumen_load_videopopup_frontend_script() {
		if ( ! is_admin() ) {
			wp_enqueue_script(
				'lmn-frontend-video-popup',
				plugins_url( 'dist/frontend_block_video_popup.js', LUMEN_FILE ),
				array(),
				LUMEN_VERSION,
				true
			);
	...
	add_action( 'lumen/video-popup/enqueue_scripts', 'lumen_load_videopopup_frontend_script' );
}

if ( ! class_exists( 'Lumen_Video_Popup_Schema' ) ) {
	class Lumen_Video_Popup_Schema {
		public $video_entities = [];

		function __construct() {
			add_filter( 'render_block_lumen/video-popup', array( $this, 'render_block_video_popup_schema' ), 10, 2 );
		...
		public function render_block_video_popup_schema( $block_content, $block ) {
			...
			if ( isset( $block[ 'innerBlocks' ] )
				...
				&& $block[ 'innerBlocks' ][ 1 ][ 'blockName' ] === 'lumen/image'
			) {
