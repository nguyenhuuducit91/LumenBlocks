...
import { getVideoProviderFromURL, urlIsVideo } from '~lumen/utils'
import {
				...
				>
					...
					{ props.videoLink && <>
						...
						<AdvancedTextControl
							...
							className="lmn-components-datetime__date-input"
...
addFilter( 'lumen.edit.margin-bottom.enable-handlers', 'lumen/video-popup', ( enabled, parentBlock ) => {
	return parentBlock?.name === 'lumen/video-popup' ? false : enabled
} )

// Disable links for image block.
addFilter( 'lumen.edit.image.enable-link', 'lumen/video-popup', ( enabled, parentBlock ) => {
	return parentBlock?.name === 'lumen/video-popup' ? false : enabled
} )
