...
const Edit = props => {
	...
	const moveActiveTabRight = () => {
		...
		const tabContentBlock = parentBlock.innerBlocks[ 0 ].name === 'lumen/tab-content' ? parentBlock.innerBlocks[ 0 ] : parentBlock.innerBlocks[ 1 ]
	...
	const addNewTab = index => {
		...
		setTimeout( () => { // We need to wait a bit for the templateLock to get applied to the tabContent component.
			const tabContentBlock = parentBlock.innerBlocks[ 0 ].name === 'lumen/tab-content' ? parentBlock.innerBlocks[ 0 ] : parentBlock.innerBlocks[ 1 ]
			const block = getBlockFromExample( 'lumen/column', {
				attributes: {
	...
	const blockStyles = _blockStyles.map( style => {
		let disabled = false
		if ( context[ 'lumen/tabOrientation' ] === 'vertical' ) {
	...
	return (
		<>
			...
			>
				<div className="lmn-block-tab-labels__wrapper">
					{ props.attributes.tabLabels.map( ( tab, index ) => {
						return (
							// eslint-disable-next-line jsx-a11y/role-supports-aria-props
							<button
								className={ classnames( 'lmn-block-tabs__tab', {
									'lmn-block-tabs__tab--active': activeTab === index + 1,
								} ) }
...
const InspectorControls = memo( props => {
	return (
		<>
			...
			<InspectorLayoutControls>
				{ props.context[ 'lumen/tabOrientation' ] !== 'vertical' && <>
					<AdvancedToggleControl
				...
				{ ( ( props.attributes.iconPosition === '' || props.attributes.iconPosition === 'right' ) && props.attributes.showIcon ) && // Check if icon position is left or right
							( ( props.context[ 'lumen/tabOrientation' ] !== 'vertical' && props.attributes.fullWidth ) || props.context[ 'lumen/tabOrientation' ] === 'vertical' ) && // Show if its in horizontal position and fullWidth OR show on vertical position
								<AdvancedToggleControl
				...
				{ ( props.attributes.fullWidth || props.context[ 'lumen/tabOrientation' ] === 'vertical' || props.attributes.iconPosition === 'top' || props.attributes.iconPosition === 'bottom' ) && <>
					<AlignButtonsControl
