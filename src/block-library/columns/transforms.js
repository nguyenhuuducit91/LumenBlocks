	...
	from: [
		{
			...
			__experimentalConvert: blocks => {
				// Avoid transforming a single `lumen/column` Block
				if ( blocks.length === 1 && blocks[ 0 ].name === 'lumen/column' ) {
					return
				}
				...
				if ( blocks.length === 1 && blocks[ 0 ].name === 'core/columns' ) {
					...
					return createBlock( 'lumen/columns', newColumnsAttributes, newInnerBlocks )
				}
				...
				const column = createBlock( 'lumen/column', {}, groupInnerBlocks )
				const block = createBlock( 'lumen/columns', {}, [ column ] )
