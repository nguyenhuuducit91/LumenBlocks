...
import { useQueryLoopInstanceId } from '~lumen/utils'
...
const Edit = props => {
	...
	const blockClassNames = classnames( applyFilters( 'lumen.columns.edit.blockClassNames',
		[
			className,
			'lmn-block-columns',
			rowClass,
...
if ( userAgent && userAgent.indexOf( 'Firefox' ) !== -1 ) {
	addFilter( 'lumen.columns.edit.blockClassNames', 'lumen/columns-has-single-block-polyfill', ( classes, props ) => {
		...
		if ( numInnerBlocks === 1 ) {
			classes.push( 'lmn-block-columns--has-single-block-polyfill' )
		}
