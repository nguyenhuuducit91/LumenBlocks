...
import { substituteCoreIfDisabled, BLOCK_STATE } from '~lumen/utils'
	...
	if ( matches ) {
		try {
			const result = await apiFetch( {
				path: '/lumen/v3/design_library_image',
				method: 'POST',
			...
			console.error( 'Lumen Design Library:', result.message ) // eslint-disable-line no-console
			return value
		} catch ( error ) {
			console.error( 'Lumen Design Library:', error.message ) // eslint-disable-line no-console
			return value
		}
	}
	return value
...
const Edit = props => {
	...
	const blockProps = useBlockProps( {
		className: 'lmb-design-library-block',
	} )
	...
	const createBlockWithAttributes = async ( category, blockName, attributes, innerBlocks, substituteBlocks, parentClientId ) => {
		...
		// Recursively substitute core blocks to disabled Lumen blocks
		...
		const cleanBlockAttributes = async blocks => {
			for ( const block of blocks ) {
				...
				const isDesignLibraryDevMode = devMode && localStorage.getItem( 'lmn__design_library__dev_mode' ) === '1'
				if ( ! isDesignLibraryDevMode ) {
					const indexToDelete = customAttributes?.findIndex( attribute => attribute[ 0 ] === 'lmn-design-library__bg-target' )
		...
		if ( ! isDesignLibraryDevMode ) {
			if ( category !== 'Header' ) {
				...
				const blockLayouts = select( 'lumen/global-spacing-and-borders' ).getBlockLayouts()
	...
	const addDesigns = async substituteBlocks => {
		...
		for ( const blockDesign of designs ) {
			...
			for ( const patterns of designData ) {
				...
				if ( name && attributes ) {
					const block = await createBlockWithAttributes( category, name, applyFilters( 'lumen.design-library.attributes', attributes ), innerBlocks || [], substituteBlocks, parentClientId )
					blocks.push( block )
				} else {
	...
	const onClickTertiary = () => {
		...
		const settingsPageUrl = `/wp-admin/options-general.php?page=lumen`
		...
		if ( newWindow ) {
			newWindow.onload = () => { // Wait for the new page to fully load
				setTimeout( () => {
					try {
						const blocksTab = newWindow.document.getElementById( 'lmn-tab__blocks' )
						if ( blocksTab ) {
							blocksTab.click()

							newWindow.postMessage( {
								type: 'LMN_ENABLE_BLOCKS', blocks: disabledBlocks, source: 'LMN_DESIGN_LIBRARY',
	...
	if ( attributes.previewMode ) {
		...
		return (
			<div className="lmb-design-library-block">
	...
	return (
		<div { ...blockProps }>
			<Placeholder
				icon={ <SVGLumenIcon /> }
				label={ __( 'Lumen Design Library', i18n ) }
			...
			{ isDialogOpen !== DIALOG_OPTIONS.CLOSE &&
				<Modal
					className="lmb-design-library__confirm-dialog"
					...
					<VStack spacing={ 8 }>
						...
						{ isDialogOpen === DIALOG_OPTIONS.DISABLED_BLOCKS && <>
							<div>
								...
								<p> { __( 'These blocks can be enabled in the Lumen settings page. Do you want to keep the disabled blocks or substitute them with other Lumen or core blocks?', i18n ) }</p>
							</div>
							<Flex direction="column" align="stretch">
								{ isInserting ? <Spinner style={ { margin: '0 auto' } } /> : <>
									<Button
