...
const Edit = props => {
	...
	useEffect( () => {
		...
		const headerEl = getDom()?.querySelector( `[data-block="${ clientId }"] [data-type="lumen/column"]` )
		const onClick = ev => {
			// Dom't open the accordion if the user is clicking on the icon.
			if ( ! ev.target.closest( '[data-type="lumen/icon"]' ) ) {
				setIsOpen( ! isOpen )
			}
		}
	...
	const blockClassNames = classnames( [
		...
		'lmn--is-open': isOpen, // This opens the accordion in the editor.
	} )
	...
	return (
		<>
			...
			<CustomCSS mainBlockClass="lmn-block-accordion" />
...
const InspectorControls = memo( () => {
	return (
		<>
			...
			<CustomCSS.InspectorControls mainBlockClass="lmn-block-accordion" />
...
addFilter( 'lumen.block-component.icon.after', 'lumen/blockquote', output => {
	...
	const accordionBlock = findLast( parentTree, pt => pt.name === 'lumen/accordion' )
	const accordionBlockDetails = getBlock( accordionBlock?.clientId )

	const columnBlock = findLast( parentTree, pt => pt.name === 'lumen/column' )
	const iconLabelBlock = findLast( parentTree, pt => pt.name === 'lumen/icon-label' )
	...
	if ( isAccordionIcon && accordionBlockDetails ) {
		...
		const defaultValue = activeVariation?.name === 'plus' ? applyFilters( 'lumen.block-component.plus.icon-close' ) : undefined
		return (
			<>
				{ output }
				<IconControl
...
addFilter( 'lumen.block-default-styles.use-saved-style', 'lumen/icon-label', ( enabled, block, parentBlockNames ) => {
	if ( block.name === 'lumen/icon-label' && parentBlockNames.length >= 2 && parentBlockNames[ parentBlockNames.length - 2 ] === 'lumen/accordion' ) {
		return false
	}
	return enabled
} )
...
addFilter( 'lumen.block-component.icon.default', 'lumen/accordion', starIcon => {
	const { clientId } = useBlockEditContext()
	const { parentTree, getBlock } = useSelect( select => {
		const { getBlock, getBlockParents } = select( 'core/block-editor' )
		const parentTree = getBlockParents( clientId ).map( parentClientId => ( { clientId: parentClientId, name: getBlock( parentClientId ).name } ) )
	...
	if ( isAccordionIcon && accordionBlockDetails ) {
		...
		return ( activeVariation?.name === 'plus' )
			? applyFilters( 'lumen.block-component.plus.icon-open' ) : defaultIcon
	}
	return starIcon
} )
