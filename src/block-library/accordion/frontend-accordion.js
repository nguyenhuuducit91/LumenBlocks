	...
	init = () => {
		...
		const RO = new ResizeObserver( entries => { // eslint-disable-line compat/compat
			return entries.forEach( entry => {
				...
				if ( el.doAnimate ) {
					...
					const doWrapHack = !! el.closest( '.lmn-block-columns' )
					let wrapper = null
		...
		const MO = new MutationObserver( function( mutations ) {
			mutations.forEach( function( mutation ) {
				...
				el.classList[
					! Array.from( el.classList ).includes( 'lmn--is-open' ) ? 'add' : 'remove'
				]( 'lmn--is-open' )
				...
				if ( el.open && el.classList.contains( 'lmn--single-open' ) ) {
					let adjacent = el.nextElementSibling
					while ( adjacent && adjacent.classList.contains( 'lmn-block-accordion' ) ) {
						if ( adjacent.open ) {
							adjacent.open = false
						}
		...
		const els = document.querySelectorAll( '.lmn-block-accordion' )
		const elsAnchors = {}
		els.forEach( el => {
			if ( ! el._LumenHasInitAccordion ) {
				el.contentEl = el.querySelector( '.lmn-block-accordion__content' )
				if ( ! isAnimationDisabled ) {
					RO.observe( el )
				}
				MO.observe( el, {
		...
		const addWrapperHack = el => {
			...
			wrapper.classList.add( 'lmn-block-accordion__wrapper' )
			el.parentNode.insertBefore( wrapper, el )
			wrapper.appendChild( el )
			const svg = el.querySelector( 'summary .lmn--svg-wrapper:not(.lmn--has-icon2)' )
			if ( svg ) {
...
window.lumenAccordion = new LumenAccordion()

// Open closed accordions when printing
// and close them again after printing
window?.matchMedia( 'print' ).addEventListener( 'change', event => {
	if ( event.matches ) {
		const els = document.querySelectorAll( 'details.lmn-block-accordion:not([open])' )
		for ( const el of els ) {
		...
		const els = document.body.querySelectorAll( 'details.lmn-block-accordion[data-wasclosed]' )
		for ( const el of els ) {
...
domReady( window.lumenAccordion.init )
