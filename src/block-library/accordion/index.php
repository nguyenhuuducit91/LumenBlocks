...
if ( ! function_exists( 'lumen_load_accordion_frontend_script' ) ) {
	function lumen_load_accordion_frontend_script() {
		if ( ! is_admin() ) {
			wp_enqueue_script(
				'lmn-frontend-accordion',
				plugins_url( 'dist/frontend_block_accordion.js', LUMEN_FILE ),
				array(),
				LUMEN_VERSION,
				true
			);
	...
	add_action( 'lumen/accordion/enqueue_scripts', 'lumen_load_accordion_frontend_script' );
}

if ( ! function_exists( 'lumen_load_accordion_frontend_polyfill_script' ) ) {
	/**
	...
	add_action( 'lumen/accordion/enqueue_scripts', 'lumen_load_accordion_frontend_polyfill_script' );
}

if ( ! class_exists( 'Lumen_Accordion_FAQ_Schema' ) ) {
	class Lumen_Accordion_FAQ_Schema {
		public $faq_entities = [];

		function __construct() {
			add_filter( 'render_block_lumen/accordion', array( $this, 'render_block_accordion_faq_schema' ), 10, 2 );
		...
		public function render_block_accordion_faq_schema( $block_content, $block ) {
			...
			if ( isset( $attributes[ 'enableFAQ' ] ) && $attributes[ 'enableFAQ' ] ) {
				...
				// retrieve lumen/column -> lumen/icon-label -> lumen/heading
				$isHeadingBlock = $block[ 'innerBlocks' ][0][ 'innerBlocks' ][0][ 'innerBlocks' ][0][ 'blockName' ] === 'lumen/heading';
				$question = '';
				if ( $isHeadingBlock ) {
	...
	new Lumen_Accordion_FAQ_Schema();
}
