...
class _LumenTabs {
	...
	getDefaultState = () => {
		...
		this.tabs = this.parentEl.querySelector( '.lmn-block-tab-labels__wrapper' ).children
		this.contents = this.parentEl.querySelector( '.lmn-block-tab-content > .lmn-inner-blocks' ).children
	...
	initTabs = () => {
		...
		Array.from( this.tabs ).forEach( ( tab, index, tabs ) => {
			...
			if ( tab.getAttribute( 'id' ) ) {
				...
				tab.setAttribute( 'id', `lmn-block-tab-label-${ this.uniqueId }-${ index + 1 }` )
				isCustomAnchor = false
			}
			tabAnchors.push( tab.getAttribute( 'id' ) )

			if ( window.location.hash === `#${ tab.getAttribute( 'id' ) }` ) {
				tabs[ this.activeTab - 1 ].classList.remove( 'lmn-block-tabs__tab--active' )
				this.activeTab = index + 1
			}

			tab.setAttribute( 'aria-controls', `lmn-block-tab-content-${ this.uniqueId }-${ index + 1 }` )
			...
			if ( this.activeTab === ( index + 1 ) ) {
				tab.classList.add( 'lmn-block-tabs__tab--active' )
			}
		...
		Array.from( this.contents ).forEach( ( content, index ) => {
			content.setAttribute( 'id', `lmn-block-tab-content-${ this.uniqueId }-${ index + 1 }` )
	...
	changeTab = tabIndex => {
		...
		Array.from( this.tabs ).forEach( ( tab, index ) => {
			...
			if ( tabIndex === ( index + 1 ) ) {
				...
				tab.classList.remove( 'lmn-block-tabs__tab--active' )
			}
		} )
		...
		this.parentEl.dispatchEvent( new CustomEvent( 'lumen-tabs-change', {
			detail: {
...
class LumenTabs {
	init = () => {
		document.querySelectorAll( '.lmn-block-tabs' )
			.forEach( el => {
				if ( ! el._LumenHasInitTabs ) {
					const tabs = new _LumenTabs( el )
					el.tabs = tabs
					el._LumenHasInitTabs = true
				}
			} )
	}
}

window.lumenTabs = new LumenTabs()
domReady( window.lumenTabs.init )
