...
const Edit = props => {
	...
	const blockClassNames = classnames( [
		...
		columnTooltipClass,
		{
			'lmn--is-slide': carouselType === 'slide',
			'lmn--is-fade': carouselType === 'fade',
			'lmn--infinite-scroll': carouselType === 'slide' && attributes.infiniteScroll,
			...
			>
				...
				<Separator>
					<div className="lmn-block-carousel__content-wrapper">
						...
						>
							{ carouselType === 'fade' && (
								<style>
									{ range( maxSlides ).map( i => {
										return `.lmn-${ attributes.uniqueId }-column > .lmn-block-carousel__slider > .block-editor-inner-blocks > .block-editor-block-list__layout > [data-type="lumen/column"]:nth-child(${ i + 1 }) {
										opacity: 0;
										visibility: hidden;
										left: ${ isRTL ? '' : '-' }${ 100 * ( i ) }%;
									}`
									} ) }
									{ `.lmn-${ attributes.uniqueId }-column > .lmn-block-carousel__slider > .block-editor-inner-blocks > .block-editor-block-list__layout > [data-type="lumen/column"]:nth-child(${ activeSlide }) {
									opacity: 1;
									visibility: visible;
									z-index: 2;
							...
							{ attributes.showArrows && (
								<div className="lmn-block-carousel__buttons">
									<button
										className="lmn-block-carousel__button lmn-block-carousel__button__prev"
										onClick={ prevSlide }
									>
										<SvgIcon value={ attributes.arrowIconPrev || defaultIcon.prev } />
									</button>
									<button
										className="lmn-block-carousel__button lmn-block-carousel__button__next"
										onClick={ nextSlide }
									>
						...
						{ attributes.showDots && (
							<div className="lmn-block-carousel__dots" role="list" data-label="Slide %d">
								{ range( maxSlides ).map( i => {
									...
									const className = classnames( 'lmn-block-carousel__dot', {
										'lmn-block-carousel__dot--active': i + 1 === dotActiveSlide,
									} )
									return (
