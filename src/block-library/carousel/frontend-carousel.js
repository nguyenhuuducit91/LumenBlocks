...
class _LumenCarousel {
	...
	init = () => {
		...
		this.wrapper = this.el.querySelector( '.lmn-block-carousel__slider-wrapper' )
		this.type = this.el.classList.contains( 'lmn--is-fade' ) ? 'fade' : 'slide'
		this.sliderEl = this.el.querySelector( '.lmn-block-carousel__slider' )
		...
		this.infiniteScroll = this.el.classList.contains( 'lmn--infinite-scroll' )
		...
		const tempDotsEls = this.el.querySelectorAll( '.lmn-block-carousel__dots' )
		const tempPrevEls = this.el.querySelectorAll( '.lmn-block-carousel__button__prev' )
		const tempNextEls = this.el.querySelectorAll( '.lmn-block-carousel__button__next' )
		...
		this.liveregion.setAttribute( 'class', 'liveregion lmn--hidden' )
	...
	initProperties = () => {
		...
		const otherInitCalls = () => {
			...
			this.slideEls[ this.currentSlide - 1 ].classList.add( 'lmn-block-carousel__slide--active' )

			this.unpauseAutoplay()
		}

		if ( this.infiniteScroll && ! this.el._LumenHasInitCarousel ) {
			// clone slides
			this.clones = []
			const clonesToAdd = []
			let lastClone = null
			...
			const runInitSteps = () => {
				if ( step === 0 ) {
					...
					clone.classList.add( `lmn-slide-clone-${ slideIndex + 1 }` )
	...
	updateDots = () => {
		...
		this.slideEls.forEach( ( slideEl, i ) => {
			...
			dotEl.classList.add( 'lmn-block-carousel__dot' )
			dotEl.setAttribute( 'aria-label', dotLabel.replace( /%+d/, i + 1 ) )
			if ( this.currentSlide === i + 1 ) {
				dotEl.classList.add( 'lmn-block-carousel__dot--active' )
			}
	...
	updateDefaultIcon = () => {
		const prevButton = this.el.querySelector( '.lmn-block-carousel__button.lmn-block-carousel__button__prev .fa-chevron-left' )
		const nextButton = this.el.querySelector( '.lmn-block-carousel__button.lmn-block-carousel__button__next .fa-chevron-right' )
		if ( prevButton ) {
	...
	goToSlide = ( slide, force = false ) => {
		...
		currentSlideEl.classList.remove( 'lmn-block-carousel__slide--active' )
		newSlideEl.classList.add( 'lmn-block-carousel__slide--active' )
		...
		this.el.dispatchEvent( new CustomEvent( 'lumen-carousel-slide-change', {
			detail: {
				element: this.el,
				currentSlide: slide,
			},
	...
	setDotActive = slide => {
		this.dotEls?.forEach( ( dotEl, i ) => {
			if ( slide === i + 1 ||
				...
				dotEl.classList.remove( 'lmn-block-carousel__dot--active' )
			}
		} )
	}
	...
	dragMouseDown = e => {
		...
		this.sliderEl.classList.add( 'lmn--snapping-deactivated' )
	...
	dragMouseUp = () => {
		...
		if ( this.type === 'slide' ) {
			...
			this.sliderEl.classList.remove( 'lmn--snapping-deactivated' )
	...
	onTouchEndIOS = e => {
		...
		if ( ! target.closest( '.lmn-block-carousel__slider' ) &&
			( target.tagName === 'A' ||
...
class LumenCarousel {
	init = () => {
		const els = document.querySelectorAll( '.lmn-block-carousel' )
		els.forEach( el => {
			if ( ! el._LumenHasInitCarousel ) {
				const carousel = new _LumenCarousel( el )
				el.carousel = carousel
				carousel.init()
				el._LumenHasInitCarousel = true
			}
		} )
	}
}

window.lumenCarousel = new LumenCarousel()
domReady( window.lumenCarousel.init )
