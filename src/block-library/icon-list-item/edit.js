			...
			>
				...
				<div className="lmn-block-icon-list-item__content">
					...
					{ ! ordered && ! icon && parentUniqueId &&
						<Icon
							value={ getUseSvgDef( `#lmn-icon-list__icon-svg-def-${ parentUniqueId }` ) }
							openEvenIfUnselected={ true }
							hasLinearGradient={ false }
						/> }
					{ ordered &&
						...
						<span className="lmn-block-icon-list-item__marker"></span>
					 }
					<Typography
						ref={ useEnterRef }
						tagName="span"
...
const withNegativeTabIndex = createHigherOrderComponent( BlockEdit => {
	return props => {
		if ( props.name !== 'lumen/icon-list-item' ) {
...
wp.hooks.addFilter(
	'editor.BlockEdit',
	'lumen/with-icon-list-negative-tab-index',
	withNegativeTabIndex
)
