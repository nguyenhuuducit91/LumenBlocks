...
function convertBlockToList( block ) {
	const list = switchToBlockType( block, 'lumen/icon-list' )
	if ( list ) {
		return list
	}
	...
	return switchToBlockType( paragraph, 'lumen/icon-list' )
}
...
export function convertToListItems( blocks ) {
	...
	for ( let block of blocks ) {
		if ( block.name === 'lumen/icon-list-item' ) {
			listItems.push( block )
		} else if ( block.name === 'lumen/icon-list' ) {
	...
	return useRefEffect(
		element => {
			function onKeyDown( event ) {
				...
				// slow with Lumen, so we just remove the excess blocks.
