...
import { currentUserHasCapability, getAttributeName } from '~lumen/utils'
...
const Edit = props => {
	...
	const { lumen_google_maps_api_key: apiKey } = settings
		...
		<>
			...
			>
				...
				>
					...
					{ resizableRef.current?.state?.isResizing && (
						<style>{ `.lmn-block.lmn-${ attributes.uniqueId } { height: auto !important; }
							.lmn-block.lmn-${ attributes.uniqueId } .lmn-block-map__canvas { height: 100% !important; }` }</style>
					) }
					{ apiKey ? (
						<div
		...
		<>
			...
			<InspectorStyleControls>
				<PanelAdvancedSettings
					...
					className={ classnames( { 'lmn--uses-api-key': props.apiKey } ) }
					initialOpen={ true }
					id="general"
				>
					{ props.userCanManageApiKey && ! props.apiKey && (
						<Notice className="lmn-block-map__api-key-notice" status="info" isDismissible={ false }>
				...
				<PanelAdvancedSettings
					...
					id="map-marker"
				>
					<div className="lmn--needs-api-key">
