...
const transforms = {
	from: [
		{
			type: 'block',
			isMultiBlock: true,
			blocks: [ 'core/paragraph', 'core/heading', 'lumen/text', 'lumen/heading', 'lumen/subtitle' ],
			transform: blockAttributes => {
				let childBlocks = []
				if ( blockAttributes.length > 1 ) {
					childBlocks = blockAttributes.map( ( { content, text } ) => {
						return createBlock( 'lumen/icon-list-item', { text: content ?? text ?? '' } )
					} )
					...
					childBlocks = split( value, '\n' ).map( result => {
						return createBlock( 'lumen/icon-list-item', {
		...
		},
		{
			...
			transform: ( blockAttributes, _childBlocks ) => {
				const childBlocks = _childBlocks.map( ( { attributes } ) => {
					return createBlock( 'lumen/icon-list-item', { text: attributes.content } )
				} )
				return createBlock(
					'lumen/icon-list',
					{
	...
	to: [
		...
		...[ 'lumen/text', 'lumen/heading' ].map( block => ( {
			type: 'block',
			blocks: [ block ],
			transform: ( _attributes, childBlocks ) => {
				return getListContentFlat( childBlocks ).map( content =>
