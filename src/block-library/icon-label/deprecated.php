...
if ( ! function_exists( 'lumen_render_block_icon_label' ) ) {
	/**
	 * Adds a special class if block uses the old icon gap attribute
	 */
	function lumen_render_block_icon_label( $block_content, $block ) {
		$attributes = $block[ 'attrs' ];

		// check if inner blocks contain an icon block
		if ( ! isset( $block[ 'innerBlocks' ][0] ) || ! isset( $block[ 'innerBlocks' ][0]['blockName'] ) || $block[ 'innerBlocks' ][0]['blockName'] !== 'lumen/icon' ) {
		...
		if ( $use_old_styles ) {
			return preg_replace( '/lmn-block-icon-label/i', 'lmn-block-icon-label lmn-block-icon-label--use-flex-basis', $block_content );
		}

		return $block_content;
	}
	add_filter( 'render_block_lumen/icon-label', 'lumen_render_block_icon_label', 10, 2 );
}
