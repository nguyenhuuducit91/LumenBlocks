...
const Notice = ( { autoGenerateAnchors } ) => {
	return (
		<Card className="lmn-table-of-contents__empty-anchor">
			<CardBody isShady>
				{ __(
...
const Edit = props => {
	...
	const blockClassNames = classnames( [
		className,
		'lmn-block-table-of-contents',
		blockAlignmentClass,
	] )

	const tableOfContentsClassNames = classnames( [
		'lmn-table-of-contents__table',
		textClasses,
	] )

	const titleClassNames = classnames(
		'lmn-table-of-contents__title',
		titleTextClasses
	)
	...
	const autoGenerateAnchors = useCallback( () => {
		const BLOCK_ANCHOR_CONTENT = applyFilters( 'lumen.table-of-contents.block-anchor-content', {
			...
			if ( block ) {
				...
				if ( ! anchor && content ) {
					...
					dispatch( 'core/block-editor' ).updateBlockAttributes( block.clientId, {
						anchor: anchor !== null ? anchor : uniqueId( 'lmn-' ),
					} )
				}
			}
		} )
	...
	return (
		<>
			...
			<CustomCSS mainBlockClass="lmn-table-of-contents" />
...
const InspectorControls = memo( props => {
	return (
		<>
			...
			<CustomCSS.InspectorControls mainBlockClass="lmn-table-of-contents" />
