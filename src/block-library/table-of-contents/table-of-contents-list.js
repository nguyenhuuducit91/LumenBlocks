	...
	if ( nestedHeadingList ) {
		...
		const childNodes = nestedHeadingList.map( childNode => {
			...
			const visibility = isSelected ? <Button
				className="lmn-block-table-of-contents__list-item__exclude-button"
			...
			const className = classnames( {
				'lmn-block-table-of-contents__list-item__hidden': isExcluded && ! isSelected,
				'lmn-block-table-of-contents__list-item__excluded': isExcluded,
			} )

			return (
				<li key={ clientId } className={ className }>
					{ isEdit ? (
						<div className="lmn-block-table-of-contents__list-item-inner">
							<span className="lmn-block-table-of-contents__link-wrapper">
								<RichText
									tagName="a"
