...
const convertBlockToHeadingObject = block => {
	...
	if ( block.name === 'lumen/heading' ) {
	...
	} else if ( block.name === 'lmb/heading' ) {
		...
		heading = applyFilters( 'lumen.block.table-of-contents.convert-block', null, block ) || { // Allow other plugins to add their own heading block.
			// Native core/heading
...
export const getHeadingsFromHeadingBlocks = ( headingBlocks, headings ) => {
	return [ ...headingBlocks ].map( headingBlock => {
		...
		const isLmnHeading = ! headingBlock.classList.contains( 'wp-block-heading' )

		const heading = isLmnHeading
			? headingBlock.querySelector( ':scope > .lmn-block > .lmn-block-heading__text' )
			: headingBlock

		let firstId
		if ( isLmnHeading ) {
			const wrapper = headingBlock.querySelector( ':scope > .lmn-block-heading' )
			firstId = wrapper
				.getAttribute( 'id' )
				?.trim()
				.split( ' ' )[ 0 ]
...
export const getHeadingsFromEditorDom = editorDom => {
	const allowedHeadingBlocks = applyFilters( 'lumen.block.table-of-contents.allowed-headings', [
		'core/heading',
		'lumen/heading',
		'lmb/heading',
	] )
