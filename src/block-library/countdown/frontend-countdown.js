...
class _LumenCountdown {
	constructor( el ) {
		this.el = el
		this.blockId = el.getAttribute( 'data-block-id' )
		this.countdownInterval
		this.date = Date.parse( el.getAttribute( 'data-lmn-countdown-date' ) )
		this.countdownType = el.getAttribute( 'data-lmn-countdown-type' )
		this.duration = parseInt( el.getAttribute( 'data-lmn-countdown-duration' ), 10 )
		this.restartInterval = parseFloat( el.getAttribute( 'data-lmn-countdown-restart-interval' ) ) || 0
		this.action = el.getAttribute( 'data-lmn-countdown-action' )
		this.timezone = el.getAttribute( 'data-lmn-countdown-timezone' ) ? { timeZone: el.getAttribute( 'data-lmn-countdown-timezone' ) } : {}
		this.isDoubleDigit = el.getAttribute( 'data-lmn-countdown-is-double-digit' ) === 'true' ? true : false
		...
		this.day = el.querySelector( '.lmn-block-countdown__digit-day' )
		this.hour = el.querySelector( '.lmn-block-countdown__digit-hour' )
		this.minute = el.querySelector( '.lmn-block-countdown__digit-minute' )
		this.second = el.querySelector( '.lmn-block-countdown__digit-second' )

		this.timer = this.el.querySelector( '.lmn-block-countdown__container' )
		this.message = this.el.querySelector( '.lmn-block-countdown__message' )
	}
...
class LumenCountdown {
	init = () => {
		const countdownBlocks = document.querySelectorAll( '.lmn-block-countdown' )
		countdownBlocks.forEach( el => {
			new _LumenCountdown( el ).init()
		} )
	}
}

window.lumenCountdown = new LumenCountdown()
domReady( window.lumenCountdown.init )
