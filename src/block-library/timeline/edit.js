...
const Edit = props => {
	...
	useEffect( () => {
		...
		for ( let i = blockIndex + 1; i < adjacentBlocks?.length; i++ ) {
			if ( adjacentBlocks[ i ].name === 'lumen/timeline' ) {
				blocksAfter++
			} else {
				break
			}
	...
	const blockClassNames = classnames( [
		...
		separatorClass,
		{
			...
			'lmn-block-timeline--last': props.attributes.timelineIsLast,
		},
	] )
	...
	const handleScroll = () => {
		...
		const fillPercent = `( ( ${ ( document?.body?.clientHeight || 10000 ) }px * var(--lmn-timeline-anchor, 0.5) ) + ${ -blockRectTop + topPadding + backgroundPadding }px ) / ${ blockHeight } * 100`
		...
		if ( ! previousBlock || previousBlock.name !== 'lumen/timeline' ) {
			fill = {
	...
	useEffect( () => {
		...
		if ( nextBlock && nextBlock.name === 'lumen/timeline' && props.attributes.timelineIsLast ) {
			dispatch( 'core/block-editor' ).__unstableMarkNextChangeAsNotPersistent()
			setAttributes( { timelineIsLast: false } )
			isUpdating = true
		} else if ( ( ! nextBlock || nextBlock.name !== 'lumen/timeline' ) && ! props.attributes.timelineIsLast ) {
			dispatch( 'core/block-editor' ).__unstableMarkNextChangeAsNotPersistent()
	...
	useEffect( () => {
		setTimelinePosition(
			{
				isFirst: ! previousBlock || previousBlock.name !== 'lumen/timeline',
				isLast: ! nextBlock || nextBlock.name !== 'lumen/timeline',
			}
		)
		...
		<>
			...
			>
				...
				>
					...
					/>
					<div
						className="lmn-block-timeline__middle"
					>
						<div
							className="lmn-block-timeline__middle__dot"
							ref={ middleRef }
						>
							<div
								className="lmn-block-timeline__middle__fill"
								style={ {
						...
						<div
							className="lmn-block-timeline__middle__branch"
							ref={ branchRef }
						>
							<div
								className="lmn-block-timeline__middle__branch__fill"
								style={ {
									height: `max(${ fillHeight.middle }, 0px)`,
					...
					<div
						className="lmn-block-timeline__vertical-line"
						style={ {
					...
					>
						<div
							className="lmn-block-timeline__vertical-line__fill"
							style={ {
					...
					{ timelinePosition.isFirst && (
						<style>
							{ range( numTimelineBlocksAfter + 1 ).map( i => {
								const adjacentSelector = range( i ).map( () => '+ [data-type="lumen/timeline"]' ).join( ' ' )
								return `[data-block="${ clientId }"] ${ adjacentSelector } {
									--lmn-timeline-anchor: ${ props.attributes.timelineAnchor === '' ? 0.5 : ( props.attributes.timelineAnchor / 100 ) };
									}
									[data-block="${ clientId }"] ${ adjacentSelector } .lmn-block-timeline {
					...
					}
					{
						<style>
							{
								`[data-block="${ clientId }"] {
									--lmn-timeline-vertical-line-bg-position: ${ backgroundPosition.verticalLine };
									--lmn-timeline-middle-bg-position: ${ backgroundPosition.middle };
								}`
							}
						</style>
					}
...
addFilter( 'lumen.resizable-bottom-margin.default', 'lumen/timeline', ( defaultBottomMargin, blockName ) => {
	return blockName === 'lumen/timeline' ? 0 : defaultBottomMargin
} )
