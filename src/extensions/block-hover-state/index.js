...
export const BlockHoverState = () => {
	const {
		...
		return {
			getEditorDom: select( 'lumen/editor-dom' ).getEditorDom,
			selectedClientId,
			...
			hoverStateClientId: select( 'lumen/hover-state' ).getSelectedBlock(),
		}
	}, [] )
	...
	useEffect( () => {
		if ( hoverStateClientId !== selectedClientId || selectedParentHoverBlock !== selectedParent ) {
			if ( selectedClientId ) {
				dispatch( 'lumen/hover-state' ).updateSelectedBlock( selectedClientId, getEditorDom() )
			} else {
				// If there's no selected block, clear the hover states.
				dispatch( 'lumen/hover-state' ).clearSelectedBlock()
			}
		}
