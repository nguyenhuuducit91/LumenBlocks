...
import { extractBlockStyleStructure, getBlocksToStyle } from '~lumen/utils'
...
export const BlockLinking = () => {
	...
	const cb = useCallback( newAttributes => {
		...
		if ( clientIdsToUpdate.length ) {
			dispatch( 'core/block-editor' ).updateBlockAttributes( clientIdsToUpdate, attributes ) // eslint-disable-line lumen/no-update-block-attributes
		}
...
const useLinkableBlockTypes = () => {
	return useMemo( () => {
		return wp.data.select( 'core/blocks' ).getBlockTypes().reduce( ( blockTypes, block ) => {
			if ( block.supports?.lmnBlockLinking ) {
	...
	return useMemo( () => {
		...
		if ( linkableClientId ) {
			...
			if ( parent ) {
				...
				if ( blockData?.supports?.lmnBlockLinking === false ) {
					return null
				}
			}
		}
...
const isSupportedBlock = blockType => {
	const isSupported = Object.keys( applyFilters( 'lumen.block-linking.blocks', {} ) ).includes( blockType )

	if ( ! isSupported && blockType.startsWith( 'lumen/' ) ) {
		// eslint-disable-next-line no-console
		console.error( `Lumen: block type "${ blockType }" was not included the list of blocks allowed for linking. Did you forget to include it? See filter "lumen.block-linking.blocks"` )
	}
...
const filterContentAttributes = ( attributes, blockType ) => {
	const blockSupport = applyFilters( 'lumen.block-linking.blocks', {} )
