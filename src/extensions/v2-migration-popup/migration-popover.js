...
const MigrationModal = () => {
	...
	const onClickEnableSometimes = () => {
		save( {
			lumen_v2_editor_compatibility: '', // eslint-disable-line camelcase
			lumen_v2_editor_compatibility_usage: '1', // eslint-disable-line camelcase
			lumen_v2_frontend_compatibility: '1', // eslint-disable-line camelcase
		}, true ) // Refresh.
	}

	const onClickEnableAlways = () => {
		save( {
			lumen_v2_editor_compatibility: '1', // eslint-disable-line camelcase
			lumen_v2_editor_compatibility_usage: '', // eslint-disable-line camelcase
			lumen_v2_frontend_compatibility: '1', // eslint-disable-line camelcase
		}, true ) // Refresh.
	}
	...
	const onClickNo = () => {
		save( {
			lumen_v2_block_detector_disabled: '1', // eslint-disable-line camelcase
		} )
	}

	return (
		<Modal
			className="lmn-v2-migration-modal"
			title={ __( 'Lumen V2 Block Detected', i18n ) }
			shouldCloseOnClickOutside={ false }
			onRequestClose={ () => setIsOpen( false ) }
		>
			<p>{ __( 'Hello! 👋', i18n ) }</p>
			<p>{ __( 'We noticed that the page you are editing contains old Lumen version 2 blocks. Starting Lumen version 3, old v2 blocks are not anymore loaded in the editor. This means that you would not be able to edit the old v2 blocks in the editor and any old v2 block would show a block error in the editor.', i18n ) }</p>
			<p>{ __( 'Do you want to enable backward compatibility, so the old blocks will be loaded in the editor?', i18n ) }</p>
			<div className="lmn-v2-migration-modal__button-wrapper">
				<Button
