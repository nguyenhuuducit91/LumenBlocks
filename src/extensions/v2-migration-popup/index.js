...
loadPromise.then( () => {
	...
	settings.fetch().then( response => {
		...
		if ( response.lumen_v2_block_detector_disabled ) {
			return
		}

		// Only do this when the v2 compatibility is not enabled at all.
		if ( ! response.lumen_v2_editor_compatibility && ! response.lumen_v2_editor_compatibility_usage ) {
			detectV2Blocks().then( hasV2Blocks => {
				if ( hasV2Blocks ) {
					registerPlugin( 'lumen-v2-migration-popup', { render: MigrationModal } )
				}
			} )
		}
	} )
