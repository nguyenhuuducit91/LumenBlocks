...
const STORE_REDUCER = ( state = DEFAULT_STATE, action ) => {
	switch ( action.type ) {
		case 'UPDATE_BLOCK_STYLES': {
			...
			const settings = new models.Settings( { lumen_global_block_styles: newBlockStyles } )
			settings.save()
...
register( createReduxStore( 'lumen/global-block-styles', {
...
domReady( () => {
	fetchSettings().then( response => {
		const {
			lumen_global_block_styles: _blockStyles,
		} = response

		const globalBlockStyles = typeof _blockStyles === 'object' && Object.keys( _blockStyles ).length ? _blockStyles : {}

		dispatch( 'lumen/global-block-styles' ).updateGlobalBlockStyles( globalBlockStyles )
	} )
} )
