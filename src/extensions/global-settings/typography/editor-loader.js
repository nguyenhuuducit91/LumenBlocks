...
export const GlobalTypographyStyles = () => {
	const { allCustomPresets } = useSelect( select => {
		const _customPresetControls = select( 'lumen/global-preset-controls.custom' )?.getCustomPresetControls() ?? {}
	...
	const updatePresetsWithTypography = ( useTypographyAsPresets, typographySettings, tags ) => {
		if ( useTypographyAsPresets ) {
			...
			let xSmallSize = typographySettings[ '.lmn-subtitle' ]?.fontSize ?? getDefaultFontSize( '.lmn-subtitle' ) ?? 16
			let xSmallUnit = typographySettings[ '.lmn-subtitle' ]?.fontSizeUnit ?? 'px'
			if ( xSmallUnit === 'px' ) {
			...
			saveTypographyAsPresetsThrottle = setTimeout( () => {
				const settings = new models.Settings( { lumen_global_custom_preset_controls: newSettings } ) // eslint-disable-line camelcase
				settings.save()
			}, 300 )

			dispatch( 'lumen/global-preset-controls.custom' ).updateCustomPresetControls( newSettings )
		}
	}
	...
	useEffect( () => {
		...
		fetchSettings().then( response => {
			setTypographySettings( ( head( response.lumen_global_typography ) ) || {} )
		...
		addAction( 'lumen.global-settings.typography.update-trigger', 'lumen/typography-styles', (
		...
		return () => {
			removeAction( 'lumen.global-settings.typography.update-trigger', 'lumen/typography-styles' )
		}
	}, [] )

	// Debounced style generation.
	useEffect( () => {
		addAction( 'lumen.global-settings.typography-update-global-styles', 'lumen/typography-styles', typographySettings => {
			...
			const styleObject = Object.keys( typographySettings ).map( tag => {
				...
				return deepmerge.all( selectors.map( selector => {
					...
					// Force styles only for Lumen blocks.
					let important = selector.includes( 'lumen/' )
					important = applyFilters( 'lumen.global-settings.typography.selector-is-lumen', important, selector )
					...
					return applyFilters( 'lumen.global-settings.typography.editor-styles', tagStyles, tag, selector, typographyStyles, important )
				} ) )
			} )
		...
		setStyleTimeout( setTimeout( () => doAction( 'lumen.global-settings.typography-update-global-styles', typographySettings ), 200 ) )

		return () => removeAction( 'lumen.global-settings.typography-update-global-styles', 'lumen/typography-styles' )
...
export const formClassOrTagSelectors = ( selector, applyTo ) => {
	...
	selectors.push( `:is([data-type^="lumen/"], .lmn-block) ${ selector }` )

	// Include native blocks.
	if ( isClassSelector || applyTo !== 'blocks-lumen' ) {
	...
	return applyFilters( 'lumen.global-settings.typography-selectors', selectors, selector )
}
...
export const formParagraphSelectors = ( applyTo, isApplyBodyToHTML ) => {
	...
	return applyFilters( 'lumen.global-settings.typography-selectors', selectors, '' )
}

// Make sure that this isn't applied to the native query loop block, otherwise
// it will wrap the last column of the block.
addFilter( 'lumen.global-settings.typography-selectors', 'lumen/native-posts-block', selectors => {
