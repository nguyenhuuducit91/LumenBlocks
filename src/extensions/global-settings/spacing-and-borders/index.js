...
import { BORDER_CONTROLS, IMAGE_SHADOWS } from '~lumen/features'
...
addFilter( 'lumen.global-settings.inspector', 'lumen/global-spacing-and-borders', output => {
	...
	const [
		...
		'lumen/global-spacing-and-borders',
	...
	return (
		<>
			...
			>
				{ isOpen && displayHoverNotice && <span className="lmn-global-block-layouts-help-tooltip">
					<HelpTooltip
						...
						onClose={ () => {
							...
							localStorage.setItem( 'lmn-disable-global-block-layouts-hover-notice', true )
						} }
					/>
				</span>
				}
				...
				<Button
					className="lmn-reset-all-block-layout-settings"
					variant="secondary"
					isSmall
				...
				<SectionSettings
					title={ __( 'Image', i18n ) }
					description={ __( 'These styles are applied to all images in Lumen Blocks.', i18n ) }
				>
					<FourRangeControl
						label={ __( 'Border Radius', i18n ) }
						min={ 0 }
...
export const GlobalSpacingAndBordersStyles = () => {
	const styles = useBlockLayoutEditorLoader( 'lumen/global-spacing-and-borders', 'spacing-and-borders' )

	return styles
}

addFilter( 'lumen.resizable-bottom-margin.default', 'lumen/block-layouts', defaultMargin => {
