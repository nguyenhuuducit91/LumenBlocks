...
register( createReduxStore( 'lumen/global-spacing-and-borders', {
...
domReady( () => {
	fetchSettings().then( response => {
		const {
			lumen_global_spacing_and_borders: _blockLayouts,
		} = response
		const blockLayouts = _blockLayouts || []

		dispatch( 'lumen/global-spacing-and-borders' ).updateBlockLayouts( blockLayouts )
	} )
} )
