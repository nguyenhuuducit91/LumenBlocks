...
if ( ! class_exists( 'Lumen_Global_Color_Schemes' ) ) {

	/**
	 * Lumen Global Color Schemes
	 */
    class Lumen_Global_Color_Schemes {
		 ...
		 */
  		...
  		function __construct() {
			...
			if ( is_frontend() ) {
				...
				add_filter( 'lumen_inline_styles_nodep', array( $this, 'add_global_color_schemes_styles' ) );
			}
		}
		...
		public function register_color_schemes() {
			$string_properties = Lumen_Global_Settings::get_string_properties();
						...
						'schema' => array(
							'items' => array(
								...
								'properties'=> array(
									...
									'colorScheme' => array(
										'type' => 'object',
										'properties' => Lumen_Global_Color_Schemes::get_color_scheme_properties( $string_properties )
									),
		...
		public function extend_color_scheme( $old_version, $new_version ) {
			// Seeds the alternate colour scheme on first install only.
			if ( empty( $old_version ) ) {
				...
				$add_alternate_scheme = ! $color_schemes || ( is_array( $color_schemes ) && ! isset( $color_schemes['scheme-default-3'] ) && array_reduce( $color_schemes, function( $carry, $scheme ) {
					return $carry && Lumen_Global_Color_Schemes::is_scheme_empty( $scheme ?? [] );
				}, true ) );

				if ( $add_alternate_scheme ) {
					$updated_schemes = get_option( 'lumen_global_color_schemes', array() );
					...
					$updated_schemes[] = array(
						...
						'colorScheme' => array(
							...
							'linkColor' => array( 'desktop' => '#f59e0b' ),
							'accentColor' => array( 'desktop' => '#f59e0b' ),
							'buttonBackgroundColor' => array( 'desktop' => '#f59e0b' ),
					...
					update_option( 'lumen_global_color_schemes', $updated_schemes );
					delete_option( 'lumen_global_color_scheme_generated_css' );
				}
		...
		public static function get_color_schemes_array() {
			$schemes_array = is_array( get_option( 'lumen_global_color_schemes' ) ) ? get_option( 'lumen_global_color_schemes' ) : [];

			// Get all color schemes, including custom color schemes if any
			$all_color_schemes = apply_filters( 'lumen_global_color_schemes/get_color_schemes', $schemes_array );
		...
		public function add_global_color_schemes_styles( $current_css ) {
			$cached_color_scheme_css = get_option( 'lumen_global_color_scheme_generated_css' );
			...
			$base_default = isset( $this->color_schemes[ get_option( 'lumen_global_base_color_scheme' ) ] ) ? get_option( 'lumen_global_base_color_scheme' ) : 'scheme-default-1';
			$background_default = isset( $this->color_schemes[ get_option( 'lumen_global_background_mode_color_scheme' ) ] )  ? get_option( 'lumen_global_background_mode_color_scheme' ) : 'scheme-default-2';
			$container_default = isset( $this->color_schemes[ get_option( 'lumen_global_container_mode_color_scheme' ) ] )  ? get_option( 'lumen_global_container_mode_color_scheme' ) : 'scheme-default-1';
			...
			$default_color_schemes = array(
				...
				array(
					...
					'mode' => 'background',
					'selectors' => array(
						'desktop' => '.lmn-block-background',
						'desktopParentHover' => ':where(.lmn-hover-parent:hover) .lmn-block-background'
					)
				),
				array(
					...
					'mode' => 'container',
					'selectors' => array(
						'desktop' => '.lmn-container:where(:not(.lmn--no-background))',
						'desktopParentHover' => array( '.lmn-container:where(:not(.lmn--no-background):hover)', ':where(.lmn-hover-parent:hover) .lmn-container:where(:not(.lmn--no-background))')
					)
				)
			);
			...
			foreach ($this->color_schemes as $key => $_ ) {
				$block_color_schemes['background'][] = array(
					...
					'mode' => 'background',
					'selectors' => array(
						'desktop' => ".lmn--background-scheme--$key",
						'desktopParentHover' => ":where(.lmn-hover-parent:hover) .lmn--background-scheme--$key"
					)
				);
				$block_color_schemes['container'][] = array(
					...
					'mode' => 'container',
					'selectors' => array(
						'desktop' => ".lmn--container-scheme--$key",
						'desktopParentHover' => array(".lmn--container-scheme--$key:where(:hover)",":where(.lmn-hover-parent:hover) .lmn--container-scheme--$key")
					)
				);
			}
			...
			update_option( 'lumen_global_color_scheme_generated_css', $color_scheme_css );
		...
		public function add_body_class_color_schemes( $classes, $color_scheme_css ) {
			if ( $color_scheme_css ) {
				if ( strpos( $color_scheme_css, ':root' ) !== false ) {
					$classes[] = 'lmn--has-base-scheme';
				}

				if ( strpos( $color_scheme_css, '.lmn-block-background' ) !== false ) {
					$classes[] = 'lmn--has-background-scheme';
				}

				if ( strpos( $color_scheme_css, '.lmn-container:where(:not(.lmn--no-background))' ) !== false ) {
					$classes[] = 'lmn--has-container-scheme';
				}

				if ( strpos( $color_scheme_css, '--lmn-default-container-background-color' ) !== false ) {
					$classes[] = 'lmn--has-default-container-scheme';
				}
			}
			return $classes;
		}
		 ...
		 * E.g., headingColor ==> --lmn-heading-color
		 *
		...
		public function get_css_custom_properties( $mode = '' ) {
			...
			$keys = Lumen_Global_Color_Schemes::get_color_scheme_properties();
			$states = array(
				'desktop' => '',
				'desktopHover' => '-hover',
				'desktopParentHover' => ''
				...
				foreach( $keys as $key ) {
					if ( $key === 'backgroundColor' ) {
						if ( $mode ) {
							...
							$properties[ $key ] = "--lmn-$prefix-background-color" . $state;
						}
						continue;
					}
		...
		public function get_default_container_colors( $styles, $scheme ) {
			...
			$default_styles[] = array(
				...
				'declarations' => array(
					'--lmn-background-color' => 'var(--lmn-default-container-background-color, #fff)',
					'--lmn-heading-color' => 'var(--lmn-default-heading-color, initial)',
					'--lmn-text-color' => 'var(--lmn-container-color, initial)',
					'--lmn-link-color' => 'var(--lmn-default-link-color, var(--lmn-text-color, initial))',
					'--lmn-accent-color' => '#ddd',
					'--lmn-subtitle-color' => 'var(--lmn-default-subtitle-color, #39414d)',
					'--lmn-default-icon-color' => 'var(--lmn-icon-color)',
					'--lmn-button-background-color' => 'var(--lmn-default-button-background-color, #008de4)',
					'--lmn-button-text-color' => 'var(--lmn-default-button-text-color, #fff)',
					'--lmn-button-outline-color' => 'var(--lmn-default-button-background-color, #008de4)'
				)
			);

			$default_styles = apply_filters( 'lumen.global-settings.global-color-schemes.default-container-scheme', $default_styles );
		...
		public function generate_css_rules( $scheme, $mode = '' ) {
				...
				foreach ( $properties as $property => $css_property ) {
					if ( $this->has_value( $scheme, $property, $state ) ) {
						...
						if ( $property === 'accentColor' ) {
							...
							$decls[ $state ][ "--lmn-subtitle-color$suffix" ] = $scheme[ $property ][ $state ];
						}
					}
					...
					if ( $property == 'buttonBackgroundColor' && $this->is_gradient( $scheme, $property, $state ) ) {
						$suffix = $state === 'desktopHover' ? '-hover' : '';
						$button_plain_decls[ $state ][ "--lmn-button-plain-text-color$suffix" ] = "var(--lmn-button-outline-color$suffix)";
					}
				}
			}
			...
			/* if ( $this->is_gradient( $scheme, 'buttonBackgroundColor', 'desktop' )
			 	...
			 	$button_plain_decls[ 'desktopHover' ][ '--lmn-button-plain-text-color-hover' ] = 'var(--lmn-button-outline-color-hover)';
			}

			if ( $this->is_gradient( $scheme, 'buttonBackgroundColor', 'desktopParentHover' )
				&& ! $this->has_value( $scheme, 'buttonBackgroundColor', 'desktopHover' )
			) {
			 	$button_plain_decls[ 'desktopParentHover' ][ '--lmn-button-plain-text-color-hover' ] = 'var(--lmn-button-outline-color-hover)';
			} */

			// if the button background color is gradient on normal state and solid on parent-hover state,
			// we need to unset the --lmn-button-plain-text-color,
			...
			if ( $this->is_gradient( $scheme, 'buttonBackgroundColor', 'desktop' )
				...
				$button_plain_decls[ 'desktopParentHover' ][ '--lmn-button-plain-text-color' ] = 'unset';
				$button_plain_decls[ 'desktopParentHover' ][ '--lmn-button-plain-text-color-hover' ] = 'unset';
			}
		...
		public function add_theme_compatibility( $styles, $scheme, $selectors, $mode ) {
			...
			$styles = apply_filters( 'lumen.global-settings.global-color-schemes.add-theme-compatibility', $styles, $scheme, $selectors, $mode, $classes );

			return $styles;
		}
	}

	new Lumen_Global_Color_Schemes();
}
