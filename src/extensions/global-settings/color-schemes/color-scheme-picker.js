...
const COLOR_SETTINGS = [ {
	...
	className: 'lmn-color-scheme__heading-color',
}, {
...
const ColorSchemePicker = props => {
	...
	const {
		...
		const { colorSchemes: _colorSchemes } = select( 'lumen/global-color-schemes' ).getSettings()
		return {
			colorSchemes: cloneDeep( _colorSchemes ),
		}
	} )

	const presets = applyFilters( 'lumen.global-settings.global-color-schemes.presets', PRESETS )
	...
	const customColorSchemes = applyFilters( 'lumen.global-settings.global-color-schemes.custom-color-schemes', [] )
	...
	const saveColorSchemeSettings = updatedColorSchemes => {
		...
		saveTimeout = setTimeout( () => {
			const settings = new models.Settings( {
				...
				lumen_global_color_schemes: hasNoColorSchemes( updatedColorSchemes ) ? [] : updatedColorSchemes, // eslint-disable-line camelcase
		...
		// Update our store.
		dispatch( 'lumen/global-color-schemes' ).updateColorSchemes( updatedColorSchemes )
	}
	...
	const onSortEnd = props => {
		doAction( 'lumen.global-settings.global-color-schemes.custom-color-schemes.sort-color-schemes', props, saveTimeout )
	}
	...
	const updateColorSchemes = currentItem => {
		...
		const customUpdate = applyFilters( 'lumen.global-settings.global-color-schemes.update-color-schemes', false, currentItem, saveTimeout )
	...
	const onChange = ( property, color ) => {
		...
		if ( currentHoverState !== 'normal' ) {
			const disableHoverNotice = localStorage.getItem( 'lmn-disable-global-block-color-schemes-hover-notice' )
	...
	const ItemPreview = ( { item, withWrapper = false } ) => {
		 ...
		 * so we use --lmn-block-background-color as the default background-color for scheme-default-2
		 */
		const defaults = {
			...DEFAULT_COLOR_SCHEME_COLORS,
			...( item.key === 'scheme-default-2'
				? { backgroundColor: { desktop: 'var(--lmn-block-background-color)' } }
				: {}
			),
		}
		...
		return withWrapper ? <div
			className="lmn-global-color-scheme__preview-wrapper"
