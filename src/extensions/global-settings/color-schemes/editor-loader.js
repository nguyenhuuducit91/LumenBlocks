 ...
 */
import { useBlockColorSchemes, useBlockHoverState } from '~lumen/hooks'
...
export const renderGlobalColorSchemeStyles = (
	...
	if ( backgroundModeColorScheme in colorSchemes && schemeHasValue( colorSchemes[ backgroundModeColorScheme ] ) ) {
		...
		let bgcss = ''
		if ( decls.desktop.length || decls.desktopHover.length ) {
			bgcss += `.lmn-block-background{ ${ [ ...decls.desktop, ...decls.desktopHover, unsetDefaults ].join( '' ) } }\n`
		}
		if ( decls.desktopParentHover.length ) {
			bgcss += `:where(.lmn-hover-parent:hover) .lmn-block-background{ ${ [ ...decls.desktopParentHover, unsetDefaults ].join( '' ) } }\n`
		}
		css += bgcss
	}
	...
	if ( containerModeColorScheme in colorSchemes && schemeHasValue( colorSchemes[ containerModeColorScheme ] ) ) {
		...
		let containercss = ''
		if ( decls.desktop.length || decls.desktopHover.length ) {
			containercss += `.lmn-container:where(:not(.lmn--no-background)){ ${ [ ...decls.desktop, ...decls.desktopHover ].join( '' ) } }\n`
		}
		if ( decls.desktopParentHover.length ) {
			containercss += `.lmn-container:where(:not(.lmn--no-background):hover), :where(.lmn-hover-parent:hover) .lmn-container:where(:not(.lmn--no-background)){ ${ decls.desktopParentHover.join( '' ) } }\n`
		}
		css += containercss
	} else if ( addContainerSchemeDefaultColors	) {
		const containercss = `.lmn-container:where(:not(.lmn--no-background)){ ${ getDefaultColors() } }\n`

		css += applyFilters( 'lumen.global-settings.global-color-schemes.default-container-scheme', containercss )
	}
	...
	Object.entries( colorSchemes ).forEach( ( [ key, scheme ] ) => {
		...
		if ( decls.desktop.length || decls.desktopHover.length ) {
			rules.background.push( `.lmn--background-scheme--${ key }{ ${ [ ...decls.desktop, ...decls.desktopHover, unsetDefaults ].join( '' ) } }` )
		}
		if ( decls.desktopParentHover.length ) {
			rules.background.push( `:where(.lmn-hover-parent:hover) .lmn--background-scheme--${ key }{ ${ [ ...decls.desktopParentHover, unsetDefaults ].join( '' ) } }` )
		}

		decls = getCSS( scheme, currentHoverState, 'container' )
		if ( decls.desktop.length || decls.desktopHover.length ) {
			rules.container.push( `.lmn--container-scheme--${ key }{ ${ [ ...decls.desktop, ...decls.desktopHover ].join( '' ) } }` )
		}
		if ( decls.desktopParentHover.length ) {
			rules.container.push( `.lmn--container-scheme--${ key }:where(:hover), :where(.lmn-hover-parent:hover) .lmn--container-scheme--${ key }{ ${ decls.desktopParentHover.join( '' ) } }` )
		}
	} )
	...
	useEffect( () => {
		if ( editorEl ) {
			const addClassNames = editor => {
				...
				if ( styles === '' ) {
					editor.classList.remove( 'lmn--has-base-scheme', 'lmn--has-background-scheme', 'lmn--has-container-scheme' )
				} else {
					if ( ! styles.includes( ':root' ) ) {
						classNamesToRemove.push( 'lmn--has-base-scheme' )
					} else if ( editor.classList.contains( 'lmn--has-base-scheme' ) === false ) {
						classNamesToAdd.push( 'lmn--has-base-scheme' )
					}

					if ( ! styles.includes( '.lmn-block-background' ) ) {
						classNamesToRemove.push( 'lmn--has-background-scheme' )
					} else if ( editor.classList.contains( 'lmn--has-background-scheme' ) === false ) {
						classNamesToAdd.push( 'lmn--has-background-scheme' )
					}

					if ( ! styles.includes( '.lmn-container:where(:not(.lmn--no-background))' ) ) {
						classNamesToRemove.push( 'lmn--has-container-scheme' )
					} else if ( editor.classList.contains( 'lmn--has-container-scheme' ) === false ) {
						classNamesToAdd.push( 'lmn--has-container-scheme' )
					}

					if ( ! styles.includes( '--lmn-default-container-background-color' ) ) {
						classNamesToRemove.push( 'lmn--has-default-container-scheme' )
					} else if ( editor.classList.contains( 'lmn--has-container-scheme' ) === false ) {
						classNamesToAdd.push( 'lmn--has-default-container-scheme' )
					}
			...
			if ( ! filterRegistered ) {
				addFilter( 'lumen.global-styles.classnames', `lumen/global-settings.color-schemes`, classnames => {
				// Access current values via closure or alternative state management
					const editor = editorEl
					const hasBase = editor.classList.contains( 'lmn--has-base-scheme' )
					const hasBackground = editor.classList.contains( 'lmn--has-background-scheme' )
					const hasContainer = editor.classList.contains( 'lmn--has-container-scheme' )
					const hasDefaultContainer = editor.classList.contains( 'lmn--has-default-container-scheme' )

					if ( hasBase && ! classnames.includes( 'lmn--has-base-scheme' ) ) {
						classnames.push( 'lmn--has-base-scheme' )
					}
					if ( hasBackground && ! classnames.includes( 'lmn--has-background-scheme' ) ) {
						classnames.push( 'lmn--has-background-scheme' )
					}
					if ( hasContainer && ! classnames.includes( 'lmn--has-container-scheme' ) ) {
						classnames.push( 'lmn--has-container-scheme' )
					}
					if ( hasDefaultContainer && ! classnames.includes( 'lmn--has-default-container-scheme' ) ) {
						classnames.push( 'lmn--has-default-container-scheme' )
					}
					return classnames
				} )
