...
const camelToKebab = property => {
	...
	// Convert the result to lowercase and return with '--lmn-' prefix
	return '--lmn-' + result
}
...
export const getCSS = ( scheme, currentHoverState = 'normal', mode = '' ) => {
	...
	states.forEach( state => {
		...
		properties.forEach( property => {
			...
			const customProperty = property === 'backgroundColor'
				? `--lmn-${ varname }-background-color` : camelToKebab( property )
			...
			if ( property === 'buttonBackgroundColor' && isGradient( scheme[ property ]?.[ state ] ) ) {
				decls[ state ].push( `:where(.is-style-plain){ --lmn-button-plain-text-color${ suffix }: var(--lmn-button-outline-color${ suffix }); }` )
			}
		} )
	} )
	...
	/* if ( isGradient( scheme.buttonBackgroundColor?.desktop ) && ! scheme.buttonBackgroundColor?.desktopHover ) {
	 	decls.desktopHover.push( `:where(.is-style-plain){ --lmn-button-plain-text-color-hover: var(--lmn-button-outline-color-hover); }` )
	 }

	if ( isGradient( scheme.buttonBackgroundColor?.desktopParentHover ) && ! scheme.buttonBackgroundColor?.desktopHover ) {
	 	decls.desktopParentHover.push( `:where(.is-style-plain){ --lmn-button-plain-text-color-hover: var(--lmn-button-outline-color-hover); }` )
	} */
	...
	if ( isGradient( scheme.buttonBackgroundColor?.desktop ) &&
		...
		decls.desktopParentHover.push( `:where(.is-style-plain){ --lmn-button-plain-text-color: unset;--lmn-button-plain-text-color-hover:unset; }` )
	}
...
export const getDefaultColors = () => {
	...
	decls += '--lmn-background-color: var(--lmn-default-container-background-color, #fff);'
	decls += '--lmn-heading-color: var(--lmn-default-heading-color, initial);'
	decls += '--lmn-text-color: var(--lmn-container-color, initial);'
	decls += '--lmn-link-color: var(--lmn-default-link-color, var(--lmn-text-color, initial));'
	decls += '--lmn-accent-color: #ddd;'
	decls += '--lmn-subtitle-color: var(--lmn-default-subtitle-color, #39414d);'
	decls += '--lmn-default-icon-color: var(--lmn-icon-color);'
	decls += '--lmn-button-background-color: var(--lmn-default-button-background-color, #008de4);'
	decls += '--lmn-button-text-color: var(--lmn-default-button-text-color, #fff);'
	decls += '--lmn-button-outline-color: var(--lmn-default-button-background-color, #008de4);'
...
export const unsetDefaultColors = () => {
	...
	decls += '--lmn-default-icon-color: unset;'
...
const addThemeCompatibility = ( decls, scheme, mode ) => {
	...
	decls = applyFilters( 'lumen.global-settings.global-color-schemes.add-theme-compatibility', decls, scheme, mode, theme )
