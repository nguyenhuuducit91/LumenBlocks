...
if ( ! class_exists( 'Lumen_Global_Buttons_And_Icons' ) ) {

	/**
	 * Lumen Global Block Spacing and Borders
	 */
    class Lumen_Global_Buttons_And_Icons {
		 ...
		 */
  		function __construct() {
			...
			add_action( 'lumen_register_global_settings', array( $this, 'register_buttons_and_icons' ) );
			...
			// add_action( 'lumen_early_version_upgraded',  array( $this, 'migrate_buttons_and_icons_schema_changes' ), 10, 2 );
			...
			if ( lumen_is_frontend() ) {
				...
				add_filter( 'lumen_inline_styles_nodep', array( $this, 'add_global_buttons_and_icons_styles' ) );
			}
		}
		...
		public function register_buttons_and_icons() {
			$four_range_properties = Lumen_Global_Settings::get_four_range_properties();
			$string_four_range_properties = Lumen_Global_Settings::get_string_four_range_properties();
			$string_properties = Lumen_Global_Settings::get_string_properties();
			$number_properties = Lumen_Global_Settings::get_number_properties();

			register_setting(
				'lumen_global_settings',
				'lumen_global_buttons_and_icons',
				array(
					'type' => 'object',
					'description' => __( 'Lumen global buttons and icons', 'lumen-blocks' ),
		...
		public function add_global_buttons_and_icons_styles( $current_css ) {
			$generated_css = Lumen_Global_Settings::generate_global_block_layouts( 'lumen_global_buttons_and_icons', 'Global Buttons and Icons' );
		...
		public function add_body_class_buttons_and_icons( $classes ) {
			$classes[] = 'lmn-has-design-system-buttons-and-icons';
			return $classes;
		}
		...
		public function migrate_buttons_and_icons_schema_changes( $old_version, $new_version ) {
			...
			$option_name = 'lumen_global_buttons_and_icons';
	...
	new Lumen_Global_Buttons_And_Icons();
}
