...
const renderGlobalStyles = ( newColors, isEditingTemplate, setStyles ) => {
	...
	// Output all our --lmn-global-colors.
	...
	setStyles( applyFilters( 'lumen.editor-render-global-styles.css', css, newColors, isEditingTemplate ) )
}

export const GlobalColorStyles = () => {
	const { colors, isEditingTemplate } = useSelect( select => ( {
		colors: select( 'lumen/global-colors' ).getSettings().lumenColors || [],
