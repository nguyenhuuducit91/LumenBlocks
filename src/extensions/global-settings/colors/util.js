...
export const replaceGlobalColorAttributes = ( attributes = {}, colorsBeforeReset = [], options = {} ) => {
	...
	if ( includeColorVar ) {
		colorsBeforeReset.forEach( colorBeforeReset => {
			if ( colorBeforeReset.slug.match( /^lmn-global-color/ ) ) {
	...
	if ( includeSlugNames ) {
		const lumenSlugNames = colorsBeforeReset.filter( color => color.slug && color.slug.match( /^lmn-global-color/ ) ).map( color => color.slug )
		lumenSlugNames.forEach( lumenSlugName => {
			const foundColorBeforeReset = find( colorsBeforeReset, color => color.slug === lumenSlugName )
			updatedStringifiedAttributes = updatedStringifiedAttributes.replace( new RegExp( `"${ lumenSlugName }"`, 'g' ), foundColorBeforeReset ? `"${ foundColorBeforeReset.color }"` || '"#000000"' : '"#000000"' )
		} )
	}
...
export const resetBlockColorAttributes = ( block, colorObjects ) => {
	...
	if ( name.startsWith( 'lmb/' ) || name.startsWith( 'lumen/' ) ) {
		//
		/**
		 * For lumen blocks.
		...
		if ( ! isEqual( updatedAttributes, block.attributes ) ) {
			updateBlockAttributes( clientId, updatedAttributes ) // eslint-disable-line lumen/no-update-block-attributes
		}
		...
		if ( name.includes( 'heading' ) || name.includes( 'paragraph' ) ) {
			...
			if ( backgroundColor ) {
				if ( backgroundColor.includes( 'lmn-global-color-' ) ) {
					// Retain the color
					const colorVarMatch = backgroundColor.match( /lmn-global-color-(\S*)/ )
					if ( colorVarMatch && Array.isArray( colorVarMatch ) && colorVarMatch.length >= 2 ) {
						...
						const appliedColor = find( colorObjects, color => color.slug === `lmn-global-color-${ colorVarID }` )
			...
			if ( textColor ) {
				if ( textColor.includes( 'lmn-global-color-' ) ) {
					// Retain the color
					const colorVarMatch = textColor.match( /lmn-global-color-(\S*)/ )
					if ( colorVarMatch && Array.isArray( colorVarMatch ) && colorVarMatch.length >= 2 ) {
						const colorVarID = colorVarMatch[ 1 ]
			...
			// Update the block attributes.
			updateBlockAttributes( clientId, newAttributes ) // eslint-disable-line lumen/no-update-block-attributes
		}
	}
}
...
export const updateFallbackColorAttributes = ( attributes = {}, colors = [] ) => {
	...
	const colorVars = colors.filter( color => color.slug.match( /^lmn-global-color/ ) ).map( color => `--${ color.slug }` )
