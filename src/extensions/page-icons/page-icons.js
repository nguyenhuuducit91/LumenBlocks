...
export const PageIcons = () => {
	const pageIcons = useSelect( select => select( 'lumen/page-icons' ).getPageIcons(), [] ) || new Map()
	return (
		<defs>
			{ Array.from( pageIcons ).map( ( [ icon, iconData ] ) => {
				...
				return (
					<symbol key={ iconId } id={ `lmn-page-icons__${ iconId }` } { ...svgAttributes } dangerouslySetInnerHTML={ { __html: innerHTML } } />
				)
			} ) }
		</defs>
	)
