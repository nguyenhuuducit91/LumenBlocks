...
if ( ! class_exists( 'Lumen_Block_Style_Inheritance' ) ) {

	/**
	 * Lumen Block Style Inheritance
	 */
    class Lumen_Block_Style_Inheritance {
		 ...
		 */
  		function __construct() {
			...
			// add_action( 'lumen_early_version_upgraded', array( $this, 'block_style_inheritance_set_default' ), 10, 2 );

			add_action( 'body_class', array( $this, 'add_body_class_block_style_inheritance' ) );
			add_filter( 'lumen_block_style_inheritance_inline_styles_nodep', array( $this, 'add_block_style_inheritance' ) );
		 ...
		 * so that for old users, existing lumen blocks won't inherit the styles from theme.json
		...
		function block_style_inheritance_set_default( $old_version, $new_version ) {
			if ( ! empty( $old_version ) && version_compare( $old_version, "3.16.0", "<" ) ) {
				if ( ! get_option( 'lumen_disable_block_style_inheritance' ) ) {
					update_option( 'lumen_disable_block_style_inheritance', 'true' );
				}
			}
		}

		/**
		 * Adds block style inheritance from theme.json styles to Lumen blocks.
		 * This allows Lumen blocks to inherit styling defined in the theme's theme.json file.
		 *
		...
		function add_block_style_inheritance( $current_css ) {
			$disable_block_style_inheritance = get_option( 'lumen_disable_block_style_inheritance' );
			...
			if ( $hash === get_transient( 'lumen_block_style_inheritance_hash' ) ) {
				$generated_css = get_transient( 'lumen_block_style_inheritance_css' );
				if ( $generated_css ) {
			...
			$style_declarations = array(
				...
				'button' => array(
					'selector' => ':where(.lmn-block-button, .lmn-block-icon-button, .lmn-block-pagination):not(.is-style-link) .lmn-button',
					'declarations' => array(),
				),
				'button-hover' => array(
					'selector' => ':where(.lmn-block-button, .lmn-block-icon-button, .lmn-block-pagination):not(.is-style-link) .lmn-button:hover',
					'declarations' => array(),
				),
				'button-default' => array(
					'selector' => ':where(.lmn-block-button, .lmn-block-pagination):not(.is-style-link, .is-style-ghost, .is-style-plain) > .lmn-button',
					'declarations' => array(),
				),
				'button-default-hover' => array(
					'selector' => ':where(.lmn-block-button, .lmn-block-pagination):not(.is-style-link, .is-style-ghost, .is-style-plain) > .lmn-button:hover',
					'declarations' => array(),
				),
				'button-ghost' => array(
					'selector' => ':where(.lmn-block-button, .lmn-block-pagination).is-style-ghost > .lmn-button',
					'declarations' => array(),
				),
				'button-ghost-hover' => array(
					'selector' => ':where(.lmn-block-button, .lmn-block-pagination).is-style-ghost > .lmn-button:hover',
					'declarations' => array(),
				),
				'columns' => array(
					'selector' => '.lmn-block-columns',
					'declarations' => array(),
				),
				'column' => array(
					'selector' => '.lmn-block-column',
					'declarations' => array(),
				),
				'heading' => array(
					'selector' => ':where(.lmn-block-heading) > :is(h1,h2,h3,h4,h5,h6)',
					'declarations' => array(),
				),
				'h1' => array(
					'selector' => ':where(.lmn-block-heading) > h1',
					'declarations' => array(),
				),
				'h2' => array(
					'selector' => ':where(.lmn-block-heading) > h2',
					'declarations' => array(),
				),
				'h3' => array(
					'selector' => ':where(.lmn-block-heading) > h3',
					'declarations' => array(),
				),
				'h4' => array(
					'selector' => ':where(.lmn-block-heading) > h4',
					'declarations' => array(),
				),
				'h5' => array(
					'selector' => ':where(.lmn-block-heading) > h5',
					'declarations' => array(),
				),
				'h6' => array(
					'selector' => ':where(.lmn-block-heading) > h6',
					'declarations' => array(),
				),
				'link' => array(
					'selector' => ':where(.lmn-block-heading__text, .lmn-block-text__text, .lmn-block-table-of-contents__link-wrapper, .lmn-block-subtitle__text, .is-style-link) a',
					'declarations' => array(),
				),
			);
			...
			if ( $root_properties[ 'button-group-gap' ] || $root_properties[ 'default-gap' ] ) {
				$style_declarations['root']['declarations'][ '--lmn-button-column-gap' ] = $root_properties[ 'button-group-gap' ] ?? $root_properties[ 'default-gap' ];
				$style_declarations['root']['declarations'][ '--lmn-button-row-gap' ] = $root_properties[ 'button-group-gap' ] ?? $root_properties[ 'default-gap' ];
			}

			// DEV NOTE: Remove column gap from inheritance since it breaks our design library.
			// if ( $root_properties[ 'columns-gap' ] || $root_properties[ 'default-gap' ] ) {
			// 	$style_declarations['root']['declarations'][ '--lmn-columns-column-gap' ] = $root_properties[ 'columns-gap' ] ?? $root_properties[ 'default-gap' ];
			// }

			if ( $root_properties[ 'text-color' ] ) {
				$style_declarations['root']['declarations'][ '--lmn-container-color' ] = $root_properties[ 'text-color' ];
				$style_declarations['root']['declarations'][ '--lmn-text-color' ] = $root_properties[ 'text-color' ];
				$style_declarations['root']['declarations'][ '--lmn-default-text-color' ] = $root_properties[ 'text-color' ];
				$style_declarations['root']['declarations'][ '--lmn-subtitle-color' ] = $root_properties[ 'text-color' ];
				$style_declarations['root']['declarations'][ '--lmn-default-subtitle-color' ] = $root_properties[ 'text-color' ];
			}

			if ( $root_properties[ 'heading-color' ] ) {
				$style_declarations['root']['declarations'][ '--lmn-default-heading-color' ] = $root_properties[ 'heading-color' ];
			} else if ( $root_properties[ 'text-color' ] ) {
				$style_declarations['root']['declarations'][ '--lmn-default-heading-color' ] = $root_properties[ 'text-color' ];
			}

			if ( $root_properties[ 'link-color' ] ) {
				$style_declarations['root']['declarations'][ '--lmn-default-link-color' ] = $root_properties[ 'link-color' ];
			} else if ( $root_properties[ 'text-color' ] ) {
				$style_declarations['root']['declarations'][ '--lmn-default-link-color' ] = $root_properties[ 'text-color' ];
			}

			/**
			 * Array that defines how which theme.json styles will be inherited by Lumen blocks.
			 *
			...
			$element_settings = array(
				...
				'button-default' => array(
					...
					'set_custom_properties' => array(
						'color' => array('--lmn-button-text-color', '--lmn-default-button-text-color'),
						'background' => array('--lmn-button-background-color', '--lmn-default-button-background-color'),
					),
					'custom_properties_selector' => 'root',
				...
				'button-ghost' => array(
					...
					'set_custom_properties' => array(
						'padding' => '--lmn-button-padding',
						'border-width' => '--lmn-button-ghost-border-width' ),
			...
			set_transient( 'lumen_block_style_inheritance_hash', $hash, 60 * 60 * 24 );
			set_transient( 'lumen_block_style_inheritance_css', $generated_css, 60 * 60 * 24 );
		...
		/**
		 * Add a class in the body to indicate if it lumen blocks will inherit styles from theme.json
		 */
		function add_body_class_block_style_inheritance( $classes ) {
			$disable_block_style_inheritance = get_option( 'lumen_disable_block_style_inheritance' );
			if ( ! $disable_block_style_inheritance ) {
				$classes[] = 'lmn-has-block-style-inheritance';
			}
			return $classes;
		}
		...
		public function add_setting( $settings ) {
            $settings['lumen_disable_block_style_inheritance'] = get_option( 'lumen_disable_block_style_inheritance', false );
            return $settings;
        }
	}

	new Lumen_Block_Style_Inheritance();
}
