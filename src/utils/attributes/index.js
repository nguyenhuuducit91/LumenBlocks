...
const REMOVE_ATTRS = [ 'lmnResponsive', 'lmnHover', 'lmnUnits' ]
...
export const expandAttributes = attributes => {
	return Object.keys( attributes ).reduce( ( newAttributes, name ) => {
		...
		let defaultUnit = 'px'
		if ( newAttributes[ name ].lmnUnits ) {
			suffixes.push( [ '', 'Unit' ] )
			defaultUnit = newAttributes[ name ].lmnUnits
		}
		if ( newAttributes[ name ].lmnResponsive ) {
			suffixes.push( [ '', 'Tablet', 'Mobile' ] )
		}
		if ( newAttributes[ name ].lmnHover ) {
...
// Common content attributes across all Lumen blocks that should be preserved.
