...
export const faGetIcon = ( prefix, iconName ) => {
	if ( ! window.LmnFontAwesome || ! window.LmnFontAwesome[ `${ prefix }-${ iconName }` ] ) {
		return ''
	}

	return window.LmnFontAwesome[ `${ prefix }-${ iconName }` ]
}
...
export const faFetchIcon = ( prefix, iconName ) => {
	...
	return new Promise( async ( resolve, reject ) => {
		...
		if ( ! window.LmnFontAwesome ) {
			window.LmnFontAwesome = {}
		}

		window.LmnFontAwesome[ `${ prefix }-${ iconName }` ] = svgText
		resolve( true )
	} )
}
