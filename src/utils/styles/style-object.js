 ...
 */
import { useBlockAttributesContext, useBlockHoverState } from '~lumen/hooks'
import { compact, pick } from 'lodash'
import { QueryLoopContext } from '~lumen/hoc'
...
class StyleObject {
	...
	appendToSelector( selector, rule, value, _device = 'desktop', renderIn = '', vendorPrefixes = [] ) {
		// Add instance id to classes. ( e.g. `lmn-abc123` -> `lmn-abc123-2`, where 2 is `this.queryLoopInstance`. )
	...
	add( attributes, styleParams = {}, blockState = 'normal' ) {
		...
		const collapsedSelector = prependClass( selector, ':where(.lmn-block-accordion.lmn--is-open) .%s, .%s.lmn--is-open', ':where(.lmn-block-accordion.lmn--is-open) .%s, .%s.lmn--is-open', 'collapsed' )
		const parentHoverSelector = prependClass( selector, ':where(.lmn-hover-parent:hover, .lmn-hover-parent.lmn--is-hovered) .%s', '.%s.lmn--is-hovered', 'parent-hover' )
		...
		if ( ! selectorHasDataBlock ) {
			...
			hoverSelector = hoverSelector
				...
				? prependClass( blockState === 'hover' ? selector : hoverSelector || selector, null, '.%s.lmn--is-hovered', 'hover' )
				: prependClass( selector, '.%s:hover', '.%s.lmn--is-hovered', 'hover' )
		} else {
			// If there is a [data-block] append the :hover or .lmn-is-hovered directly to it.
			hoverSelector = hoverSelector
				// In editor, always use the `selector` instead of the hoverSelector.
				? appendClass( blockState === 'hover' ? selector : hoverSelector || selector, null, '.lmn--is-hovered', 'hover' )
				: appendClass( selector, ':hover', '.lmn--is-hovered', 'hover' )
		}
