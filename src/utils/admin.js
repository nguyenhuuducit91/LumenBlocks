...
export const importBlocks = r => {
	...
	r.keys().forEach( key => {
		...
		( meta[ 'lmn-variants' ] || [] ).forEach( variation => {
			const type = variation[ 'lmn-type' ]
			if ( type ) {
				if ( ! blocks[ type ] ) {
					blocks[ type ] = []
				}
		...
		if ( meta[ 'lmn-block-dependency' ] ) {
			blockDependencies[ meta.name ] = meta[ 'lmn-block-dependency' ]
		}
	} )
