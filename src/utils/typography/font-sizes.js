...
export const getFontSizes = selectors => {
	...
	dummyElement.classList.add( 'lmb-default-font-size' )
 ...
 */
const initFontSizes = () => {
	const SELECTORS = [ 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', '.lmn-subtitle', '.lmn-button__inner-text' ]
	fontSizes = {
 ...
 */
addFilter( 'lumen.global-settings.typography.editor-styles', 'lumen/default-font-sizes', ( tagStyles, tag, selector, typographyStyles ) => {
	// Change the default font size depending on the font size in the global typography.
	tagStyles[ `.lmb-default-font-size .wp-block ${ tag }` ] = createTypographyStyles( '%s', 'desktop', typographyStyles, { important: true } )
