...
const applySettingsToMeta = metadata => {
	...
	if ( metadata[ 'lmn-block-dependency' ] && metadata[ 'lmn-block-dependency' ] in disabledBlocks ) {
		inserter = ! disabledBlocks[ metadata[ 'lmn-block-dependency' ] ] === BLOCK_STATE.HIDDEN
	}
	...
	if ( variationsToRemove.length ) {
		...
		if ( ! variations.length ) {
			if ( metadata[ 'lmn-type' ] === 'hidden' ) {
				inserter = false
			}
		}
	}

	const whitelist = metadata[ 'lmn-substitution-blocks' ]
	if ( whitelist ) {
...
addFilter( 'lumen.block.metadata', 'lumen/disabled-blocks', applySettingsToMeta )
