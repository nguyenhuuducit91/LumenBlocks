...
if ( function_exists( 'jetpack_photon_url' ) ) {
	if ( ! function_exists( 'lumen_jetpack_photon_render_block_replace_style_images' ) ) {
		/**
		 ...
		 */
		function lumen_jetpack_photon_render_block_replace_style_images( $matches ) {
			return jetpack_photon_url( $matches[0] );
		}
	}

	if ( ! function_exists( 'lumen_jetpack_photon_render_block_replace_style' ) ) {
		/**
		 ...
		 * @return string The replacement string.
		 */
		function lumen_jetpack_photon_render_block_replace_style( $matches ) {
			return preg_replace_callback( '/https?:[^;\}<]+.(jpg|gif|png)/i', 'lumen_jetpack_photon_render_block_replace_style_images', $matches[0] );
		}
	}

	if ( ! function_exists( 'lumen_jetpack_photon_render_block' ) ) {
		/**
		 ...
		 */
		function lumen_jetpack_photon_render_block( $block_content, $block ) {
			if ( $block_content === null ) {
				return $block_content;
			}

			// Only for Lumen blocks.
			if ( ! isset( $block['blockName'] ) || strpos( $block['blockName'], 'lmb/' ) === false ) {
				return $block_content;
			}
