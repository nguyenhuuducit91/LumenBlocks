...
import { fetchSettings, importBlocks } from '~lumen/utils/admin'
import { createRoot } from '~lumen/utils/element'
import AdminSelectSetting from '~lumen/ui/admin-select-setting'
import AdminToggleSetting from '~lumen/ui/admin-toggle-setting'
import AdminTextSetting from '~lumen/ui/admin-text-setting'
import AdminToolbarSetting from '~lumen/ui/admin-toolbar-setting'
import { GettingStarted } from './getting-started'
import { BLOCK_STATE } from '~lumen/utils/blocks'
import { BlockToggler, OptimizationSettings } from '~lumen/legacy/v2/welcome/admin'
import blockData from '~lumen/legacy/v2/welcome/blocks'

const [ FREE_BLOCKS, BLOCK_DEPENDENCIES ] = importBlocks( require.context( '../block-library', true, /block\.json$/ ) )

export const getAllBlocks = () => applyFilters( 'lumen.settings.blocks', FREE_BLOCKS )
...
const SEARCH_TREE = [
	{
		...
		groups: [
			...
			},
			{
				id: 'editor',
				children: [
					__( 'Lumen Text as Default Block', i18n ),
					__( 'Design Library', i18n ),
					__( 'Lumen Settings', i18n ),
...
const REQUIRED_BLOCKS = BLOCK_CATEROGIES.reduce( ( acc, { id } ) => {
	DERIVED_BLOCKS[ id ].forEach( block => {
		acc[ block.name ] = block[ 'lmn-required-blocks' ] ?? []
	} )
	return acc
}, {} )
							...
							{ DERIVED_BLOCKS[ id ].map( ( block, i ) => {
								return (
									...
									>
										...
										{ block[ 'lmn-demo' ] && <a href={ block[ 'lmn-demo' ] } target="_example">{ __( 'See example', i18n ) }</a> }
									</div>
								)
							} ) }
						</div>
...
const RestSettingsNotice = () => {
	...
	return (
		<div className="notice notice-error">
			<p>{ __( 'Error getting Lumen settings. We got the following error. Please contact your administrator.', i18n ) }</p>
			{ error.responseJSON &&
...
const Settings = () => {
	...
	const hasV2Compatibility = currentSettings => {
		return currentSettings.lumen_v2_frontend_compatibility === '1' ||
			currentSettings.lumen_v2_editor_compatibility === '1' ||
			currentSettings.lumen_v2_editor_compatibility_usage === '1'
	}
				...
				<>
					{ ( blocks.children === null || blocks.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<p className="s-settings-subtitle">{ __( 'Adjust the width of Lumen blocks here.', i18n ) }</p>
							<AdminTextSetting
								label={ __( 'Nested Block Width', i18n ) }
								searchedSettings={ blocks.children }
								value={ settings.lumen_block_default_width }
								type="text"
								onChange={ value => {
									handleSettingsChange( { lumen_block_default_width: value } ) // eslint-disable-line camelcase
								} }
							...
							<AdminTextSetting
								...
								searchedSettings={ blocks.children }
								value={ settings.lumen_block_wide_width }
								type="text"
								onChange={ value => {
									handleSettingsChange( { lumen_block_wide_width: value } ) // eslint-disable-line camelcase
								} }
					...
					{ ( editor.children === null || editor.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<p className="s-settings-subtitle">{ __( 'You can customize some of the features and behavior of Lumen in the editor here.', i18n ) }	</p>
							<AdminToggleSetting
								label={ __( 'Lumen Text as Default Block', i18n ) }
								searchedSettings={ editor.children }
								value={ settings.lumen_enable_text_default_block }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_text_default_block: value } ) // eslint-disable-line camelcase
								} }
								help={ __( 'If enabled, Lumen Text blocks will be added by default instead of the native Paragraph Block.', i18n ) }
							/>
							<AdminToggleSetting
								label={ __( 'Design Library', i18n ) }
								searchedSettings={ editor.children }
								value={ settings.lumen_enable_design_library }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_design_library: value } ) // eslint-disable-line camelcase
								} }
							...
							/>
							<AdminToggleSetting
								label={ __( 'Lumen Settings', i18n ) }
								searchedSettings={ editor.children }
								value={ settings.lumen_enable_global_settings }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_global_settings: value } ) // eslint-disable-line camelcase
								} }
								help={ __( 'Adds a button on the top of the editor which gives access to Lumen settings. Note: You won\'t be able to access Lumen settings when this is disabled.', i18n ) }
							/>
							<AdminToggleSetting
								label={ __( 'Use Size Presets by Default', i18n ) }
								searchedSettings={ editor.children }
								value={ props.settings.lumen_use_size_presets_by_default }
								onChange={ value => {
									props.handleSettingsChange( { lumen_use_size_presets_by_default: value } ) // eslint-disable-line camelcase
								} }
							...
							<AdminToggleSetting
								...
								searchedSettings={ editor.children }
								value={ settings.lumen_enable_block_linking }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_block_linking: value } ) // eslint-disable-line camelcase
								} }
								help={
									<>
					...
					{ ( toolbar.children === null || toolbar.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<AdminToggleSetting
								...
								value={ settings.lumen_enable_text_highlight }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_text_highlight: value } ) // eslint-disable-line camelcase
								} }
							...
							<AdminToggleSetting
								...
								searchedSettings={ toolbar.children }
								value={ settings.lumen_enable_dynamic_content }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_dynamic_content: value } ) // eslint-disable-line camelcase
								} }
							...
							<AdminToggleSetting
								...
								searchedSettings={ toolbar.children }
								value={ settings.lumen_enable_copy_paste_styles }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_copy_paste_styles: value } ) // eslint-disable-line camelcase
								} }
							...
							<AdminToggleSetting
								...
								searchedSettings={ toolbar.children }
								value={ settings.lumen_enable_reset_layout }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_reset_layout: value } ) // eslint-disable-line camelcase
								} }
								help={ __( 'Adds a toolbar button for resetting the layout of a stackble block back to the original', i18n ) }
							/>
							{ settings.lumen_enable_block_defaults && <AdminToggleSetting
								label={ __( 'Save as Default Block', i18n ) }
								searchedSettings={ toolbar.children }
								value={ settings.lumen_enable_save_as_default_block }
								onChange={ value => {
									handleSettingsChange( { lumen_enable_save_as_default_block: value } ) // eslint-disable-line
								} }
					...
					{ ( inspector.children === null || inspector.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<p className="s-settings-subtitle">{ __( 'You can customize some of the features and behavior of Lumen in the inspector here.', i18n ) }</p>
							<AdminToggleSetting
								label={ __( 'Don\'t show help video tooltips', i18n ) }
								searchedSettings={ inspector.children }
								value={ settings.lumen_help_tooltip_disabled === '1' }
								onChange={ value => {
									handleSettingsChange( { lumen_help_tooltip_disabled: value ? '1' : '' } ) // eslint-disable-line camelcase
								} }
							...
							<AdminToggleSetting
								...
								searchedSettings={ inspector.children }
								value={ settings.lumen_auto_collapse_panels }
								onChange={ value => {
									handleSettingsChange( { lumen_auto_collapse_panels: value } ) // eslint-disable-line camelcase
								} }
					...
					{ ( dynamicBreakpoints.children === null || dynamicBreakpoints.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<AdminTextSetting
								...
								type="number"
								value={ settings.lumen_dynamic_breakpoints?.tablet || '' } // eslint-disable-line camelcase
								onChange={ value => {
									handleSettingsChange( {
										lumen_dynamic_breakpoints: { // eslint-disable-line camelcase
											tablet: value,
											mobile: settings.lumen_dynamic_breakpoints?.mobile || '', // eslint-disable-line camelcase
										},
									} )
								} }
							...
							<AdminTextSetting
								...
								searchedSettings={ dynamicBreakpoints.children }
								type="number"
								value={ settings.lumen_dynamic_breakpoints?.mobile || '' } // eslint-disable-line camelcase
								onChange={ value => {
									handleSettingsChange( {
										lumen_dynamic_breakpoints: { // eslint-disable-line camelcase
											tablet: settings.lumen_dynamic_breakpoints?.tablet || '', // eslint-disable-line camelcase
											mobile: value,
										},
									} )
								} }
...
const Blocks = props => {
	...
	const hasGroupMatch = groups.some( group => group.children === null || group.children.length > 0 )
	const disabledBlocks = settings.lumen_block_states ?? {} // eslint-disable-line camelcase
	...
	useEffect( () => {
		...
		window.addEventListener( 'message', ev => {
			if ( typeof ev.data !== 'object' || ev.data.source !== 'LMN_DESIGN_LIBRARY' || ev.origin !== window.location.origin ) {
				return
			}

			if ( ev.data.type === 'LMN_ENABLE_BLOCKS' && ev.data.blocks instanceof Set && ev.data.blocks.size ) {
	...
	const enableAllBlocks = () => {
		...
		BLOCK_CATEROGIES.forEach( ( { id } ) => {
			DERIVED_BLOCKS[ id ].forEach( block => {
				const availableStates = mapStringStates( block[ 'lmn-available-states' ] )
		...
		handleSettingsChange( { lumen_block_states: newDisabledBlocks } ) // eslint-disable-line camelcase
	}
			...
			<div className="s-blocks">
				{ ! hasGroupMatch ? (
					...
					<div className="s-setting-group">
						...
						<p className="s-settings-subtitle">{ __( 'Here you can enable, hide and disable Lumen blocks. Hiding blocks will hide the block from the list of available blocks. Disabling blocks will prevent them from being registered at all. When using block variations or design library patterns, disabled blocks will be substituted with the relevant core blocks.', i18n ) }</p>
				...
				<>
					{ ( optimizations.children === null || optimizations.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<p className="s-settings-subtitle">{ __( 'Here you can adjust some optimization settings that are performed by Lumen.', i18n ) }</p>
							<AdminToggleSetting
								label={ __( 'Optimize Inline CSS', i18n ) }
								searchedSettings={ optimizations.children }
								value={ settings.lumen_optimize_inline_css }
								onChange={ value => {
									handleSettingsChange( { lumen_optimize_inline_css: value } ) // eslint-disable-line camelcase
								} }
					...
					{ ( globalSettings.children === null || globalSettings.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<AdminToggleSetting
								...
								value={ props.settings.lumen_global_force_typography }
								onChange={ value => {
									props.handleSettingsChange( { lumen_global_force_typography: value } ) // eslint-disable-line camelcase
								} }
...
const RoleManager = props => {
	...
	const EditorModeSettings = applyFilters( 'lumen.admin.settings.editorModeSettings', Fragment )
...
const CustomFields = props => {
	...
	const CustomFieldsEnableSettings = applyFilters( 'lumen.admin.settings.customFieldsEnableSettings', Fragment )
	const CustomFieldsManagerSettings = applyFilters( 'lumen.admin.settings.customFieldsManagerSettings', Fragment )
...
const Integrations = props => {
	...
	const IconSettings = applyFilters( 'lumen.admin.settings.iconSettings', Fragment )
				...
				<>
					{ ( propsToPass.integrations.children === null || propsToPass.integrations.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<p className="s-settings-subtitle">{ __( 'Here are settings for the different integrations available in Lumen.', i18n ) }</p>
							<AdminTextSetting
								label={ __( 'Google Maps API Key', i18n ) }
								searchedSettings={ propsToPass.integrations.children }
								value={ settings.lumen_google_maps_api_key }
								type="text"
								maskValue={ true }
								onChange={ value => {
									handleSettingsChange( { lumen_google_maps_api_key: value } ) // eslint-disable-line camelcase
								} }
								help={
									<>
										{ __(
											'Adding a Google API Key enables additional features of the Lumen Map Block.',
											i18n
										) }
									</>
								}
							...
							<div className="s-icon-settings-fa-version">
								<div className="s-icon-settings-fa-pro-version">
									<label className="lmb-admin-setting__label-wrapper" htmlFor="s-icon-settings-fa-pro-version">
										<span className="lmb-admin-setting__label"> { __( 'FontAwesome Icon Library Version', i18n ) }</span>
								...
								<div className="s-icon-settings-fa-free-version">
									<AdminSelectSetting
										...
										searchedSettings={ propsToPass.integrations.children }
										value={ settings.lumen_icons_fa_free_version }
										options={ [
											{
												name: '6.5.1',
												value: '6.5.1',
										...
										onChange={ value => {
											handleSettingsChange( { lumen_icons_fa_free_version: value } ) // eslint-disable-line camelcase
										} }
									/>
								</div>
							</div>
...
const AdditionalOptions = props => {
	...
	const searchClassname = ( label, searchedSettings ) => {
		return searchedSettings.children === null || searchedSettings.children.includes( label )
			? ''
			: 'lmb-admin-setting--not-highlight'
	}
					...
					{ ( miscellaneous.children === null || miscellaneous.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<CheckboxControl
								...
								help={ __( `When enabled, extra frontend CSS is generated to support Lumen global colors used in native blocks. If you don't use Lumen global colors in native blocks, simply toggle this OFF. Please note that Lumen global colors are no longer available for native blocks. To ensure your styles always look perfect, our auto-detect feature will activate this option whenever needed.`, i18n ) }
								checked={ !! settings.lumen_global_colors_native_compatibility }
								onChange={ checked => {
									handleSettingsChange( { lumen_global_colors_native_compatibility: checked } ) // eslint-disable-line camelcase
								} }
								__nextHasNoMarginBottom
							/>
							<CheckboxControl
								...
								help={ __( `When enabled, Lumen blocks will inherit styles from the theme.json file of your Block Theme.`, i18n ) }
								checked={ ! settings.lumen_disable_block_style_inheritance }
								onChange={ checked => {
									handleSettingsChange( { lumen_disable_block_style_inheritance: ! checked } ) // eslint-disable-line camelcase
								} }
								__nextHasNoMarginBottom
							/>
							<CheckboxControl
								...
								checked={ !! settings.lumen_use_v3_16_0_color_scheme_inheritance }
								onChange={ checked => {
									handleSettingsChange( {
										lumen_use_v3_16_0_color_scheme_inheritance: checked,
							...
							<CheckboxControl
								...
								help={ __( `Default state of blocks were previously allowed to be saved. This functionality has since been sunsetted in lieu of the Lumen Design System. Use at your own risk as this feature is slated to be removed in a future version.`, i18n ) }
								checked={ settings.lumen_enable_block_defaults }
								onChange={ checked => {
									handleSettingsChange( {
										lumen_enable_block_defaults: checked, // eslint-disable-line camelcase
									 } )
								} }
								__nextHasNoMarginBottom
							/>
					...
					{ ( migrationSettings.children === null || migrationSettings.children.length > 0 ) &&
						<div className="s-setting-group">
							...
							<CheckboxControl
								...
								checked={ settings.lumen_v2_editor_compatibility === '1' } // eslint-disable-line camelcase
								onChange={ checked => {
									if ( checked ) {
										handleSettingsChange( { lumen_v2_editor_compatibility_usage: '' } ) // eslint-disable-line camelcase
									}
									handleSettingsChange( { lumen_v2_editor_compatibility: checked ? '1' : '' } ) // eslint-disable-line camelcase
								} }
								__nextHasNoMarginBottom
							/>
							<CheckboxControl
								...
								checked={ settings.lumen_v2_editor_compatibility_usage === '1' } // eslint-disable-line camelcase
								onChange={ checked => {
									if ( checked ) {
										handleSettingsChange( { lumen_v2_editor_compatibility: '' } ) // eslint-disable-line camelcase
									}
									handleSettingsChange( { lumen_v2_editor_compatibility_usage: checked ? '1' : '' } ) // eslint-disable-line camelcase
								} }
								__nextHasNoMarginBottom
							/>
							<CheckboxControl
								disabled={ settings.lumen_v2_editor_compatibility === '1' || settings.lumen_v2_editor_compatibility_usage === '1' }
								...
								checked={
									settings.lumen_v2_editor_compatibility	=== '1' ||
									settings.lumen_v2_editor_compatibility_usage === '1' ||
									settings.lumen_v2_frontend_compatibility === '1'
								}
								onChange={ checked => {
									handleSettingsChange( { lumen_v2_frontend_compatibility: checked ? '1' : '' } ) // eslint-disable-line camelcase
								} }
								__nextHasNoMarginBottom
							/>
						</div>
		...
		<div className={ classes }>
			{ ( optimizations.children === null || optimizations.children.length > 0 ) &&
				<div className="s-setting-group">
					...
					<p className="s-settings-subtitle">
						{ __( 'Here are some settings that you can tweak to optimize Lumen.', i18n ) }
						<br />
