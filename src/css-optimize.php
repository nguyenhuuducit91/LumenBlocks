...
if ( ! class_exists( 'Lumen_CSS_Optimize' ) ) {

	/**
	 * Lumen Optimize CSS.
	 */
    class Lumen_CSS_Optimize {
		...
		function __construct() {
			...
			// If lumen_optimize_inline_css === false (or option isn't
			// present), that's the default value (true) for the option.
			if ( ! is_admin() && get_option( 'lumen_optimize_inline_css' ) !== '' ) {
		...
		public function generate_optimied_css_for_post( $post_id, $post, $update ) {
			if ( $post->post_type === 'attachment' ||
				 ...
				 $post->post_type === 'lumen_temp_post' ) { // Temporary post type used by Lumen for editing default blocks or UI Kits.
				return;
			}
			...
			update_post_meta( $post_id, 'lumen_optimized_css', $optimized_css );
			update_post_meta( $post_id, 'lumen_optimized_css_raw', $styles );
		}
			...
			if ( stripos( $block_content, '<style' ) !== false ) {
				...
				if ( is_array( $block['attrs'] ) && array_key_exists( 'uniqueId', $block['attrs'] ) ) {
					...
					foreach ( $styles[0] as $i => $style_tag ) {
						...
						if ( stripos( $styles[1][ $i ], '!#lmn_dynamic' ) ) {
							return;
						}
		...
		public function load_cached_css_for_post() {
			...
			$optimize_css = apply_filters( 'lumen/load_cached_css_for_post', $optimize_css );
			if ( $optimize_css ) {
				$post_id = apply_filters( 'lumen/get_post_id_for_cached_css', get_the_ID() );
				$this->optimized_css = get_post_meta( $post_id, 'lumen_optimized_css', true );

				if ( ! empty( $this->optimized_css ) ) {
					$this->css_raw = get_post_meta( $post_id, 'lumen_optimized_css_raw', true );
		 ...
		 * src/utils/styles/style-object.js get reordered because of
		...
		public static function combine_selectors( $selectors ) {
			...
			foreach( $selectors as $selector ) {
				...
				// Find all the unique id classes of the form ".lmn-123bcd"
				preg_match( '/(.lmn-[a-f0-9-]{7})(?=\s|$)/', $selector, $matches );
		...
		public static function generate_css( $styles ) {
			...
			foreach ( $media_queries as $media_query ) {
				...
				// styles are put last.
				uksort( $styles, array( 'Lumen_CSS_Optimize', 'selector_sort' ) );
		...
		public function protect_optimized_css_meta( $protected, $meta_key ) {
			if ( $meta_key === 'lumen_optimized_css' ||
			     $meta_key === 'lumen_optimized_css_raw' ||
			     $meta_key === 'lumen_css_hash' ) {
				return true;
			}
			return $protected;
		}
