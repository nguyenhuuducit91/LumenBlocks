...
if ( ! function_exists( 'lumen_blocksy_global_color_schemes_compatibility' ) ) {
	function lumen_blocksy_global_color_schemes_compatibility( $styles, $scheme, $selectors, $mode, $classes ) {
		/**
		 * This is WIP. Blocksy is not supported yet.
		 */

		/*
		if ( in_array( 'lmn--is-blocksy-theme', $classes ) ) {
				$bg_property = '--lmn-button-background-color';
				$text_property = '--lmn-button-text-color';
				...
				switch ( $mode ) {
					case 'background':
						$desktop_button_selector = implode(", ", array(
							$selectors[ 'desktop' ] . ' > :where(.lmn-button-group) > .lmn-block-button',
							$selectors[ 'desktop' ] . ' > :where(.lmn-container) > :where(.lmn-inner-blocks) > :where(.lmn-block:not(.lmn-block-background)) > :where(.lmn-button-group) > .lmn-block-button',
						) );

						if (isset( $selectors[ 'desktopParentHover' ] )) {
							$parent_hover_button_selector = implode(", ", array_map( function ( $s ){ return "$s > :where(.lmn-button-group) > .lmn-block-button, $s > :where(.lmn-container) > :where(.lmn-inner-blocks) > :where(.lmn-block:not(.lmn-block-background)) > :where(.lmn-button-group) > .lmn-block-button"; }, $parent_hover_selector ) );
						}
						break;
					case 'container':
						$desktop_button_selector = $selectors[ 'desktop' ] . ' > :where(.lmn-inner-blocks) > :where(.lmn-block:not(.lmn-block-background)) > :where(.lmn-button-group) > .lmn-block-button';

						if (isset( $selectors[ 'desktopParentHover' ] )) {
							$parent_hover_button_selector = implode(", ", array_map( function ( $s ){ return "$s > :where(.lmn-inner-blocks) > :where(.lmn-block:not(.lmn-block-background)) > :where(.lmn-button-group) > .lmn-block-button"; }, $parent_hover_selector ) );
						}
						break;
					default:
						$desktop_button_selector = $selectors[ 'desktop' ] . ' :where(.lmn-block:not(.lmn-block-background)) > :where(.lmn-button-group) > .lmn-block-button';
				}
	...
	add_filter( 'lumen.global-settings.global-color-schemes.add-theme-compatibility', 'lumen_blocksy_global_color_schemes_compatibility', 10, 6 );
}

if ( ! function_exists( 'lumen_blocksy_theme_global_styles' ) ) {
	function lumen_sanitize_css_string( $css ) {
		if ( ! is_string( $css ) ) {
			return '';
		}
	...
	function lumen_blocksy_theme_global_styles( $styles ) {
		...
		if ( class_exists( 'Blocksy_Static_Css_Files' ) ) {
			...
			$blocksy_static_files = array_filter(
				...
				function( $file ) {
					return isset( $file['id'] ) && in_array( $file['id'], array( 'ct-main-styles', 'ct-lumen-styles' ), true );
				}
			);
			...
			if ( $styles_from_files ) {
				...
				$styles_from_files = lumen_sanitize_css_string( $styles_from_files );
	...
	add_filter( 'lumen.design-library.global-theme-styles', 'lumen_blocksy_theme_global_styles' );
}
