...
addFilter( 'lumen.text.edit.layouts', 'defaults', layouts => {
	const newLayouts = [
		{
...
addFilter( 'lumen.text.edit.inspector.layout.before', 'lumen/text', ( output, props ) => {
	...
	return (
		<Fragment>
			...
			<DesignPanelBody
				initialOpen={ true }
				selected={ design }
				options={ applyFilters( 'lumen.text.edit.layouts', [] ) }
...
addFilter( 'lumen.text.edit.inspector.style.before', 'lumen/text', ( output, props ) => {
	const { setAttributes } = props
	const {
		design = 'plain',
		columns = 1,
...
export default compose(
	...
	withClickOpenInspector( [
		[ '.lmb-text__title', 'title' ],
		[ '.lmb-text__subtitle', 'subtitle' ],
		[ '.lmb-text__text', 'text' ],
	] ),
)( edit )
