...
addFilter( 'lumen.expand.edit.inspector.style.before', 'lumen/expand', ( output, props ) => {
...
const edit = props => {
	...
	const mainClasses = classnames( [
		className,
		'lmb-expand--v2',
	], applyFilters( 'lumen.expand.mainclasses', {}, design, props ) )
	...
	return (
		<BlockContainer.Edit className={ mainClasses } blockProps={ props } render={ () => (
			<Fragment>
				...
				{ isSelected && <label className="lmb-expand__label">{ __( 'Less text', i18n ) }</label> }
				<RichText
					multiline="p"
					className="lmb-expand__less-text"
					value={ text }
					onChange={ text => setAttributes( { text } ) }
					placeholder={ __( 'Some short text that can be expanded to show more details.', i18n ) }
					keepPlaceholderOnFocus
				/>
				<div className="lmb-expand__toggle-wrapper">
				...
				{ /* eslint-disable-next-line jsx-a11y/label-has-for */ }
				{ isSelected && <label className="lmb-expand__label">{ __( 'More text', i18n ) }</label> }
				{
					isSelected &&
						<RichText
							multiline="p"
							className="lmb-expand__more-text"
							value={ moreText }
...
export default compose(
	...
	withClickOpenInspector( [
		[ '.lmb-expand__title', 'title' ],
		[ '.lmb-expand__less-text, .lmb-expand__more-text', 'text' ],
		[ '.lmb-expand__more-toggle-text, .lmb-expand__less-toggle-text', 'link' ],
	] ),
)( edit )
