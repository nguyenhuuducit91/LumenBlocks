...
addFilter( 'lumen.count-up.edit.layouts', 'default', layouts => {
	const newLayouts = [
		{
...
addFilter( 'lumen.count-up.edit.inspector.layout.before', 'lumen/count-up', ( output, props ) => {
	...
	return (
		<Fragment>
			...
			<DesignPanelBody
				initialOpen={ true }
				selected={ design }
				options={ applyFilters( 'lumen.count-up.edit.layouts', [] ) }
...
addFilter( 'lumen.count-up.edit.inspector.style.before', 'lumen/count-up', ( output, props ) => {
	const { setAttributes } = props
	const {
		columns,
		borderRadius = '',
...
export default compose(
	...
	withClickOpenInspector( [
		[ '.lmb-container__wrapper', 'column-background' ],
		[ '.lmb-countup__icon svg', 'icon' ],
		[ '.lmb-countup__title', 'title' ],
		[ '.lmb-countup__counter', 'number' ],
		[ '.lmb-countup__description', 'description' ],
	] ),
)( edit )
