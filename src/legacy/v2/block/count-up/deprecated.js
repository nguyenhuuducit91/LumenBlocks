...
export const deprecatedSave_1_15_4 = props => {
	...
	const show = applyFilters( 'lumen.count-up.edit.show_1_15_4', {
	...
	const mainClasses = classnames( [
		// className,
		className.split( ' ' ).filter( name => name !== 'lmb-count-up' ),
		'lmb-countup',
		'lmb-countup--v3', // For backward compatibility.
		`lmb-countup--columns-${ columns }`,
		'lmb--background-opacity-' + ( 1 * Math.round( backgroundOpacity / 1 ) ),
	], applyFilters( 'lumen.count-up.mainclasses_1_15_4', {
		...
		[ `lmb--shadow-${ shadow }` ]: design === 'basic' && shadow !== 3,
		[ `lmb--has-background-gradient` ]: design === 'basic' && backgroundColorType === 'gradient',
		[ `lmb--has-background-video` ]: design === 'basic' && backgroundType === 'video',
	}, design, props ) )
	...
	const mainStyle = applyFilters( 'lumen.count-up.mainstyle_1_15_4', {
	...
	return (
		<div className={ mainClasses } style={ mainStyle }>
			...
			{ applyFilters( 'lumen.count-up.save.output.before_1_15_4', null, design, props ) }
			<div className="lmb-content-wrapper">
				{ range( 1, columns + 1 ).map( i => {
					...
					return applyFilters( 'lumen.count-up.save.output_1_15_4', (
						<div className="lmb-countup__item" key={ i }>
							{ titleComp }
							{ countComp }
							{ descriptionComp }
						</div>
...
const deprecated = [
	{
		...
		migrate: attributes => {
			...
			const updateCSS = css => ( css || '' ).replace( /\.lmb-countup([\s\{\[\.\#\:])/g, '.lmb-count-up$1' )
