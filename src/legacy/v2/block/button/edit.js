...
import { createButtonAttributeNames } from '~lumen/utils'
...
addFilter( 'lumen.button.edit.layouts', 'default', layouts => {
	const newLayouts = [
		{
...
addFilter( 'lumen.button.edit.inspector.layout.before', 'lumen/button', ( output, props ) => {
	...
	return (
		<Fragment>
			...
			<DesignPanelBody
				initialOpen={ true }
				selected={ design }
				options={ applyFilters( 'lumen.button.edit.layouts', [] ) }
...
addFilter( 'lumen.button.edit.inspector.style.before', 'lumen/button', ( output, props ) => {
	const { setAttributes } = props
	const {
		borderRadius = '',
		collapseOn = '',
		...
		<Fragment>
			...
			<PanelAdvancedSettings title={ __( 'General', i18n ) } initialOpen={ false }>
				<AdvancedRangeControl
					...
					allowReset={ true }
					placeholder="12"
					className="lmb--help-tip-button-border-radius"
				/>
				<ContentAlignControl
				...
				<SelectControl
					...
					className="lmb--help-tip-button-collapse"
				/>
			</PanelAdvancedSettings>
			<PanelAdvancedSettings
...
const edit = props => {
	...
	const mainClasses = classnames( [
		className,
	], applyFilters( 'lumen.button.mainclasses', {
		'lmb-button--tablet-aligned': tabletContentAlign !== '',
		'lmb-button--mobile-aligned': mobileContentAlign !== '',
		[ `lmb-button--collapse-${ collapseOn }` ]: collapseOn,
