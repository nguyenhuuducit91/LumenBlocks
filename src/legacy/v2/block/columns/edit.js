...
addFilter( 'lumen.columns.edit.layouts', 'default', layouts => {
	const newLayouts = [
		{
...
addFilter( 'lumen.columns.edit.inspector.layout.attributes', 'lumen/columns', attributes => {
	const {
		design,
		columns = 2,
	} = attributes
...
addFilter( 'lumen.columns.edit.inspector.layout.before', 'lumen/columns', ( output, props ) => {
	...
	return (
		<Fragment>
			...
			>
				...
				{ applyFilters( 'lumen.columns.edit.inspector.columns.after', null, props ) }
				...
				<DesignControl
					...
					options={ applyFilters( 'lumen.columns.edit.layouts', [] ) }
					onChange={ design => {
						setAttributes( applyFilters( 'lumen.columns.edit.inspector.layout.attributes', { design, columns } ) )
					} }
				/>
...
addFilter( 'lumen.columns.edit.inspector.style.before', 'lumen/columns', ( output, props ) => {
	const { setAttributes } = props
	const {
		design = 'plain',
		columns = 2,
	...
	return (
		<Fragment>
			...
			>
				...
				{ applyFilters( 'lumen.columns.edit.inspector.style.column-widths.after', null, props ) }
...
addFilter( 'lumen.columns.edit.advanced.responsive.after', 'lumen/columns', output => {
	return (
		<Fragment>
			{ output }
			{ showProNotice && <ProControlButton
...
const edit = props => {
	...
	const mainClasses = classnames( [
		...
		[ `lmb-columns--highlight-${ sortColumnHighlight + 1 }` ]: sortColumnHighlight !== null,
	}, props ) )
	...
	const template = range( columns ).map( () => [ 'lmb/column' ] )
...
addFilter( 'lumen.columns.setAttributes', 'lumen/columns/columns-change', ( attributes, blockProps ) => {
	...
	const newInnerBlocks = range( columns || 2 ).map( i => {
		return currentInnerBlocks[ i ] || createBlock( 'lmb/column', {}, [] )
	} )
