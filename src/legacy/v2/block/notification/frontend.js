 ...
 */
domReady( () => {
	const elems = document.querySelectorAll( '.lmb-notification.lmb-notification--dismissible' )
	elems.forEach( el => {
		// Dismiss handler.
		...
		el.querySelector( '.lmb-notification__close-button' ).addEventListener( 'click', ev => {
			ev.preventDefault()
			localStorage.setItem( `stckbl-notif-${ uid }`, 1 )
			el.style.display = ''
		} )
		el.querySelector( '.lmb-notification__close-button' ).addEventListener( 'keypress', ev => {
			ev.preventDefault()
			localStorage.setItem( `stckbl-notif-${ uid }`, 1 )
			el.style.display = ''
		} )
 ...
 */
domReady( () => {
	const elemsDep = document.querySelectorAll( '.lmb-notification.dismissible-true[data-uid]' )
	elemsDep.forEach( el => {
		// Dismiss handler.
		const uid = el.getAttribute( 'data-uid' )
