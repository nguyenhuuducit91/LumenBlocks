...
if ( ! function_exists( 'lumen_block_blog_posts_is_deprecated_v2' ) ) {
	/**
	 ...
	 * @since 2.0
	 */
	function lumen_block_blog_posts_is_deprecated_v2( $attributes, $content = '' ) {
...
if ( ! function_exists( 'lumen_block_blog_posts_migrate_attributes_v2' ) ) {
	/**
	 ...
	 * @since 2.0
	 */
	function lumen_block_blog_posts_migrate_attributes_v2( $attributes, $block_name ) {
		...
		$css = preg_replace( '/\.lmb-blog-posts__category-list/i', '.lmb-blog-posts__category', $css );
		$css = preg_replace( '/\.lmb-blog-posts__read_more/i', '.lmb-blog-posts__readmore', $css );
		$css = preg_replace( '/\.lmb-blog-posts__side/i', '.lmb-blog-posts__content', $css );
	...
	add_filter( 'lumen_block_migrate_attributes_v2', 'lumen_block_blog_posts_migrate_attributes_v2', 10, 2 );
}
...
if ( ! function_exists( 'lumen_block_blog_posts_migrate_output_v2' ) ) {
	 ...
	 * @since 2.0
	 */
	function lumen_block_blog_posts_migrate_output_v2( $output, $attributes, $content ) {
		...
		return sprintf( '<div class="lmb-blog-posts lmb-blog-posts-old-%s lmb-blog-posts--columns-%s lmb-blog-posts--v2 lmb-main-block lmb-blog-posts--design-%s %s %s" style="%s%s"><div class="lmb-inner-block">%s<div class="lmb-block-content">%s</div></div></div>',
			...
			$attributes['categoryHighlighted'] ? 'lmb-blog-posts--cat-highlighted' : '',
	...
	add_filter( 'lumen/blog-posts/v2/edit.output.markup', 'lumen_block_blog_posts_migrate_output_v2', 10, 3 );
}
