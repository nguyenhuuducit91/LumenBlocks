...
addFilter( 'lumen.blog-posts.edit.layouts', 'default', layouts => {
	const newLayouts = [
		{
...
addFilter( 'lumen.blog-posts.edit.inspector.layout.before', 'lumen/blog-posts', ( output, props ) => {
	...
	return (
		<Fragment>
			...
			<DesignPanelBody
				initialOpen={ true }
				selected={ design }
				options={ applyFilters( 'lumen.blog-posts.edit.layouts', [] ) }
...
addFilter( 'lumen.blog-posts.edit.inspector.style.before', 'lumen/blog-posts', ( output, props ) => {
	const { setAttributes } = props
	const {
		columns = 2,
		borderRadius = '',
		...
		<Fragment>
			...
			>
				...
				<AdvancedSelectControl
					...
					className="lmb--help-tip-posts-content-order"
				/>
				<ContentAlignControl
			...
			>
				...
				/>
				{ applyFilters( 'lumen.blog-posts.edit.inspector.style.posts.after', null, props ) }
				{ showProNotice && <ProControlButton
					// Generic label, no translations to keep old text out of the translation files.
					title="Upgrade to Premium"
					description="Get more layouts, effects, designs and options for this block if you upgrade to Lumen Premium."
			...
			{ show.columnBackground &&
				<PanelAdvancedSettings
					...
					initialOpen={ false }
					className="lmb--help-tip-column-background-on-off"
				>
					<ButtonIconPopoverControl
				...
				{ show.imageSpacing &&
					...
					>
						<AdvancedRangeControl
							...
							placeholder="16"
							allowReset={ true }
							className="lmb--help-tip-alignment-image"
						/>
					</ResponsiveControl>
				}
				{ show.categorySpacing &&
					...
					>
						<AdvancedRangeControl
							...
							placeholder="16"
							allowReset={ true }
							className="lmb--help-tip-alignment-description"
						/>
					</ResponsiveControl>
				}
				{ show.titleSpacing &&
					...
					>
						<AdvancedRangeControl
							...
							placeholder="16"
							allowReset={ true }
							className="lmb--help-tip-alignment-title"
						/>
					</ResponsiveControl>
				}
				{ show.excerptSpacing &&
				...
				{ show.readmoreSpacing &&
					...
					>
						<AdvancedRangeControl
							...
							placeholder="16"
							allowReset={ true }
							className="lmb--help-tip-alignment-button"
						/>
					</ResponsiveControl>
				}
			...
			>
				<ImageSizeControl
					...
					className="lmb--help-tip-image-size"
				/>
				{ show.imageWidth &&
					<ResponsiveControl
					...
					>
						<AdvancedRangeControl
							...
							placeholder="100"
							allowReset={ true }
							className="lmb--help-tip-image-width-crop"
						/>
					</ResponsiveControl>
				}
				{ show.imageHeight &&
					...
					>
						<AdvancedRangeControl
							...
							placeholder="500"
							allowReset={ true }
							className="lmb--help-tip-image-height-crop"
						/>
					</ResponsiveControl>
				}
			</PanelAdvancedSettings>
			...
			>
				...
				<AdvancedSelectControl
					...
					className="lmb--help-tip-posts-meta-separator"
				/>
				<ResponsiveControl
			...
			{ applyFilters( 'lumen.blog-posts.edit.inspector.style.pagination.after', null, props ) }
			...
			{ applyFilters( 'lumen.blog-posts.edit.inspector.style.read-more.after', null, props ) }
...
const Edit = props => {
	...
	const postQuery = useMemo( () => {
		...
		const postQuery = pickBy( {
			...applyFilters( 'lumen.blog-posts.postQuery', {
				order,
				orderby: orderBy,
	...
	} = applyFilters( 'lumen.blog-posts.pagination.hooks', posts, numberOfItems ) || {}
	...
	const itemClasses = classnames( [
		'lmb-blog-posts__item',
	], {
		[ `lmb--shadow-${ shadow || 3 }` ]: ! show.imageShadow,
	} )
			...
			<Fragment>
				{ ( currentPagePosts || [] ).map( ( post, i ) => {
					...
					const featuredImageBackground = featuredImageSrc &&
						<div
							className="lmb-blog-posts__featured-image-background"
							style={ { backgroundImage: `url(${ featuredImageSrc })` } }
						/>

					const title = <TitleTag className="lmb-blog-posts__title">
						{ /* eslint-disable-next-line jsx-a11y/anchor-is-valid */ }
						<a>{ decodeEntities( post.title.rendered.trim() ) || __( '(Untitled)', i18n ) }</a>
					</TitleTag>

					const category = post.category_list_v2 &&
						<div className="lmb-blog-posts__category" dangerouslySetInnerHTML={ { __html: post.category_list_v2.replace( /href=['"].*?['"]/g, '' ) } } />

					const separator = <span className="lmb-blog-posts__sep">{ META_SEPARATORS[ metaSeparator || 'dot' ] }</span>

					const author = post.author_info_v2 && post.author_info_v2.name &&
						<span>{ post.author_info_v2.name }</span>

					const date = post.date_gmt &&
						<time dateTime={ format( 'c', post.date_gmt ) } className="lmb-blog-posts__date">
					...
					// Trim the excerpt.
					let excerptString = post.post_excerpt_lumen_v2.split( ' ' )
					...
					const excerpt = excerptString &&
						<div className="lmb-blog-posts__excerpt" dangerouslySetInnerHTML={ { __html: excerptString } } />

					const readmore = <p className="lmb-blog-posts__readmore">
						{ /* eslint-disable-next-line jsx-a11y/anchor-is-valid */ }
						<a>{ readmoreText ? readmoreText : __( 'Continue reading', i18n ) }</a>
					</p>

					const meta = ( showAuthor || showDate || showComments ) &&
						<aside className="entry-meta lmb-blog-posts__meta">
							{ showAuthor && author }
					...
					const output = applyFilters( 'lumen.blog-posts.edit.output', null, props, {
						itemClasses,
					...
					return (
						...
						>
							...
							<DivBackground
								className="lmb-blog-posts__content"
								backgroundAttrName="column%s"
								blockProps={ props }
				...
				{ applyFilters( 'lumen.blog-posts.edit.output.pagination.after', null, {
					...props,
					showPagination,
					containerClassName: 'lmb-blog-posts__pagination',
...
export default compose(
	...
	withClickOpenInspector( [
		[ '.lmb-blog-posts__featured-image', 'image' ],
		[ '.lmb-blog-posts__featured-image img', 'image' ],
		[ '.lmb-blog-posts__category', 'category' ],
		[ '.lmb-blog-posts__title', 'title' ],
		[ '.lmb-blog-posts__excerpt', 'excerpt' ],
		[ '.lmb-blog-posts__meta', 'meta' ],
		[ '.lmb-blog-posts__readmore', 'readmore' ],
		[ '.lmb-blog-posts--design-image-card .lmb-blog-posts__header', 'image' ],
		[ '.lmb-blog-posts__item', 'column-background' ],
		[ '.lmb-blog-posts__load-more-button', 'loadmore' ],
		[ '.lmb-blog-posts__pagination', 'pagination' ],
	] ),
)( Edit )

addFilter( 'lumen.click-open-inspector.listener-override', 'lumen/blog-posts', override => {
	return {
		...override,
		'[data-type="lmb/blog-posts"]': [
			'img',
			'p',
			'h1',
			'h2',
