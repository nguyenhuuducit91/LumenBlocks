<?php
/**
 * Server-side rendering of the `lmb/blog-posts` block.
 *
 * @package Lumen
 */
...
if ( ! function_exists( 'lumen_attributes_default_v2' ) ) {
	/**
	 ...
	 * @since 2.0
	 */
	function lumen_attributes_default_v2( $attributes, $defaults ) {
		$out = array();
...
if ( ! function_exists( 'lumen_blog_posts_block_default_attributes_v2' ) ) {
	function lumen_blog_posts_block_default_attributes_v2( $attributes ) {
		$defaults = array(
			'postType' => 'post',
			'numberOfItems' => 6,
			'orderBy' => 'date',
		...
		return lumen_attributes_default_v2( $attributes, $defaults );
	}
}

if ( ! function_exists( 'lumen_blog_posts_post_query_v2' ) ) {
	function lumen_blog_posts_post_query_v2( $attributes ) {
		...
		return apply_filters( 'lumen/blog-post/v2/post_query',
			$passed_attributes,
			$attributes
		);
	}
...
/**
 * Renders the `lmb/blog-posts` block on server.
 *
 * @since 1.7
 *
 ...
 */
if ( ! function_exists( 'lumen_render_blog_posts_block_v2' ) ) {
    function lumen_render_blog_posts_block_v2( $attributes, $content ) {
		// Migrate attributes if this is an old block.
		if ( lumen_block_blog_posts_is_deprecated_v2( $attributes, $content ) ) {
			$attributes = apply_filters( 'lumen_block_migrate_attributes_v2', $attributes, 'blog-posts' );
		}

		$attributes = lumen_blog_posts_block_default_attributes_v2( $attributes );
		$post_query = lumen_blog_posts_post_query_v2( $attributes );
		...
		$posts_markup = '';
		$show = lumen_blog_posts_util_show_options_v2( $attributes );
		...
		// Item classes.
		$item_classes = array( 'lmb-blog-posts__item' );
		if ( ! $show['imageShadow'] && (int) $attributes['shadow'] !== 0 ) {
			$item_classes[] = 'lmb--shadow-' . $attributes['shadow'];
		}
		...
		if ( $show['showBackgroundInItem'] &&
			...
			$item_classes[] = 'lmb--has-background-overlay';
		}
		$item_classes = implode( ' ', $item_classes );

		$content_classes = array( 'lmb-blog-posts__content' );
		// Add background gradient class.
		if ( $show['showBackgroundInContent'] &&
			( $attributes['columnBackgroundColorType'] === 'gradient' ||
			  ...
			  $attributes['columnTabletBackgroundMediaUrl' ] ||
			  $attributes['columnMobileBackgroundMediaUrl' ] ) ) {
			$content_classes[] = 'lmb--has-background-overlay';
		}
		$content_classes = implode( ' ', $content_classes );

		// Image classes.
		$featured_image_classes = array( 'lmb-blog-posts__featured-image' );
		if ( $show['imageShadow'] && (int) $attributes['shadow'] !== 0 ) {
			$featured_image_classes[] = 'lmb--shadow-' . $attributes['shadow'];
		}
        ...
        foreach ( $recent_posts as $i => $post ) {
			...
			if ( ! empty( $featured_image_id ) ) {
				$featured_image_urls = lumen_featured_image_urls_from_url_v2( $featured_image_id );
				...
				if ( ! empty( $featured_image_src ) ) {
					...
					$featured_image_background = sprintf(
						...
						'lmb-blog-posts__featured-image-background',
			...
			// Separator.
			$separator = sprintf(
				'<span class="lmb-blog-posts__sep">%s</span>',
            ...
            // Date.
			$date = sprintf(
				'<time datetime="%1$s" class="lmb-blog-posts__date">%2$s</time>',
            ...
            // Excerpt.
			$excerpt = lumen_get_excerpt_v2( $post_id, $post );
			...
			if ( ! empty( $excerpt ) ) {
				...
				$excerpt = sprintf(
					'<div class="lmb-blog-posts__excerpt">%s</div>',
			...
			$readmore = sprintf(
				'<p class="lmb-blog-posts__readmore"><a href="%s">%s</a></p>',
				esc_url( get_permalink( $post_id ) ),
			...
			if ( $showAuthor || $showDate || $showComments ) {
				$meta = sprintf(
					'<aside class="entry-meta lmb-blog-posts__meta">%s%s%s%s%s</aside>',
			...
			$output = apply_filters( 'lumen/blog-posts/v2/edit.output', null, $attributes, array(
		...
		do_action( 'lumen/blog-posts/v2/render', $attributes, $content );

        return wp_kses_post( apply_filters( 'lumen/blog-posts/v2/edit.output.markup', $posts_markup, $attributes, $content ) );
    }
}

if ( ! function_exists( 'lumen_register_blog_posts_block_v2' ) ) {
    /**
     * Add our render attributes.
     */
    function lumen_register_blog_posts_block_v2( $options, $block_name ) {
		if ( $block_name !== 'lmb/blog-posts' ) {
			return $options;
		}

		$options['attributes'] = lumen_blog_posts_attributes_v2();
		$options['render_callback'] = 'lumen_render_blog_posts_block_v2';
		return $options;
    }
    add_filter( 'lumen.v2.register-blocks.options', 'lumen_register_blog_posts_block_v2', 10, 2 );
}

if ( ! function_exists( 'lumen_blog_posts_rest_fields_v2' ) ) {
    /**
     * Add more data in the REST API that we'll use in the blog post.
     *
     * @since 1.7
     */
    function lumen_blog_posts_rest_fields_v2() {
		...
		foreach ( $post_types as $post_type => $data ) {
			...
			// Excerpt.
			register_rest_field( $post_type, 'post_excerpt_lumen_v2',
				array(
...
if ( ! function_exists( 'lumen_featured_image_urls_v2' ) ) {
    /**
     ...
     * @since 1.7
     */
    function lumen_featured_image_urls_v2( $object, $field_name, $request ) {
		return lumen_featured_image_urls_from_url_v2( ! empty( $object['featured_media'] ) ? $object['featured_media'] : '' );
	}
}

if ( ! function_exists( 'lumen_featured_image_urls_from_url_v2' ) ) {
	/**
     * Get the different featured image sizes that the blog will use.
     *
     * @since 2.0
     */
	function lumen_featured_image_urls_from_url_v2( $attachment_id ) {
...
if ( ! function_exists( 'lumen_author_info_v2' ) ) {
    /**
     * Get the author name and link.
     *
     * @since 1.7
     */
    function lumen_author_info_v2( $object ) {
...
if ( ! function_exists( 'lumen_commments_number_v2' ) ) {
    /**
...
if ( ! function_exists( 'lumen_category_list_v2' ) ) {
    /**
...
if ( ! function_exists( 'lumen_post_excerpt_v2' ) ) {
    /**
     * Get the post excerpt.
     *
     * @since 1.7
     */
    function lumen_post_excerpt_v2( $object ) {
        return lumen_get_excerpt_v2( $object['id'] );
    }
}

if ( ! function_exists( 'lumen_get_excerpt_v2' ) ) {
	/**
	 * Get the excerpt.
	 *
	 * @since 1.7
	 */
	function lumen_get_excerpt_v2( $post_id, $post = null ) {
...
if ( ! function_exists( 'lumen_render_block_blog_posts_v2' ) ) {
	/**
	 * Combine our JS & PHP block outputs.
	 *
	 * @since 2.0
	 */
	function lumen_render_block_blog_posts_v2( $block_content, $block ) {
		if ( $block['blockName'] !== 'lmb/blog-posts' ) {
		 ...
		 *
		 * <div class="wp-block-lmb-blog-posts ...">
		 *     <style> ...some generated styles </style>
		 *     <div class="lmb-inner-block">
		 *         <div class="lmb-block-content"></div>
		 *     </div>
		 * </div>
		 *
		 * We need to place the contents inside ".lmb-block-content"
		 */
		...
		// the replacement 
		$parts = preg_split( '/(lmb-block-content[\'"]\s*>)/', $new_content, -1, PREG_SPLIT_DELIM_CAPTURE );
		if ( count( $parts ) < 3 ) {
			return $block_content;
		}
...
if ( ! function_exists( 'lumen_rest_get_terms_v2' ) ) {
	/**
	 ...
	 * @since 2.0
	 */
	function lumen_rest_get_terms_v2() {
		$args = array(
			'public' => true,
		);
...
if ( ! function_exists( 'lumen_get_terms_endpoint_v2' ) ) {
	/**
	 * Define our custom REST API endpoint for getting all the terms/taxonomies.
	 *
	 * @since 2.0
	 */
	function lumen_get_terms_endpoint_v2() {
		register_rest_route( 'lumen/v2', '/terms', array(
			'methods' => 'GET',
			'callback' => 'lumen_rest_get_terms_v2',
