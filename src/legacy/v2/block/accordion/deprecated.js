...
const deprecatedSchema_1_17_3 = {
	heading: {
		source: 'html',
		selector: '.lmb-accordion__heading h4',
		default: __( 'Title for This Block', i18n ),
	},
	text: {
		source: 'html',
		selector: '.lmb-accordion__text',
...
const deprecatedSave_1_17_3 = props => {
	...
	const mainClasses = classnames( [
		className,
		'lmb-accordion',
	], applyFilters( 'lumen.accordion.mainclasses_1_17_3', {
		[ `lmb-accordion--design-${ design }` ]: design !== 'basic',
		'lmb-accordion--open': openStart,
	}, design, props ) )

	const headingClasses = classnames( [
		'lmb-accordion__heading',
	], applyFilters( 'lumen.accordion.headingclasses_1_17_3', {
		[ `lmb--shadow-${ shadow }` ]: design !== 'plain' && shadow !== 3,
	}, design, props ) )

	const styles = applyFilters( 'lumen.accordion.styles_1_17_3', {
		main: {},
		heading: {
	...
	return (
		<div className={ mainClasses } style={ styles.main } role="presentation">
			{ applyFilters( 'lumen.accordion.save.output.before_1_17_3', null, design, props ) }
			...
			<RichText.Content
				tagName="p"
				className="lmb-accordion__text"
				role="region"
				value={ text }
			/>
			{ applyFilters( 'lumen.accordion.save.output.after', null, design, props ) }
		</div>
	)
}
	...
	},
	{
		...
		migrate: attributes => {
			...
			const updateCSS = css => ( css || '' )
				.replace( /\n\.lmb-accordion__heading h4(\s*{)/g, '\n.lmb-accordion__title$1' )
				.replace( /\.lmb-accordion__text/g, '.lmb-accordion__content' )
				.replace( /\.lmb-accordion__heading svg/g, '.lmb-accordion__arrow' )
