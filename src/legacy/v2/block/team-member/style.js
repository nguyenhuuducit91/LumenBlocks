...
export const createStyles = props => {
	...
	if ( show.border ) {
		styles.push( {
			...createBorderStyleSet( 'column%s', '.lmb-team-member__item', props.attributes ),
		} )
	}

	// Column Background.
	const columnBackgroundSelector = applyFilters( 'lumen.team-member.styles.column-background', 'lmb-team-member__item', props.attributes )
	styles.push( {
	...
	styles.push( {
		...
		editor: {
			desktopTablet: {
				'> .lmb-inner-block > .lmb-block-content > .lmb-team-member__item': appendImportantAll( {
					paddingTop: getValue( 'columnPaddingTop', `%s${ columnPaddingUnit }` ),
					paddingBottom: getValue( 'columnPaddingBottom', `%s${ columnPaddingUnit }` ),
					paddingRight: getValue( 'columnPaddingRight', `%s${ columnPaddingUnit }` ),
					paddingLeft: getValue( 'columnPaddingLeft', `%s${ columnPaddingUnit }` ),
	...
	if ( ! show.imageAsBackground && showImage ) {
		...
		styles.push( ...createResponsiveStyles( '.lmb-team-member__image', 'image%sWidth', 'width', '%spx', props.attributes, { important: true, inherit: false } ) )

		styles.push( {
			'.lmb-img, .lmb-team-member__image': {
		...
		styles.push( {
			...createImageBackgroundStyleSet( 'image%s', 'lmb-team-member__image', props.attributes ),
		} )

		range( 1, columns + 1 ).forEach( i => {
			styles.push( {
				[ `.lmb-team-member__item.lmb-team-member__item${ i } .lmb-team-member__image` ]: {
	...
	if ( showSocial ) {
		styles.push( {
			...createSocialButtonStyleSet( `social%s`, `lmb-button`, {
				...props.attributes,
				buttonDesign: socialDesign,
			} ),
		} )
		styles.push( {
			'.lmb-team-member__buttons': {
	...
	if ( show.imageSpacing ) {
		styles.push( ...createResponsiveStyles( '.lmb-team-member__image', 'image%sBottomMargin', 'marginBottom', '%spx', props.attributes, { important: true } ) )
	}
	if ( show.nameSpacing ) {
		styles.push( ...createResponsiveStyles( '.lmb-team-member__name', 'name%sBottomMargin', 'marginBottom', '%spx', props.attributes, { important: true } ) )
	}
	if ( show.positionSpacing ) {
		styles.push( ...createResponsiveStyles( '.lmb-team-member__position', 'position%sBottomMargin', 'marginBottom', '%spx', props.attributes, { important: true } ) )
	}
	if ( show.descriptionSpacing ) {
		styles.push( ...createResponsiveStyles( '.lmb-team-member__description', 'description%sBottomMargin', 'marginBottom', '%spx', props.attributes, { important: true } ) )
	}
	if ( show.socialSpacing ) {
		styles.push( ...createResponsiveStyles( '.lmb-button-container', 'social%sBottomMargin', 'marginBottom', '%spx', props.attributes, { important: true } ) )
		styles.push( ...createResponsiveStyles( '.lmb-button', 'social%sGap', 'marginLeft', '%spx', props.attributes, { important: true } ) )
		styles.push( ...createResponsiveStyles( '.lmb-button', 'social%sGap', 'marginRight', '%spx', props.attributes, { important: true } ) )
	}
	...
	if ( showImage ) {
		range( 1, columns + 1 ).forEach( i => {
			if ( props.attributes[ `image${ i }Shape` ] ) {
				styles.push( {
					[ `.lmb-team-member__item${ i } .lmb-img` ]: createImageMask( `image${ i }%s`, props.attributes ),
				} )
			}
		} )
	}
