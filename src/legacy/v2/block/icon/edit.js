...
addFilter( 'lumen.icon.edit.inspector.style.before', 'lumen/icon', ( output, props ) => {
...
const Edit = props => {
	...
	const mainClasses = classnames( [
		className,
		`lmb-icon--design-${ design }`,
	], applyFilters( 'lumen.icon.mainclasses', {
	}, props ) )

	// Updates the position of the url popup to be opposite of the icon popup.
	const refreshPosition = () => {
		const popover = document.querySelector( '.lmb-icon-popover' )
		if ( popover ) {
	...
	return (
		<BlockContainer.Edit className={ mainClasses } blockProps={ props } render={ () => (
			<Fragment>
				<div className="lmb-icon__content-wrapper">
					{ range( 1, columns + 1 ).map( i => {
						...
						const boxClasses = classnames( [
							'lmb-icon__item',
							`lmb-icon__item${ i }`,
						], applyFilters( 'lumen.icon.boxclasses', {}, props ) )

						const iconComp = (
							<div key="icon" className="lmb-icon__icon">
								<SvgIconPlaceholder
									attrNameTemplate="icon%s"
									blockAttributes={ props.attributes }
									value={ icon }
...
export default compose(
	...
	withClickOpenInspector( [
		[ '.lmb-icon__item', 'column-background' ],
		[ '.lmb-icon-inner-svg svg', 'icon' ],
		[ '.lmb-icon__title', 'title' ],
	] ),
)( Edit )
