...
addFilter( 'lumen.card.edit.layouts', 'default', layouts => {
	const newLayouts = [
		{
...
addFilter( 'lumen.card.edit.inspector.layout.before', 'lumen/card', ( output, props ) => {
	...
	return (
		<Fragment>
			...
			<DesignPanelBody
				initialOpen={ true }
				selected={ design }
				options={ applyFilters( 'lumen.card.edit.layouts', [] ) }
...
addFilter( 'lumen.card.edit.inspector.style.before', 'lumen/card', ( output, props ) => {
	const { setAttributes } = props
	const {
		columns,
		borderRadius = '',
...
const edit = props => {
	...
	const mainClasses = classnames( [
		className,
		'lmb-card--v2',
		`lmb-card--design-${ design }`,
		`lmb-card--columns-${ columns }`,
	], applyFilters( 'lumen.card.mainclasses', {
	}, props ) )
			...
			<Fragment>
				{ range( 1, columns + 1 ).map( i => {
					...
					const itemClasses = classnames( [
						'lmb-card__item',
						`lmb-card__item${ i }`,
					], applyFilters( 'lumen.card.itemclasses', {
						[ `lmb--shadow-${ shadow }` ]: show.columnBackground && shadow !== '',
					}, props ) )

					const imageClasses = classnames( [
						'lmb-card__image',
					], applyFilters( 'lumen.card.imageclasses', {
						[ `lmb--shadow-${ shadow }` ]: ! show.columnBackground,
					}, props ) )
					...
					return (
						...
						>
							...
							<div className="lmb-card__content">
								{ showTitle &&
									<RichText
										tagName={ titleTag || 'h4' }
										value={ title }
										className="lmb-card__title"
								...
								{ showDescription &&
									<RichText
										tagName="p"
										value={ description }
										className="lmb-card__description"
...
export default compose(
	...
	withClickOpenInspector( [
		[ '.lmb-card__title', 'title' ],
		[ '.lmb-card__subtitle', 'subtitle' ],
		[ '.lmb-card__description', 'description' ],
		[ '.lmb-button', 'button' ],
		[ '.lmb-card__item', 'column-background' ],
	] ),
