...
{ [ 1, 2, 3, 4 ].forEach( i => {
	deprecatedSchema_1_17_3[ `title${ i }` ] = {
		source: 'html',
		selector: `.lmb-image-box__item:nth-of-type(${ i }) .lmb-image-box__title`,
		default: __( 'Title', i18n ),
	}
	deprecatedSchema_1_17_3[ `description${ i }` ] = {
		source: 'html',
		selector: `.lmb-image-box__item:nth-of-type(${ i }) .lmb-image-box__description`,
	...
	deprecatedSchema_1_17_3[ `link${ i }` ] = {
		type: 'string',
		source: 'attribute',
		selector: `.lmb-image-box__item:nth-of-type(${ i }) .lmb-image-box__overlay`,
		attribute: 'href',
		default: '',
	}
...
const deprecatedSave_1_17_3 = props => {
	...
	], applyFilters( 'lumen.image-box.mainclasses_1_17_3', {
		[ `lmb-image-box--design-${ design }` ]: design !== 'basic',
		[ `lmb-image-box--effect-${ imageHoverEffect }` ]: imageHoverEffect,
		[ `lmb-image-box--overlay-${ overlayOpacity }` ]: overlayOpacity !== 7,
		'lmb-image-box--arrow': arrow,
	}, design, props ) )
	...
	return (
		<div className={ mainClasses } style={ mainStyles }>
			{ applyFilters( 'lumen.image-box.save.output.before_1_17_3', null, design, props ) }
			{ range( 1, columns + 1 ).map( i => {
				...
				const boxClasses = classnames( [
					'lmb-image-box__item',
				], applyFilters( 'lumen.image-box.itemclasses_1_17_3', {
					[ `lmb--shadow-${ shadow }` ]: shadow !== 3,
				}, design, i, props ) )

				const arrowClasses = classnames( [
					'lmb-image-box__arrow',
					`lmb-image-box__arrow--align-${ arrow }`,
				] )

				return (
					<div className={ boxClasses } style={ boxStyles } key={ i }>
						{ imageHoverEffect && <div
							className="lmb-image-box__image-effect"
							style={ {
								backgroundImage: imageURL ? `url(${ imageURL })` : undefined,
							} } />
						}
						...
						<a
							className="lmb-image-box__overlay"
							href={ link }
			...
			{ applyFilters( 'lumen.image-box.save.output.after_1_17_3', null, design, props ) }
		</div>
	)
}
...
const migrate_1_17_3 = attributes => {
	...
	const updateCSS = css => ( css || '' )
		.replace( /\.lmb-image-box__overlay(\s*{)/g, '.lmb-image-box__overlay-hover$1' )
