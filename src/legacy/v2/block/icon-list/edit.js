...
const includeEditorContentAlignClassName = attributes => {
	...
	addedAlignmentClass[ `lmb-icon-list__${ alignment }-align` ] = alignment

	return addedAlignmentClass
}

addFilter( 'lumen.icon-list.edit.inspector.style.before', 'lumen/icon-list', ( output, props ) => {
		...
		<Fragment>
			...
			>
				...
				{ ( ( columns && columns > 1 ) || ( tabletColumns && tabletColumns > 1 ) || ( mobileColumns && mobileColumns > 1 ) ) && (
					<AdvancedToggleControl
						...
						className="lmb--help-tip-icon-list-grid"
					/>
				) }
				<ContentAlignControl
					setAttributes={ setAttributes }
			...
			<PanelSpacingBody initialOpen={ false } blockProps={ props }>
				<AdvancedRangeControl
					...
					allowReset={ true }
					placeholder="16"
					className="lmb--help-tip-icon-list-gap"
				/>
			</PanelSpacingBody>
