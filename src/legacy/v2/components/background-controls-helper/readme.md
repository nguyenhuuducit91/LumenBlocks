...
Use the helper functions in `@lumen/util/background` in conjuction with this.

Adds the `BackgroundControls` in a quicker way:

```js
import { BackgroundControlsHelper } from '@lumen/components'
...
```js
import { BackgroundControls } from '@lumen/components'
