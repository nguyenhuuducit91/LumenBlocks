...
const ColumnsWidthControl = props => {
	...
	const resetButton = (
		<Button
			className="lmb-columns-width-control__reset"
	...
	return (
		<BaseControl
			help={ props.help }
			className={ classnames( [ 'lmb-columns-width-control', 'lmb--help-tip-column-width', props.className ] ) }
		>
			...
			{ props.hasIndividualControls &&
				<Fragment>
					...
					{ false && range( props.columns ).map( i => {
						return (
							<div key={ i } className="lmb-columns-width-control__range">
								<span className="lmb-columns-width-control__icon">{ i + 1 }</span>
								<AdvancedRangeControl
									...
									step="1"
								/>
								<span className="lmb-columns-width-control__suffix">%</span>
							</div>
						)
					} ) }
				</Fragment>
