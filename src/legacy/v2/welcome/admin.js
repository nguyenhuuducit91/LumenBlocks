...
import { fetchSettings } from '~lumen/utils/admin'
...
export class BlockToggler extends Component {
	...
	componentDidUpdate( prevProps, prevState ) {
		...
		this.ajaxTimeout = setTimeout( () => {
			ajaxSend( 'lumen_update_disable_blocks_v2', {
				success: () => {
...
export const OptimizationSettings = ( { searchSettings } ) => {
	...
	useEffect( () => {
		fetchSettings().then( response => {
			setOptimizeScriptLoad( !! response.lumen_optimize_script_load )
		} )
	}, [] )

	const updateOptimizeScriptLoad = value => {
		const model = new models.Settings( { lumen_optimize_script_load: value } ) // eslint-disable-line camelcase
		model.save()
	...
	return <Fragment>
		<AdminToggleSetting
			...
			enabled={ __( 'Load only in posts with Lumen blocks', i18n ) }
		/>
	</Fragment>
}
