export const openPanelId = panelId => {
	...
	// they're not open.
	if ( ! panelToOpen ) {
		const selectorsToTry = [ `.lmb-tab--style`, `.lmb-tab--layout`, `.lmb-tab--section`, `.lmb-tag--advanced` ]
...
const getPanel = panelId => {
	return document.querySelector( `.lmb-panel--${ panelId }` )
}

const getTabOfPanel = panelEl => {
	const panelContainer = panelEl.closest( '.lmb-inspector-panel-controls' )
	if ( ! panelContainer ) {
		return null
	}
	const panelTabMatch = panelContainer.getAttribute( 'class' ).match( /lmb-panel-(\w+)/ )
	if ( panelTabMatch ) {
		const panelTabName = panelTabMatch[ 1 ]
		return document.querySelector( `.lmb-tab--${ panelTabName }` )
	}
	return null
}
...
export const scrollPanelIntoView = panelEl => {
	const tabs = document.querySelector( '.lmb-panel-tabs' )
