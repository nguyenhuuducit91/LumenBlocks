 ...
 */
addFilter( 'lumen.user-design-library.save.ignore', 'lumen/unique-class', attributesToIgnore => {
	return [ ...attributesToIgnore, 'uniqueClass' ]
} )

/**
 * Always add the uniqueClass attribute to all Lumen blocks.
 */
addFilter( `lumen.block.attributes`, 'lumen/unique-class', attributes => {
	return {
		...attributes,
		uniqueClass: {
			type: 'string',
