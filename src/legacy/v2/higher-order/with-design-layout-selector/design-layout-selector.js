...
import { getDesigns, getDesign } from '~lumen/design-library'
...
const DesignLayoutSelector = props => {
	...
	useEffect( () => {
		...
		const layouts = applyFilters( `lumen.${ name }.edit.layouts`, [] ).map( layout => ( { ...layout, plan: layout.premium ? 'premium' : 'free' } ) )
	...
	const layoutItems = !! layouts.length && (
		<div className="lmb-design-layout-selector__layout-items">
			{ ( layouts.filter( layout => ! applyFilters( `lumen.${ name }.edit.inspector.layout.excludeDesigns`, [] ).includes( layout.value ) ) || [] ).map( layout => (
				<DesignLayoutSelectorItem
					className="lmb-design-layout-selector__layout-item"
					onClick={ () => {
	...
	const designClassNames = classnames( 'lmb-design-layout-selector__design-library', { 'is-busy': isDesignBusy } )
	const designItems = !! designs.length && (
		<div className={ designClassNames }>
			<div className="components-placeholder__fieldset lmb-design-layout-selector__design-container">
				{ isDesignBusy && <div className="lmb-design-layout-selector__spinner" data-testid="spinner"><Spinner /></div> }
				<div className="lmb-design-layout-selector__design-items">
					{ ( designs || [] ).map( design => {
						const passedProps = {
							image: design.image,
							plan: design.plan,
						...
						return (
							<DesignLayoutSelectorItem
								className="lmb-design-layout-selector__design-item"
								onClick={ () => {
	...
	const classNames = classnames( 'lmb-design-layout-selector', { 'is-busy': isBusy } )
	return (
		<Placeholder
			className={ classNames }
			label={ label }
		>
			{ isBusy && <div className="lmb-design-layout-selector__spinner" data-testid="spinner"><Spinner /></div> }
			<div className="lmb-design-layout-selector__content">
				{ !! layouts.length &&
			...
			<div className="lmb-design-layout-selector__close-button">
				<ButtonGroup>
					<Button
						isLink
						isLarge
				...
				{ ! isPro && showProNotice && ( ( !! layouts.length && layouts.some( layout => layout.plan === 'premium' ) ) || ( !! designs.length && designs.some( design => design.plan === 'premium' ) ) ) &&
					<p className="lmb-design-layout-selector__notice">{ __( 'You can hide premium hints in the settings', i18n ) }</p>
				}
			</div>
		</Placeholder>
	)
