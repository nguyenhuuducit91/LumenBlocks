...
export const isInvalid = ( block, allowedTags = ALLOWED_ERROR_TAGS ) => {
	...
	if ( ! name || ! name.match( /^lmb\// ) ) {
		return false
	}

	// Only do this for blocks with .lmb-main-block and not separator block.
	if ( ! name.match( /separator/ ) && ! originalContent.match( /lmb-main-block/ ) ) {
		return false
	}
	...
	// Check whether we're missing a Lumen class.
	if ( isMissingLumenClass( validationIssues[ 0 ] ) ) {
		return true
	}
...
/**
 * Checks whether the validation error is because of a missing / additional Lumen classname.
 *
 * @param {Array} issue The invalidation object
 *
 * @return {boolean} True or false
 */
export const isMissingLumenClass = issue => {
	if ( ! issue.args ) {
		return false
	}
	...
	return ! isEqual( issue.args[ 2 ].match( /lmb-[^\s]+/g ), issue.args[ 3 ].match( /lmb-[^\s]+/g ) )
}
