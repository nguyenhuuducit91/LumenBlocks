...
const disableBlockWarnings = () => {
	...
	// warningStyle.setAttribute( 'id', 'lmb-disable-block-warnings' )
	// warningStyle.innerHTML = '.has-warning[data-type^="lmb/"] { opacity: 0 !important; }'
	// document.body.appendChild( warningStyle )
}

// Remove the styles that hides the flash of errored blocks.
const enableBlockWarnings = () => {
	const warningStyle = document.querySelector( '#lmb-disable-block-warnings' )
	if ( warningStyle ) {
...
export const recoverBlocks = blocks => {
	return blocks.map( _block => {
		...
		// If the block is a reusable block, recover the Lumen blocks inside it.
		if ( _block.name === 'core/block' ) {
			...
			if ( recoveryCalled ) {
				console.log( 'Lumen notice: block ' + block.name + ' (' + block.clientId + ') was auto-recovered, you should not see this after saving your page.' ) // eslint-disable-line no-console
				return {
