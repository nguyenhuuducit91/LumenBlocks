...
const registerBlock = ( name, settings = {} ) => {
	...
	const blockSettings = {
		...
		modules: applyFilters( `lumen.${ blockName }.modules`, settings.modules || {} ),
	...
	blockSettings.attributes = applyFilters( `lumen.block.attributes`, blockSettings.attributes )
	blockSettings.attributes = applyFilters( `lumen.${ blockName }.attributes`, blockSettings.attributes )

	blockSettings.edit = withMainClassname( name )( blockSettings.edit )
	blockSettings.save = withMainClassname( name )( blockSettings.save )

	// Register the block.
	registerBlockType( name, applyFilters( `lumen.${ blockName }.settings`, blockSettings ) )
