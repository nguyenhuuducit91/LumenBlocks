 ...
 */
import { fetchDesignLibrary } from '~lumen/design-library'
...
addFilter( 'lumen.design-library.versions', 'lumen/v2', versions => {
	versions.push( 'v2' )
	return versions
} )

// Load the v2 design library when switched to.
addFilter( 'lumen.design-library.modal-component', 'lumen/v2', ( component, version ) => {
	return version === 'v2' ? ModalDesignLibrary : component
} )

// Get the design data for v2 designs.
addFilter( 'lumen.design-library.get-design', 'lumen/v2', async ( design, designId, meta, version ) => {
	if ( version === 'v2' ) {
...
export const fetchBlockDesigns = async block => { // Always v2
	...
	if ( ! blockDesigns[ blockName ] ) {
		const results = await apiFetch( {
			path: `/lumen/v2/block_designs/${ blockName }`,
			method: 'GET',
		} )
...
addAction( 'lumen.design-library.reset-cache', 'lumen/v2', () => {
	blockDesigns = {}
} )
...
applyFilters( 'lumen.design-library.search-properties', 'lumen/v2', ( properties, version ) => {
	if ( version === 'v2' ) {
		return [
			...properties,
			'block',
