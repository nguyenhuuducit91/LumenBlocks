...
if ( ! hasFilter( 'lumen.div-background.output.after', 'lumen/container-link' ) ) {
	addFilter( 'lumen.div-background.output.after', 'lumen/container-link', ( output, props, blockProps ) => {
		const {
			index = 1,
			className,
		} = props

		if ( ! className.match( /lmb-container-link/ ) ) {
			return output
		}
