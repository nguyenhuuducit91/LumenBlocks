/**
 * External dependencies
 */
import {
	AdvancedSelectControl,
	AdvancedRangeControl,
	PanelAdvancedSettings,
} from '~lumen/ui'
import {
	createAllCombinationAttributes,
	appendImportant,
	__getValue,
} from '~lumen/utils'
import { ResponsiveControl } from '../../components'

/**
 * WordPress dependencies
 */
import {
	__, _x, sprintf,
} from '@wordpress/i18n'
import {
	addFilter, applyFilters, doAction,
} from '@wordpress/hooks'
import deepmerge from 'deepmerge'
import { Fragment } from '@wordpress/element'
import { i18n } from 'lumen'

const inspectorControls = ( blockName, options ) => ( output, props ) => {
	const { setAttributes } = props
	const {
		blockTag = 'div',
	} = props.attributes

	return (
		<Fragment>
			{ output }
			<PanelAdvancedSettings
				title={ __( 'General', i18n ) }
				initialOpen={ false }
			>
				{ applyFilters( `lumen.${ blockName }.edit.advanced.general.before`, null, props ) }
				{ options.blockTag &&
					<AdvancedSelectControl
						label={ sprintf( _x( '%s HTML Tag', 'component', i18n ), __( 'Block', i18n ) ) }
						value={ blockTag }
						options={ [
							{ value: '', label: __( 'Default', i18n ) },
							{ value: 'div', label: _x( 'Div', 'HTML Tag', i18n ) },
							{ value: 'blockquote', label: _x( 'Blockquote', 'HTML Tag', i18n ) },
							{ value: 'section', label: _x( 'Section', 'HTML Tag', i18n ) },
							{ value: 'article', label: _x( 'Article', 'HTML Tag', i18n ) },
							{ value: 'aside', label: _x( 'Aside', 'HTML Tag', i18n ) },
							{ value: 'main', label: _x( 'Main', 'HTML Tag', i18n ) },
							{ value: 'header', label: _x( 'Header', 'HTML Tag', i18n ) },
							{ value: 'footer', label: _x( 'Footer', 'HTML Tag', i18n ) },
							{ value: 'nav', label: _x( 'Nav', 'HTML Tag', i18n ) },
							{ value: 'address', label: _x( 'Address', 'HTML Tag', i18n ) },
							{ value: 'hgroup', label: _x( 'Hgroup', 'HTML Tag', i18n ) },
						] }
						onChange={ blockTag => setAttributes( { blockTag } ) }
					/>
				}

				{ options.opacity &&
					<ResponsiveControl
						attrNameTemplate="%sBlockOpacity"
						setAttributes={ setAttributes }
						blockAttributes={ props.attributes }
					>
						<AdvancedRangeControl
							label={ __( 'Opacity', i18n ) }
							min={ 0.0 }
							max={ 1.0 }
							step={ 0.1 }
							allowReset={ true }
							placeholder="1.0"
							className="lmb--help-tip-advanced-opacity"
						/>
					</ResponsiveControl>
				}

				{ options.zIndex &&
					<ResponsiveControl
						attrNameTemplate="%sBlockZIndex"
						setAttributes={ setAttributes }
						blockAttributes={ props.attributes }
					>
						<AdvancedRangeControl
							label={ __( 'Z-Index', i18n ) }
							min={ -100 }
							max={ 1000 }
							allowReset={ true }
							placeholder="1"
							className="lmb--help-tip-advanced-zindex"
						/>
					</ResponsiveControl>
				}

				{ applyFilters( `lumen.${ blockName }.edit.advanced.general.after`, null, props ) }
			</PanelAdvancedSettings>
		</Fragment>
	)
}

const addToStyleObject = () => ( styleObject, props ) => {
	const getValue = __getValue( props.attributes )

	const blockClass = `.${ props.mainClassName }`

	const others = {
		[ blockClass ]: {
			zIndex: appendImportant( getValue( 'blockZIndex' ) ),
			opacity: appendImportant( getValue( 'blockOpacity' ) ),
		},
		tablet: {
			[ blockClass ]: {
				zIndex: appendImportant( getValue( 'tabletBlockZIndex' ) ),
				opacity: appendImportant( getValue( 'tabletBlockOpacity' ) ),
			},
		},
		mobile: {
			[ blockClass ]: {
				zIndex: appendImportant( getValue( 'mobileBlockZIndex' ) ),
				opacity: appendImportant( getValue( 'mobileBlockOpacity' ) ),
			},
		},
	}

	return deepmerge.all( [ styleObject, others ] )
}

const addAttributes = attributes => {
	return {
		...attributes,

		blockTag: {
			type: 'string',
			default: '',
		},
		...createAllCombinationAttributes(
			'%sBlockOpacity',
			{
				type: 'number',
				default: '',
			},
			[ '', 'Tablet', 'Mobile' ]
		),
		...createAllCombinationAttributes(
			'%sBlockZIndex',
			{
				type: 'number',
				default: '',
			},
			[ '', 'Tablet', 'Mobile' ]
		),
	}
}

const advancedBlockGeneral = ( blockName, options = {} ) => {
	const optionsToPass = {
		blockTag: true,
		opacity: true,
		zIndex: true,
		modifyStyles: true,
		...options,
	}

	addFilter( `lumen.${ blockName }.edit.inspector.advanced.before`, `lumen/${ blockName }/advanced-general`, inspectorControls( blockName, optionsToPass ), 2 )
	if ( optionsToPass.modifyStyles ) {
		addFilter( `lumen.${ blockName }.styles`, `lumen/${ blockName }/advanced-general`, addToStyleObject( blockName, optionsToPass ) )
	}
	addFilter( `lumen.${ blockName }.attributes`, `lumen/${ blockName }/advanced-general`, addAttributes )
	doAction( `lumen.module.advanced-general`, blockName )
}

export default advancedBlockGeneral
