...
if ( ! class_exists( 'Lumen_Disabled_Blocks_V2' ) ) {
	class Lumen_Disabled_Blocks_V2 {
		...
		function __construct() {
			if ( has_lumen_v2_frontend_compatibility() || has_lumen_v2_editor_compatibility() ) {
				...
				add_filter( 'lumen_localize_settings_script', array( $this, 'add_disabled_blocks_settings' ) );

				// Save settings ajax action handler.
				add_action( 'wp_ajax_lumen_update_disable_blocks_v2', array( $this, 'ajax_update_disable_blocks' ) );
			}
		}
		...
		public function get_disabled_blocks() {
			$disabled_blocks = get_option( 'lumen_v2_disabled_blocks' );
			if ( false === $disabled_blocks ) {
				$disabled_blocks = array();
			}
			return apply_filters( 'lumen_disabled_blocks', $disabled_blocks );
		}
		...
		function get_disabled_blocks_nonce() {
			return wp_create_nonce( 'lumen_disable_blocks' );
		}
	}

	new Lumen_Disabled_Blocks_V2();
}
