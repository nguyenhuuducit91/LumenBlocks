...
if ( ! class_exists( 'Lumen_Custom_Block_Styles' ) ) {

	/**
	 * Lumen Custom Block Styles.
	 */
    class Lumen_Custom_Block_Styles {
		...
		function __construct() {
			...
			add_filter( 'option_lumen_block_styles', array( $this, 'sanitize_stored_block_styles' ) );
		...
		public function register_block_style_settings() {
			...
			/**
			 * [
			 * { block: 'lumen/heading', styles: [{ name: 'default', data: 'some-serialized-data', slug: 'default' }] },
			 * { block: 'lumen/hero', styles: [{ name: 'Horizontal', data: 'some-serialized-data', slug: 'horizontal' }] },
			 * ]
			 */
			register_setting(
				'lumen_block_styles',
					...
					'show_in_rest' => array(
						'schema' => array(
							'items' => array(
								'type' => 'object',
								'properties' => array(
									'block' => array( // The block name e.g. lumen/heading
										'type' => 'string',
									),
		...
		/**
		 * Sanitize a block name (must be a Lumen block).
		 *
		...
		/**
		 * Sanitize parsed blocks, allowing only Lumen blocks.
		 *
		...
		public function sanitize_parsed_blocks( $blocks ) {
			...
			foreach ( $blocks as $block ) {
				...
				if ( stripos( $block['blockName'], 'lumen/' ) !== 0 ) {
					continue;
				}
		...
		public function register_route() {
			register_rest_route( 'lumen/v2', '/update_block_style', array(
				'methods' => 'POST',
			...
			register_rest_route( 'lumen/v2', '/delete_block_style', array(
				'methods' => 'POST',
		...
		public function update_block_style( $request ) {
			...
			$block_styles = get_option( 'lumen_block_styles', array() );
			...
			update_option( 'lumen_block_styles', $block_styles, 'no' );
		...
		public function delete_block_style( $request ) {
			...
			$style_post_ids = get_option( 'lumen_block_edit_posts', array() );
			if ( array_key_exists( $block . '-' . $slug, $style_post_ids ) ) {
				...
				update_option( 'lumen_block_edit_posts', $style_post_ids, 'no' );
			}
		 ...
		 *
		 * @param string $block_name The block name e.g. lumen/heading
		...
		public function redirect_to_block_style_editor( $block_name, $style_slug = 'default', $block_title = 'Block' ) {
			...
			update_post_meta( $style_post_id, 'lmn_block_name', $block_name );
			update_post_meta( $style_post_id, 'lmn_block_title', $block_title );
			update_post_meta( $style_post_id, 'lmn_style_slug', $style_slug );

			// Update the post id.
			$style_post_ids[ $block_name . '-' . $style_slug ] = $style_post_id;
			update_option( 'lumen_block_edit_posts', $style_post_ids );
