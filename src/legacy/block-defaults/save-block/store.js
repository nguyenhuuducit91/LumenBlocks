...
const STORE_REDUCER = ( state = DEFAULT_STATE, action ) => {
	switch ( action.type ) {
		...
		case 'DELETE_BLOCK_STYLE': {
			...
			apiFetch( {
				path: `/lumen/v2/delete_block_style/`,
				method: 'POST',
				data: {
		...
		case 'UPDATE_BLOCK_STYLE': {
			...
			// Save in the database.
			apiFetch( {
				path: `/lumen/v2/update_block_style/`,
				method: 'POST',
				data: {
					block: action.blockName,
					name: action.name,
...
registerStore( 'lumen/block-styles', {
...
domReady( () => {
	const updateBlockStyles = () => {
		fetchSettings().then( response => {
			const {
				lumen_block_styles: blockStyles,
			} = response
			dispatch( 'lumen/block-styles' ).updateBlockStylesStore( {
				blockStyles,
			} )
		} )
	}
