/**
 * This is only used when editing a lumen_temp_post post. See `src/custom-block-styles.php`
 */
...
const performSave = () => {
	const {
		lmn_block_name: blockName,
		lmn_block_title: blockTitle,
		lmn_style_slug: styleSlug,
		...
		setTimeout( () => { // We need to delay this for this to show up.
			dispatch( 'core/notices' ).createNotice(
				...
				sprintf( __( 'Error saving block, please make sure you only have a %s block', i18n ), blockTitle ),
				{
					type: 'snackbar',
					isDismissible: true,
					id: 'lmn-block-style-saved', // This is the "save post" notice id, override the "Please wait" message.
				}
			)
		}, 100 )
		return
			...
			setTimeout( () => {
				...
				dispatch( 'core/notices' ).createNotice(
					...
					message,
					{
						type: 'snackbar',
						isDismissible: true,
						id: 'lmn-block-style-save-done', // This is the "save post" notice id, override the "Please wait" message.
					}
				)
			}, 100 )
		} )
...
// Listen to editor updates when editing a 'lumen_temp_post'
subscribe( () => {
	const postType = select( 'core/editor' )?.getCurrentPostType()
	if ( postType && postType === 'lumen_temp_post' ) {
...
subscribe( () => {
	...
	if ( postType && postType === 'lumen_temp_post' ) {
		...
		setTimeout( () => {
			...
			const { lmn_block_name: blockName } = select( 'core/editor' ).getEditedPostAttribute( 'meta' )
			if ( ! select( 'core/editor' ).getEditedPostContent() ) {
				if ( blockName ) {
					...
					// Allow lmnSaveBlockStyle definition to define the blocks
					// that will be added to the style editor.
					const blockEditorBlocks = getBlockSupport( blockName, 'lmnSaveBlockStyle' )
					let blocks
