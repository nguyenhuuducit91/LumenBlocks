 ...
 */
import { getDefinedBlockStyles } from '~lumen/hooks'
import { CONTENT_ATTRIBUTES, recursivelyAddUniqueIdToInnerBlocks } from '~lumen/utils'
...
export const useSavedDefaultBlockStyle = blockProps => {
	...
	if ( ! name.startsWith( 'lumen/' ) || attributes.uniqueId ) {
		return
	}
	...
	if ( ! isApplied ) {
		...
		if ( ! hasVariations ) {
			...
			const enable = applyFilters( 'lumen.block-default-styles.use-saved-style', true, blockProps, parentBlocks )
			if ( ! enable ) {
				return
			}

			// Get the default block styles.
			const blockStyle = select( 'lumen/block-styles' ).getDefaultBlockStyle( name )
			if ( ! blockStyle ) {
				return
			}
			...
			if ( blockData.innerBlocks?.length && name !== 'lumen/icon-list' ) {
