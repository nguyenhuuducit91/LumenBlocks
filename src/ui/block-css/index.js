...
const BlockCss = props => {
	const {
		...
		versionAdded = '', // The version of Lumen when this style was added.
		versionDeprecated = '', // The version of Lumen when this style wasn't used anymore.
	...
	if ( hasCollapsed ) {
		if ( generateForAllBlockStates ) {
			collapsedSelector = prependClass( selector, '%h :where(.lmn-block-accordion.lmn--is-open) .%s' )
		} else if ( blockState === 'collapsed' ) {
			collapsedSelector = prependClass( selector, ':where(.lmn-block-accordion.lmn--is-open) .%s' )
		} else {
			collapsedSelector = prependClass( selector, ':where(.lmn-block-accordion.lmn--is-open) .%s' )
		}
	}
	...
	if ( hasParentHover ) {
		if ( generateForAllBlockStates ) {
			parentHoverSelector = [ prependClass( selector, '%h.%s.lmn--is-hovered' ), prependClass( selector, ':where(.lmn-hover-parent:hover, .lmn-hover-parent.lmn--is-hovered) .%s' ) ]
		} else if ( blockState === 'parent-hover' ) {
			parentHoverSelector = prependClass( selector, '.%s.lmn--is-hovered' )
		} else {
			parentHoverSelector = prependClass( selector, ':where(.lmn-hover-parent:hover, .lmn-hover-parent.lmn--is-hovered) .%s' )
		}
	}
	...
	if ( hasHover ) {
		...
		// If there is a [data-block] append the :hover or .lmn-is-hovered directly to it.
			if ( generateForAllBlockStates ) {
				hoverSelector = [ appendClass( selector, '%h.lmn--is-hovered' ), hoverSelector || appendClass( selector, ':hover' ) ]
			} else if ( blockState === 'hover' ) {
			// In editor, always use the `selector` instead of the hoverSelector.
				hoverSelector = appendClass( selector, '.lmn--is-hovered' )
			} else {
		...
		// Prepend .%s:hover to the selector.
			if ( generateForAllBlockStates ) {
				hoverSelector = [ prependClass( selector, '%h.%s.lmn--is-hovered' ), hoverSelector || prependClass( selector, '.%s:hover' ) ]
			}
			if ( blockState === 'hover' ) { // eslint-disable-line no-lonely-if
			// In editor, always use the `selector` instead of the hoverSelector.
				hoverSelector = prependClass( selector, '.%s.lmn--is-hovered' )
			} else {
	...
	if ( instanceId ) {
		...
		// Add instance id to classes. ( e.g. `lmn-abc123` -> `lmn-abc123-2`, where 2 is `instanceId`. )
	...
	if ( ! props.editorMode ) {
		blockUniqueClassName = applyFilters( 'lumen.block-css.uniqueClass.save', blockUniqueClassName, attributes )
	} else {
		blockUniqueClassName = applyFilters( 'lumen.block-css.uniqueClass.edit', blockUniqueClassName )
	}
	...
	if ( hasHover ) {
		hoverSelector = prependCSSClass( hoverSelector, blockUniqueClassName, blockUniqueClassName, wrapSelector, generateForAllBlockStates ? '.lmn-preview-state--hover' : '' )
	}
	if ( hasParentHover ) {
		parentHoverSelector = prependCSSClass( parentHoverSelector, blockUniqueClassName, blockUniqueClassName, wrapSelector, generateForAllBlockStates ? '.lmn-preview-state--parent-hover' : '' )
	}
	if ( hasCollapsed ) {
		collapsedSelector = prependCSSClass( collapsedSelector, blockUniqueClassName, blockUniqueClassName, wrapSelector, generateForAllBlockStates ? '.lmn-preview-state--collapsed' : '' )
	}
	...
	if ( ! props.editorMode ) {
		...
		css = applyFilters( 'lumen.block-styles.save', css, blockUniqueClassName, attributes )
	}
...
const BlockCssEdit = props => {
	...
	output = applyFilters( 'lumen.block-styles.edit', output, getUniqueBlockClass( attributes.uniqueId ) )
...
function createCssEdit( selector, rule, value, device = 'desktop', vendorPrefixes = [] ) {
	...
	const { tablet: tabletBreakpoint, mobile: mobileBreakpoint } = applyFilters(
		'lumen.block-css.editor-preview-breakpoints',
		{
			tablet: 782,
			mobile: 480,
		}
