...
export const BlockStylesControl = props => {
	...
	useEffect( () => {
		...
		const selected = list.querySelector( '.lmb-block-styles-controls__selected' )
	...
	const globalBlockStyles = useSelect( select => {
		const globalBlockStyles = select( 'lumen/global-block-styles' ).getBlockStyles( blockName )
	...
	const onSelectDefaultBlockStyle = () => {
		...
		dispatch( 'lumen/hover-state' ).updateHoverState( 'normal' )
	}
	...
	const SaveUpdateModal = applyFilters( 'lumen.global-settings.global-block-styles.save-update-modal', Fragment )

	return (
		<>
			<div className={ mainClasses } >
				<div
					className={ `lmb-block-styles-controls__wrapper ${ isModified && inBlockStyleOptions ? 'has-modified' : '' }` }
				>
					<Button
						variant="tertiary"
						className={ classnames( 'lmb-block-styles-controls__block-style-button', {
						...
						ref={ buttonRef }
					>
						{ `${ __( 'Block Style', i18n ) }:` } <wbr /> { blockStyleLabel }{ isModified && inBlockStyleOptions ? <span className="lmn-panel-modified-indicator lmn--visible"></span> : '' }
			...
			{ openPopover && (
				<Popover
					className="lmb-button-icon-control__popover lmb-block-styles-controls__popover"
				...
				>
					<PanelBody>
						...
						<ul className="lmb-block-styles-controls__list" ref={ blockStylesListRef }>
							<li>
								<Button
									onClick={ () => onSelectDefaultBlockStyle() }
									className={ ! blockStyle ? 'lmb-block-styles-controls__selected' : '' }
									tabIndex={ 0 }
								>
									{ ! blockStyle && <span className="lmb-block-styles-controls__selected-icon"> <Dashicon icon="saved" /> </span> }
									<span className="lmb-block-styles-controls__label">{ __( 'Default', i18n ) }</span>
								</Button>
							</li>
							{ globalBlockStyles.map( ( option, index ) => {
								return <li key={ index }>
									<Button
										...
										className={ blockStyle === option.slug ? 'lmb-block-styles-controls__selected' : '' }
										tabIndex={ 0 }
										data-slug={ option.slug }
										data-name={ option.name }
									>
										{ blockStyle === option.slug && <span className="lmb-block-styles-controls__selected-icon"> <Dashicon icon="saved" /> </span> }
...
const SaveUpdateButtons = props => {
	...
	const ActionButtons = applyFilters( 'lumen.global-settings.global-block-styles.action-buttons', Fragment )
