 ...
 */
import { AdvancedTextControl, Popover } from '~lumen/ui'
...
const convertSizeByUnit = ( value, toUnit = '%', side = 'height', imgEl ) => {
	...
	// Convert to %
	const imgResizerEl = imgEl?.closest( '.lmn-img-resizer' )
...
const ResizerTooltip = props => {
	...
	const className = classnames( [
		...
		'lmn--is-editing': isEditing,
	} )

	const popupClassName = classnames( [
		'lmn-resizer-popup',
	], {
		'lmn--is-wide': props.enableWidth && props.enableHeight,
	} )
	...
	const widthControl = <AdvancedTextControl
		...
		className="lmn-resizer-popup__size"
		...
		onChangeUnit={ unit => {
			...
			if ( prevSwitchedWidth === null ) {
				...
				const imgEl = tooltipRef.current?.parentElement.querySelector( '.lmn-img' )
				...
				>
					<div ref={ popupRef }>
						{ props.enableWidth && props.enableHeight &&
							<BaseControl
								help={ props.help }
								className={ classnames( 'lmn-control lmn--no-padding', props.className ) }
								__nextHasNoMarginBottom
							>
								<div className="lmn-control-label">
									<div className="components-base-control__label"><h3>{ sprintf( __( '%s Size', i18n ), props.label ) }</h3></div>
								</div>
								<div className="lmn-resizer-popup__control-wrapper">
									{ widthControl }
									<span className="lmn-resizer-popup__x">×</span>
									{ heightControl }
								</div>
							</BaseControl>
						}
