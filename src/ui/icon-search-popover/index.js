...
import { faGetIcon, faFetchIcon } from '~lumen/utils'
...
/**
 * Adds a new class "lmb-custom-icon" to an SVG string.
 *
 ...
 */
export const addCustomIconClass = ( svgString, customClass = 'lmb-custom-icon' ) => {
...
const IconSearchPopover = props => {
	...
	const uploadSvg = event => {
		...
		input.onchange = e => {
			...
			fr.onload = function( e ) {
				...
				doAction( 'lumen.icon-search-popover.svg-upload', svgString )
	...
	const labelContainerClasses = classnames( [
		'lmb-icon-popover__label-container',
	], {
		'lmb-icon--has-settings': isPro,
		'lmb-icon--has-upload': allowSVGUpload,
		'lmb-icon--has-reset': props.allowReset,
	} )

	const IconLibraryIcons = applyFilters( 'lumen.global-settings.inspector.icon-library.icons', Fragment )

	const content = (
		<div className="lmn-icon-search-popover-container">
			<FileDrop
			...
			>
				<div className={ labelContainerClasses }>
					<TextControl
						className="lmb-icon-popover__input"
						value={ value }
						onChange={ setValue }
					...
					{ isPro &&
						<Button
							className="lmb-icon-popover__settings-button"
							icon="admin-generic"
				...
				<div className="lmb-icon-popover__iconlist">
					{ isBusy && <Spinner /> }
					...
					{ ! isBusy && results.faIcons.map( ( { prefix, iconName }, i ) => {
						...
						return <button
							key={ i }
							className={ `components-button lmb-prefix--${ prefix } lmb-icon--${ iconName }` }
							onClick={ async () => {
				...
				{ allowSVGUpload && isDropping &&
					<div className="lmb-icon-popover__drop-indicator">
