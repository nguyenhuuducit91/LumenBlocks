...
import { i18n, settings as lumenSettings } from 'lumen'
...
const FourRangeControl = memo( props => {
	...
	const lockClassNames = classnames( [
		'lmb-four-range-control__lock',
	], {
		'lmb--is-locked': props.hasLock && isLocked,
	} )
	...
	const isMarkModeDefault = !! ( lumenSettings?.lumen_use_size_presets_by_default ?? true )
	...
	const stepSupport = ( isMarkMode, initialValue, initialOnChange, conversionKey = null ) => {
		...
		if ( props.marks ) {
			...
			controlProps.className += 'lmn-range-control--with-marks'
			controlProps.className += isMarkMode ? ' lmn-range-control--mark-mode' : ''
		}
	...
	return (
		<AdvancedControl { ...controlProps }>
				...
				<Fragment>
					...
					>
						{ props.allowCustom && props.marks && (
							<Button
								className="lmn-range-control__custom-button"
								size="small"
								variant="tertiary"
								onClick={ () => {
			...
			{ isLocked && props.vhMode && (
				<Fragment>
					<div className="lmb-four-range-control__range">
						<Tooltip text={ __( 'Top and Bottom', i18n ) }>
							<span className="lmb-four-range-control__icon"><SVGVerticalImage /></span>
						</Tooltip>
						<RangeControl
							{ ...propsToPassVertical }
					...
					<div className="lmb-four-range-control__range">
						<Tooltip text={ __( 'Left and Right', i18n ) }>
							<span className="lmb-four-range-control__icon"><SVGHorizontalImage /></span>
						</Tooltip>
						<RangeControl
					...
					{ props.enableTop &&
						<div className="lmb-four-range-control__range">
							<Tooltip text={ props.isCorner ? __( 'Top Left', i18n ) : __( 'Top', i18n ) }>
								<span className="lmb-four-range-control__icon">{ props.isCorner ? <SVGUpperLeftImage /> : <SVGTopImage /> }</span>
							</Tooltip>
							<RangeControl
								{ ...propsToPassTop }
								value={ rangeValueTop }
					...
					{ props.enableRight &&
						<div className="lmb-four-range-control__range">
							<Tooltip text={ props.isCorner ? __( 'Top Right', i18n ) : __( 'Right', i18n ) }>
								<span className="lmb-four-range-control__icon">{ props.isCorner ? <SVGUpperRightImage /> : <SVGRightImage /> }</span>
							</Tooltip>
							<RangeControl
								{ ...propsToPassRight }
					...
					{ props.enableBottom &&
						<div className="lmb-four-range-control__range">
							<Tooltip text={ props.isCorner ? __( 'Bottom Left', i18n ) : __( 'Bottom', i18n ) }>
								<span className="lmb-four-range-control__icon">{ props.isCorner ? <SVGLowerLeftImage /> : <SVGBottomImage /> }</span>
							</Tooltip>
							<RangeControl
								{ ...propsToPassBottom }
					...
					{ props.enableLeft &&
						<div className="lmb-four-range-control__range">
							<Tooltip text={ props.isCorner ? __( 'Bottom Right', i18n ) : __( 'Left', i18n ) }>
								<span className="lmb-four-range-control__icon">{ props.isCorner ? <SVGLowerRightImage /> : <SVGLeftImage /> }</span>
							</Tooltip>
							<RangeControl
								{ ...propsToPassLeft }
