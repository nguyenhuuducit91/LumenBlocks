 ...
 */
import { guidedTourStates } from 'lumen'
...
const GuidedModalTour = memo( props => {
	...
	const conditions = applyFilters( 'lumen.guided-tour.conditions', TOUR_CONDITIONS )
	...
	const ModalTour = lazy( () => import( /* webpackChunkName: "modal-tour" */ '~lumen/ui-lazy/modal-tour' ) )
		...
		<Suspense fallback={ null }>
			<ModalTour
				...
				onClose={ () => {
					...
					// Update the lumen_guided_tour_states setting
					if ( ! guidedTourStates.includes( tourId ) ) {
						// eslint-disable-next-line camelcase
						const settings = new models.Settings( { lumen_guided_tour_states: [ ...guidedTourStates, tourId ] } )
						settings.save()
					}
