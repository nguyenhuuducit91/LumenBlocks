...
const AdvancedToolbarControl = props => {
	...
	const _CONTROLS = applyFilters( 'lumen.toolbar-control.controls', CONTROLS )
	const controls = typeof _controls === 'string' ? _CONTROLS[ _controls ] : _controls

	const toolbarClasses = classnames( {
		'lmb-toolbar--full-width': fullwidth,
		'lmb-toolbar--multiline': multiline,
		'lmb-toolbar--small': props.isSmall,
	} )
	...
	return (
		<AdvancedControl
			{ ...controlProps }
			className={ classnames( 'lmb-advanced-toolbar-control', className, controlProps.className ) }
		>
			<ButtonGroup
				children={
					controls.map( ( option, i ) => {
						...
						const controlProps = {
							...
							children: ! option.icon ? option.custom || <span className="lmb-advanced-toolbar-control__text-button">{ option.title }</span> : null,
						}
						return <Button
							key={ option.value }
							{ ...controlProps }
