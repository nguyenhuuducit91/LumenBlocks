...
const LabelTooltip = props => {
	...
	return (
		<div className="components-base-control__label">
			<span
				className="lmn-control__label--has-tooltip"
				onMouseEnter={ () => {
					...
					timeout.current = setTimeout( () => {
						...
						setTimeout( () => {
							document.querySelector( '.lmn-control-help-tooltip' ).focus()
						}, 100 )
					}, 800 )
				} }
				onMouseLeave={ () => {
...
domReady( () => {
	fetchSettings().then( response => {
		helpTooltipEnabled = response.lumen_help_tooltip_disabled !== '1'
	} ).catch( () => {} )
} )
...
function useHelpTooltipEnabled() {
	...
	useEffect( () => {
		...
		window?.addEventListener( '_lmnHelpTooltipEnabledChanged', onChangeHandler )

		// Clean up the event listener when the component unmounts
		return () => {
			// eslint-disable-next-line @wordpress/no-global-event-listener
			window.removeEventListener( '_lmnHelpTooltipEnabledChanged', onChangeHandler )
		}
	}, [] )
	...
	const updateHelpTooltipEnabled = newValue => {
		...
		const model = new models.Settings( {
			lumen_help_tooltip_disabled: newValue ? '' : '1', // eslint-disable-line
		} )
		model.save()

		// Let the other components know that the value has changed.
		window.dispatchEvent( new CustomEvent( '_lmnHelpTooltipEnabledChanged', { detail: newValue } ) )
	}
...
const useCloseOtherTooltips = setShowTooltip => {
	...
	useEffect( () => {
		...
		// eslint-disable-next-line @wordpress/no-global-event-listener
		window?.addEventListener( '_lmnHelpTooltipOpened', closeHandler )

		return () => {
			// eslint-disable-next-line @wordpress/no-global-event-listener
			window.removeEventListener( '_lmnHelpTooltipOpened', closeHandler )
		}
	}, [] )
	...
	const newSetShowTooltip = show => {
		if ( show ) {
			window.dispatchEvent( new CustomEvent( '_lmnHelpTooltipOpened' ) )
		}
