 ...
 */
import { useDeviceType, useWithShift } from '~lumen/hooks'
import { clamp, isEqual } from 'lodash'
import classnames from 'classnames'
import { i18n } from 'lumen'
import { getRowsFromColumns } from '~lumen/features/column/util'
...
const ResizableColumn = props => {
	...
	const allowResize = ! props.context[ 'lumen/innerBlockOrientation' ]
	const columnWrapDesktop = !! props.context[ 'lumen/columnWrapDesktop' ]
	...
	const onResizeStart = ( _event, _direction ) => {
		...
		if ( isDesktop && ! columnWrapDesktop ) {
			...
			const parentEl = editorDom.querySelector( `[data-block="${ parentBlockClientId }"] .lmn-inner-blocks` )
	...
	const onResize = ( _event, _direction, elt, delta ) => {
		...
		if ( isDesktop && ! columnWrapDesktop ) {
			...
			const columnStyles = columnPercentages.map( ( width, i ) => {
				...
				[data-block="${ adjacentBlocks[ i ].clientId }"] .lmn-resizable-column__size-tooltip {
		...
		// control.
			...
			const columnStyles = `.editor-styles-wrapper [data-block][data-block="${ clientId }"] {
					flex: 1 1 calc(${ columnPercentages }% - var(--lmn-column-gap, 0px) * ${ adjacentColumnCount - 1 } / ${ adjacentColumnCount } ) !important;
					max-width: calc(${ columnPercentages }% - var(--lmn-column-gap, 0px) * ${ adjacentColumnCount - 1 } / ${ adjacentColumnCount } ) !important;
				}
				[data-block="${ clientId }"] .lmn-resizable-column__size-tooltip {
...
const ResizableTooltip = memo( props => {
	...
	const openNextColumn = ( direction = 'right' ) => {
		...
		if ( direction === 'right' ) {
			...
			const nextTooltip = tooltipRef.current.closest( '.lmn-row' ).querySelectorAll( '.lmn-resizable-column__size-tooltip' )[ nextIndex ]
			...
			const prevTooltip = tooltipRef.current.closest( '.lmn-row' ).querySelectorAll( '.lmn-resizable-column__size-tooltip' )[ prevIndex ]
	...
	return (
		<Fragment>
			{ ! isOnlyBlock && isEditWidth &&
				<Popover
					className="lmn-resizable-column__popup"
				...
				>
					<div ref={ popupRef }>
						<AdvancedTextControl
							...
							className="lmn-resizable-column__input"
			...
			}
			{
				! isOnlyBlock && (
					<div
						{ ...props.tooltipProps }
						className="lmn-resizable-column__size-tooltip"
						ref={ tooltipRef }
