...
import { applyBlockDesign, getCleanAttributes } from '~lumen/utils'
	...
	const {
		...
		return {
			...
			getJSONBlockAttributes: () => {
				...
				return JSON.stringify( applyFilters( `lumen.${ blockName }.design.filtered-block-attributes`, cleanedAttributes ), null, 4 )
			},
		}
	} )
		...
		<>
				...
				{ ( { onClose } ) => (
					<MenuItem
						...
						onClick={ () => {
							setOpenPopover( true )
							setChanged( false )
							setTimeout( () => document.querySelector( '.lmb-modal-get-block-attributes-button textarea' ).select(), 100 )
							onClose()
						} }
					>
			...
			{ openPopover &&
				<Modal
					...
					focusOnMount={ true }
					className="lmb-modal-get-block-attributes-button"
					onRequestClose={ () => {
						if ( changed ) {
							const value = document.querySelector( '.lmb-modal-get-block-attributes-button textarea' ).value
