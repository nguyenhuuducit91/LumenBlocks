...
const StyleGuidePopover = props => {
		...
		const loadStyleGuide = async () => {
			try {
				...
				const { default: StyleGuideComponent } = await import(
					...
					/* webpackMode: "lazy" */
					'../../ui-lazy/style-guide/editor-style-guide'
				)
	...
	return (
		<Popover
			className="lmb-style-guide-popover"
			anchor={ editorStylesWrapper }
			placement="overlay"
			style={ style }
		>
			<Button
				className="lmb-style-guide-popover__close-button"
				variant="tertiary"
				onClick={ onClose }
			...
			<div className="lmb-style-guide-popover__heading">
				<ExportButton printRef={ styleGuideRef } />
				<h1 className="lmb-style-guide-popover__title">{ __( 'Design System Style Guide', i18n ) }</h1>
				<p className="lmb-style-guide-popover__description">
			...
			{ isLoading ? (
				<div className="lmb-style-guide__loading">
					{ __( 'Loading style guide…', i18n ) }
				</div>
			) : StyleGuide ? (
				<StyleGuide { ...props } contentRef={ styleGuideRef } />
			) : (
				<div className="lmb-style-guide__error">
...
const ExportButton = props => {
	...
	return (
		<Button
			className="lmb-style-guide__print-button"
			variant="secondary"
			onClick={ handlePrint }
