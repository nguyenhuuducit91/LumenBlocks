...
const ProControl = props => {
	const classNames = classnames( [ 'lmb-design-control-pro-note', props.className ] )
	const description = props.description || LABELS[ props.type ]?.description

	return (
		<div className={ classNames }>
			{ props.isDismissible && <Button className="lmb-design-control-pro-note__close" icon="no-alt" isTertiary onClick={ props.onClose } /> }
			{ props.showImage && <img src={ prependBaseUrl( ImageProIcon ) } className="lmb-design-control-pro-note__logo" alt="" /> }
			<h4>{ props.title || LABELS[ props.type ]?.title || __( 'This Is a Premium Feature', i18n ) }</h4>
			{ description && (
				<div className="lmb-design-control-pro-note__description">{ description }</div>
			) }
			{ props.showButton && (
				<div>
					{ props.learnMoreUrl && (
			...
			{ props.demoUrl && (
				<p className="lmb-design-control-pro-note__demo-link">
					<a href={ props.demoUrl } target="_premium" className="button button-secondary">{ __( 'View Demo', i18n ) }</a>
				</p>
			) }
			<p className="lmb-design-control-pro-note__notice">{ __( 'Part of the complete website builder in Lumen Premium.', i18n ) }</p>
		</div>
	)
}
