/**
 * A Panel for Lumen-built designs
 */
/**
 * External dependencies
 */
import { DesignControl, DesignPanelBody } from '~lumen/ui'
...
function PanelDesignLibrary( props ) {
	const {
		block = 'all',
		options = applyFilters( `lumen.${ block }.edit.templates`, props.options || [] ),
		className = '',
		title = __( 'Designs', i18n ),
		help = __( 'Pick a design to start from, this will override your block settings', i18n ),
	} = props

	const mainClasses = classnames( [ 'lmb-panel-design-settings', className ] )
