...
const ControlIconToggle = props => {
	...
	useEffect( () => {
		...
		const clickOutsideListener = event => {
			if ( isOpen ) {
				const closest = event.target?.closest( '.lmn-label-unit-toggle' )
	...
	const className = classnames( [
		props.className,
		'lmn-label-unit-toggle',
	], {
		'lmn-label-unit-toggle__colored': props.hasColors,
		'is-open': isOpen,
	} )
	...
	return (
		...
		>
			<div
				className={ classnames( 'lmn-label-unit-toggle__wrapper', { 'is-open': isOpen } ) }
