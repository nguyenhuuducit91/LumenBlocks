...
const registerLumenBlock = ( name, settings = {} ) => {
...
export const blockSaveRender = ( name, settings, attributes = null ) => {
	if ( ! getBlockType( name ) ) {
		registerLumenBlock( name, { ...settings, category: 'common' } )
	}
...
export const getSavedBlockHTML = ( blockName, blockSettings, attributes = {}, innerBlocks = [] ) => {
	const save = registerLumenBlock( blockName, { ...blockSettings, category: 'common' } ).save
