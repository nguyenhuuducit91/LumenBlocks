...
export class LumenFixture {
	...
	// Wait for Lumen Settings to be fetched
	async waitForSettings(): Promise<void> {
		return new Promise( ( resolve, reject ) => {
			...
			const finishedCallback = async ( request: Request ) => {
				if ( decodeURIComponent( request.url() ).includes( 'lumen/v3/settings' ) && request.method() === 'GET' ) {
					try {
						let settings = null
						await test.step( 'Wait for Lumen settings to load', async () => {
						...
						if ( settings ) {
							...
							throw Error( 'Failed to get Lumen settings' )
						}
					} catch ( error ) {
						cleanup()
	...
	// Wait for Lumen Settings to be saved via the WordPress REST API.
