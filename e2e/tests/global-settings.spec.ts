...
test.describe( 'Global Settings', () => {
	...
	test( 'When a color is added in the Global Colors, it should be present in the color picker', async ( {
		...
		await page.getByLabel( 'Lumen Design System' ).click()
		await page.getByRole( 'button', { name: 'Global Color Palette' } ).click()

		// Add a new Global Color
		const panel = page.locator( '.lmb-global-settings-color-picker ' ).filter( { hasText: 'Global Colors' } )
		await panel.locator( 'button.lmb-global-settings-color-picker__add-button' ).click()

		const globalColors = panel.locator( '.lmb-global-settings-color-picker__color-indicators > div' )
		...
		await page.locator( '.lmn-color-palette-control .lmn-control-content > .components-dropdown > .components-button' ).first().click()
	...
	test( 'Global Typography Styles should be applied when adding a heading', async ( {
		...
		await page.locator( '.lmb-global-settings-typography-control' ).nth( 1 ).locator( '.components-base-control__field > .lmb-button-icon-control__wrapper > .components-button' ).click()
		await page.locator( '.lmn-popover .components-base-control:nth-of-type(4)', { hasText: /Transform/ } ).getByRole( 'listbox' ).selectOption( 'uppercase' )
		await page.locator( '.lmb-global-settings-typography-control' ).nth( 1 ).locator( '.components-base-control__field > .lmb-button-icon-control__wrapper > .components-button' ).click()
		...
		// Check if the added Lumen Heading Block has a text-transform uppercase
		editor.insertBlock( {
			name: 'lumen/heading',
			attributes: {
				text: 'test',
		...
		await expect( editor.canvas.locator( '[data-type="lumen/heading"] > .lmn-block-heading > h2[role="textbox"]' ) ).toHaveCSS( 'text-transform', 'uppercase' )

		// Reset Global Typography Styles
		await page.getByLabel( 'Lumen Design System' ).click()

		const resetButton = page.locator( '.lmb-global-settings-typography-control' ).nth( 1 ).getByLabel( 'Reset' )
		const deleteRequest = page.waitForResponse( response => response.url().includes( 'wp/v2/settings' ) && response.request().method() === 'POST' )
