...
test.describe( 'Block Editor', () => {
	...
	test( 'Lumen blocks can be added in the editor', async ( {
		page,

		editor,
	} ) => {
		// Insert Lumen Text Block through block inserter
		// Also checks if Lumen Block is in the list of blocks in the Editor
		...
		await page.locator( '.editor-block-list-item-lumen-text' ).click()

		await expect( editor.canvas.getByLabel( 'Block: Text' ) ).toBeVisible()
	} )

	test( 'Lumen Inspector Controls should show up upon clicking a Lumen block', async ( {
		page,
		editor,
	} ) => {
	...
	test( 'A Lumen block\'s attributes should update when settings are changed in the Inspector Controls.', async ( {
		page,
		editor,
	} ) => {
		await editor.insertBlock( {
		...
		// Add content and color to Lumen Text Block
		await editor.canvas.locator( '[data-type="lumen/text"] > .lmn-block-text > p[role="textbox"]' ).fill( 'test' )
		...
		await expect( editor.canvas.locator( '[data-type="lumen/text"]' ) ).toContainText( 'test' )
		await expect( editor.canvas.locator( '[data-type="lumen/text"] > .lmn-block-text > p[role="textbox"]' ) ).toHaveCSS( 'color', 'rgb(255, 0, 0)' )
	...
	test( 'The Lumen block added in the editor should be visible in the frontend', async ( {
		page, editor, admin,
	} ) => {
		await editor.insertBlock( {
			name: 'lumen/text',
