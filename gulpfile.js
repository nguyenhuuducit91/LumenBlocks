...
const buildInclude = [
	...
	path.resolve( __dirname, './NOTICE.md' ), // Attribution required by GPL section 5.
...
gulp.task( 'generate-lmn-block-typesphp', function( cb ) {
	...
	const blockJsonToAssocArray = ( key, value, isArrayElement, changeToSnakeCase ) => {
		...
		const _value = needTranslate.includes( key ) ? `__( '${ value }', LUMEN_I18N )` : `'${ value }'`
...
gulp.task( 'style', gulp.series(
	...
	function styleGenerateResponsivePHP() {
		...
		return gulp.src( [ path.resolve( __dirname, './src/dynamic-breakpoints.php' ) ] )
			.pipe( replace(
				/return <<<LMN_RESPONSIVE_CSS([\s\S]*?)LMN_RESPONSIVE_CSS;/gm,
...
gulp.task( 'zip', function() {
	return gulp.src( 'build/lumen-blocks/**/*' )
